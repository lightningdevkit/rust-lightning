...
impl fmt::Debug for Event {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match self {
			&Event::FundingGenerationReady{ .. } => write!(f, "{}", "FundingGenerationReady"),
			&Event::FundingBroadcastSafe{ .. } => write!(f, "{}", "FundingBroadcastSafe"),
			&Event::PaymentReceived{ .. } => write!(f, "{}", "PaymentReceived"),
			&Event::PaymentSent{ .. } => write!(f, "{}", "PaymentSent"),
			&Event::PaymentFailed{ .. } => write!(f, "{}", "PaymentFailed"),
			&Event::PendingHTLCsForwardable{ .. } => write!(f, "{}", "PendingHTLCsForwardable"),
			&Event::SendOpenChannel{ .. } => write!(f, "{}", "SendOpenChannel"),
			&Event::SendFundingCreated{ .. } => write!(f, "{}", "SendFundingCreated"),
			&Event::SendFundingLocked{ .. } => write!(f, "{}", "SendFundingLocked"),
			&Event::UpdateHTLCs{ .. } => write!(f, "{}", "UpdateHTLCs"),
			&Event::SendShutdown{ .. } => write!(f, "{}", "SendShutdown"),
			&Event::BroadcastChannelAnnouncement{ .. } => write!(f, "{}", "BroadcastChannelAnnouncement"),
			&Event::BroadcastChannelUpdate{ .. } => write!(f, "{}", "BroadcastChannelUpdate"),
			&Event::HandleError{ .. } => write!(f, "{}", "HandleError"),
		}
	}
}
