...
Personal notes:
intuitively to disconnect a peer two things would need to happen, the peer is identified somehow and whatever action disconects the peer is called
how to identify peer:


how to perform disconnect action:
enum MessageSendEvent which contains a HandleError variant that variant contains two fields
node_id: PublicKey
action: option<msgs::ErrorAction>


pub enum ErrorAction has a DisconnectPeer variant which allows the user to specify specific error message


possibly something like peermanager.messagehabdler.pending_events.lock().unwrap().push(events::MessageSendEvent::HandleError {
			node_id: their_id,
			action: Some(msgs::ErrorAction::DisconnectPeer { msg: None }),
		});
...
pub struct ErrorMessage msgs.rs ln 155 /// An error message to be sent or received from a peerhandler
...
pub fn check_peer(pm: PeerManager<Descriptor>){


	loop{
		// probably need to acquire lock as peers is a mutex type the loop below needs to be fancier.
		//TODO: since we would be acquiring the peers lock in both cases
	for i in pm.peers.lock().unwrap().peers()iter()
		{
	...
	//probably need tp acquire lock to iterate through peers as its a mutex type
	for i in pm.peers.lock().unwrap().peers()iter(){
		//check for pong message
		// disconnect peer if need to
		pm.message_handler.chan_handler.handle_error(&peers.Peer.their_node_id.unwrap(), &msg);
