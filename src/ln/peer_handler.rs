...
impl<Descriptor: SocketDescriptor> PeerManager<Descriptor> {
						...
						if peer.pending_read_buffer_pos == peer.pending_read_buffer.len() {
										...
										Err(e) => {
											if let Some(action) = e.action {
												match action {
													...
													msgs::ErrorAction::DropChannel { msg: _ } => {
														//TODO: Try to push msg
														log_trace!(self, "Got Err handling message, dropping peer because {}", e.err);
														return Err(PeerHandleError{ no_connection_possible: false });
													},
							...
							match next_step {
								...
								NextNoiseStep::ActTwo => {
									...
									local_features.set_supports_data_loss_protect();
									...
									if peer.pending_read_is_header {
										...
										match msg_type {
											...
											32 => {
												...
												try_potential_handleerror!(self.message_handler.chan_handler.handle_open_channel(&peer.their_node_id.unwrap(), &msg, &peer.their_local_features));
											},
											33 => {
					...
					MessageSendEvent::HandleError { ref node_id, ref action } => {
						if let Some(ref action) = *action {
							match *action {
								...
								msgs::ErrorAction::DropChannel{ref msg } => {
									log_trace!(self, "Handling DropChannel HandleError event in peer_handler for node {} with message {}",
											log_pubkey!(node_id),
											msg.data);},
								msgs::ErrorAction::IgnoreError => {},
	...
	pub fn disconnect_event(&mut self, descriptor: &Descriptor) {
		self.disconnect_event_internal(descriptor, false);
	}

	fn disconnect_event_internal(&mut self, descriptor: &Descriptor, no_connection_possible: bool) {
		let mut peers = self.peers.lock().unwrap();
		peers.peers_needing_send.remove(descriptor);
