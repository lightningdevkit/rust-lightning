...
impl ChannelMonitor {
	pub(super) fn new(revocation_base_key: &SecretKey, delayed_payment_base_pubkey: &PublicKey, htlc_base_key: &SecretKey, delayed_payment_base_key: &SecretKey, our_to_self_delay: u16, destination_script: Script) -> ChannelMonitor {
		ChannelMonitor {
			funding_txo: None,
			...
			delayed_payment_base_key: delayed_payment_base_pubkey.clone(),
	...
	fn check_spend_remote_transaction(&self, tx: &Transaction, height: u32) -> (Vec<Transaction>, (Sha256dHash, Vec<TxOut>), Vec<CustomOutputScriptDescriptor>) {
		...
		if commitment_number >= self.get_min_seen_secret() {
			...
			spendable_outputs.push(CustomOutputScriptDescriptor::static_key(BitcoinOutPoint { txid: spend_tx.txid(), vout: 0 }));
					...
					for (idx, htlc) in per_commitment_data.iter().enumerate() {
						if let Some(payment_preimage) = self.payment_preimages.get(&htlc.payment_hash) {
							...
							if htlc.cltv_expiry > height + CLTV_SHARED_CLAIM_BUFFER {
								...
								spendable_outputs.push(CustomOutputScriptDescriptor::static_key(BitcoinOutPoint { txid: single_htlc_tx.txid(), vout: 0 }));
								txn_to_broadcast.push(single_htlc_tx);
							}
						}
					}
	...
	fn check_spend_remote_htlc(&self, tx: &Transaction, commitment_number: u64) -> (Option<Transaction>, Option<CustomOutputScriptDescriptor>) {
		...
		if !inputs.is_empty() {
			...
			(Some(spend_tx), Some(CustomOutputScriptDescriptor::static_key(BitcoinOutPoint { txid, vout: 0 })))
		} else { (None, None) }
	}

	fn broadcast_by_local_state(&self, local_tx: &LocalSignedTx, per_commitment_point: &Option<PublicKey>, delayed_payment_base_key: &Option<SecretKey>) -> (Vec<Transaction>, Vec<CustomOutputScriptDescriptor>) {
			...
			if htlc.offered {
				...
				if let Some(ref per_commitment_point) = *per_commitment_point {
					if let Some(ref delayed_payment_base_key) = *delayed_payment_base_key {
						if let Ok(local_delayedkey) = chan_utils::derive_private_key(&self.secp_ctx, per_commitment_point, delayed_payment_base_key) {
							spendable_outputs.push(CustomOutputScriptDescriptor::dynamic_key(BitcoinOutPoint { txid: htlc_timeout_tx.txid(), vout: 0 }, local_delayedkey, chan_utils::get_revokeable_redeemscript(&local_tx.revocation_key, self.our_to_self_delay, &local_tx.delayed_payment_key), self.our_to_self_delay));
						}
					}
				}
				...
				if let Some(payment_preimage) = self.payment_preimages.get(&htlc.payment_hash) {
					...
					if let Some(ref per_commitment_point) = *per_commitment_point {
						if let Some(ref delayed_payment_base_key) = *delayed_payment_base_key { 
							if let Ok(local_delayedkey) = chan_utils::derive_private_key(&self.secp_ctx, per_commitment_point, delayed_payment_base_key) {
								spendable_outputs.push(CustomOutputScriptDescriptor::dynamic_key(BitcoinOutPoint { txid: htlc_success_tx.txid(), vout: 0 }, local_delayedkey, chan_utils::get_revokeable_redeemscript(&local_tx.revocation_key, self.our_to_self_delay, &local_tx.delayed_payment_key), self.our_to_self_delay));
							}
						}
					}
	...
	fn check_spend_local_transaction(&self, tx: &Transaction, _height: u32) -> (Vec<Transaction>, Vec<CustomOutputScriptDescriptor>) {
	...
	fn block_connected(&self, txn_matched: &[&Transaction], height: u32, broadcaster: &BroadcasterInterface)-> (Vec<(Sha256dHash, Vec<TxOut>)>, Vec<CustomOutputScriptDescriptor>) {
		let mut watch_outputs = Vec::new();
		let mut spendable_outputs = Vec::new();
...
mod tests {
	...
	fn test_per_commitment_storage() {
...

		{
			...
			monitor = ChannelMonitor::new(&SecretKey::from_slice(&secp_ctx, &[42; 32]).unwrap(), &delayed_payment_base_key, &SecretKey::from_slice(&secp_ctx, &[43; 32]).unwrap(), &SecretKey::from_slice(&secp_ctx, &[42; 32]).unwrap(), 0, Script::new());
			secrets.clear();
	...
	fn test_prune_preimages() {
		...
		// old state.
		let delayed_payment_base_key = PublicKey::from_secret_key(&secp_ctx, &SecretKey::from_slice(&secp_ctx, &[42; 32]).unwrap());
		let mut monitor = ChannelMonitor::new(&SecretKey::from_slice(&secp_ctx, &[42; 32]).unwrap(), &delayed_payment_base_key, &SecretKey::from_slice(&secp_ctx, &[43; 32]).unwrap(), &SecretKey::from_slice(&secp_ctx, &[42; 32]).unwrap(), 0, Script::new());
