...
mod tests {
	...
	fn run_onion_failure_test<F1,F2>(_name: &str, test_case: u8, nodes: &Vec<Node>, route: &Route, payment_hash: &[u8; 32], callback_msg: F1, callback_node: F2, expected_retryable: bool, expected_error_code: Option<u16>, expected_channel_update: Option<HTLCFailChannelUpdate>)
	...
	fn run_onion_failure_test_with_fail_intercept<F1,F2,F3>(_name: &str, test_case: u8, nodes: &Vec<Node>, route: &Route, payment_hash: &[u8; 32], mut callback_msg: F1, mut callback_fail: F2, mut callback_node: F3, expected_retryable: bool, expected_error_code: Option<u16>, expected_channel_update: Option<HTLCFailChannelUpdate>)
		where F1: for <'a> FnMut(&'a mut msgs::UpdateAddHTLC),
	...
	fn test_onion_failure() {
		...
		}, ||{}, true, Some(PERM|1), Some(msgs::HTLCFailChannelUpdate::ChannelClosed{short_channel_id: channels[0].0.contents.short_channel_id, is_permanent: true}));
		...
		run_onion_failure_test("invalid_onion_version", 0, &nodes, &route, &payment_hash, |msg| { msg.onion_routing_packet.version = 1; }, ||{}, true,
		  Some(BADONION|PERM|4), Some(msgs::HTLCFailChannelUpdate::ChannelClosed{short_channel_id: channels[0].0.contents.short_channel_id, is_permanent: true}));

		run_onion_failure_test("invalid_onion_hmac", 0, &nodes, &route, &payment_hash, |msg| { msg.onion_routing_packet.hmac = [3; 32]; }, ||{}, true,
      Some(BADONION|PERM|5), Some(msgs::HTLCFailChannelUpdate::ChannelClosed{short_channel_id: channels[0].0.contents.short_channel_id, is_permanent: true}));
      ...
      Some(BADONION|PERM|6), Some(msgs::HTLCFailChannelUpdate::ChannelClosed{short_channel_id: channels[0].0.contents.short_channel_id, is_permanent: true}));
