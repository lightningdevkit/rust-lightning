...
impl<ChannelSigner: WriteableEcdsaChannelSigner> OnchainTxHandler<ChannelSigner> {
		...
		L::Target: Logger,
	{
		...
		if cached_request.is_malleable() {
				...
				if let Some(htlcs) = cached_request.construct_malleable_package_with_external_funding(self) {
					return Some((
						...
						OnchainClaim::Event(ClaimEvent::BumpHTLC {
							...
							htlcs,
							tx_lock_time: absolute::LockTime::from_consensus(cached_request.package_locktime(cur_height)),
						}),
					));
				} else {
					return None;
			...
			if let Some((output_value, new_feerate)) = cached_request.compute_package_output(
				...
				assert!(predicted_weight >= transaction.weight().to_wu() as usize);
				...
				PackageSolvingData::HolderFundingOutput(output) => {
					...
					if let Some(input_amount_sat) = output.funding_amount {
						...
						let commitment_tx_feerate_sat_per_1000_weight =
							compute_feerate_sat_per_1000_weight(fee_sat, tx.weight().to_wu() as u64);
	...
	// to monitor before.
	pub(crate) fn get_fully_signed_holder_tx(&mut self, funding_redeemscript: &ScriptBuf) -> Transaction {
	...
	pub(crate) fn get_fully_signed_copy_holder_tx(&mut self, funding_redeemscript: &ScriptBuf) -> Transaction {
