...
/// A single on-chain output claim generated by [`ChannelMonitor`].
///
/// These requests are converted to [`PackageTemplate`]s once [`OnchainTxHandler`] has deduplicated
/// them and is ready to aggregate compatible claims.
///
/// [`ChannelMonitor`]: crate::chain::channelmonitor::ChannelMonitor
/// [`OnchainTxHandler`]: crate::chain::onchaintx::OnchainTxHandler
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct ClaimRequest {
	input: (BitcoinOutPoint, PackageSolvingData),
...
impl ClaimRequest {
	pub(crate) fn new(
		...
		Self {
			input: (BitcoinOutPoint { txid, vout }, input_solving_data),
			counterparty_spendable_height,
		}
	}

	pub(crate) fn outpoint(&self) -> &BitcoinOutPoint {
		&self.input.0
	}

	pub(crate) fn into_package_template(self) -> PackageTemplate {
		let (outpoint, input_solving_data) = self.input;
...
impl PackageTemplate {
	...
	pub(crate) fn contains_outpoint(&self, outpoint: &BitcoinOutPoint) -> bool {
		self.inputs.iter().any(|(input, _)| input == outpoint)
	}
