...
pub(crate) struct ChannelMonitorImpl<Signer: EcdsaChannelSigner> {
	...
	/// move the responsibility of off-chain payment resolution from the Channel to the monitor. Will
	/// be always set once support is complete.
...
pub(crate) fn write_chanmon_internal<Signer: EcdsaChannelSigner, W: Writer>(
	...
	write_tlv_fields!(writer, {
		...
		(5, pending_monitor_events_legacy, option),
	...
	for Option<(BlockHash, ChannelMonitor<SP::EcdsaSigner>)>
{
	...
	fn read<R: io::Read>(reader: &mut R, args: (&'a ES, &'b SP)) -> Result<Self, DecodeError> {
		...
		read_tlv_fields!(reader, {
			...
			(2, persistent_events_enabled, (default_value, false)),
			...
			(39, next_monitor_event_id, option),
		});

		#[cfg(not(any(feature = "_test_utils", test)))]
		if persistent_events_enabled {
