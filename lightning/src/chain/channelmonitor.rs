...
impl HolderCommitment {
	...
	fn write_as_legacy<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
		...
		let mut nondust_htlcs =
			self.tx.nondust_htlcs().iter().zip(self.tx.counterparty_htlc_sigs.iter());
...
impl Readable for IrrevocablyResolvedHTLC {
	fn read<R: io::Read>(reader: &mut R) -> Result<Self, DecodeError> {
		...
		Ok(Self {
			commitment_tx_output_idx: if mapped_commitment_tx_output_idx == u32::MAX {
				None
			} else {
...
impl TryFrom<(HolderCommitmentTransaction, HolderSignedTx)> for HolderCommitment {
	...
	fn try_from(value: (HolderCommitmentTransaction, HolderSignedTx)) -> Result<Self, Self::Error> {
		...
		let dust_htlcs = holder_signed_tx
			.htlc_outputs
			.into_iter()
			.filter_map(|(htlc, _, source)| {
		...
		Ok(Self { tx: holder_commitment_tx, nondust_htlc_sources, dust_htlcs })
	}
}
...
impl<Signer: EcdsaChannelSigner> ChannelMonitor<Signer> {
	...
	pub(crate) fn new(
		...
		let onchain_tx_handler = OnchainTxHandler::new(
			...
			initial_holder_commitment_tx.clone(),
			secp_ctx,
		);
		...
		outputs_to_watch.insert(
			...
			vec![(funding_outpoint.index as u32, funding_script.clone())],
		);
...
impl<Signer: EcdsaChannelSigner> ChannelMonitorImpl<Signer> {
	...
	fn get_htlc_balance(
		...
		let htlc_resolved = self.htlcs_resolved_on_chain.iter().any(|v| {
		...
		if let Some(conf_thresh) = holder_delayed_output_pending {
			...
			let htlc_output_claim_pending =
				self.onchain_events_awaiting_threshold_conf.iter().any(|event| {
...
impl<Signer: EcdsaChannelSigner> ChannelMonitor<Signer> {
	...
	pub fn get_claimable_balances(&self) -> Vec<Balance> {
		...
		if let Some(txid) = confirmed_txid {
			...
			if let Some(counterparty_tx_htlcs) =
				us.funding.counterparty_claimable_outpoints.get(&txid)
			{
			...
			let to_self_value_sat =
				us.funding.current_holder_commitment.tx.to_broadcaster_value_sat();
			res.push(Balance::ClaimableOnChannelClose {
				...
				transaction_fee_satoshis: if us.holder_pays_commitment_tx_fee.unwrap_or(true) {
					chan_utils::commit_tx_fee_sat(
						us.funding.current_holder_commitment.tx.feerate_per_kw(),
						nondust_htlc_count,
...
impl<Signer: EcdsaChannelSigner> ChannelMonitorImpl<Signer> {
		...
		if let Some(txid) = self.funding.prev_counterparty_commitment_txid.take() {
			if self.funding.current_counterparty_commitment_txid.unwrap() != txid {
				...
				for &mut (_, ref mut source_opt) in
					self.funding.counterparty_claimable_outpoints.get_mut(&txid).unwrap()
				{
					*source_opt = None;
				}
			} else {
		...
		L::Target: Logger,
	{
		...
		self.initial_counterparty_commitment_info = Some((
			...
			commitment_tx.to_broadcaster_value_sat(),
			commitment_tx.to_countersignatory_value_sat(),
		));
		...
		self.provide_latest_counterparty_commitment_tx(
			...
			Vec::new(),
			commitment_tx.commitment_number(),
			commitment_tx.per_commitment_point(),
			logger,
		);
		...
		L::Target: Logger,
	{
		...
		self.funding.prev_counterparty_commitment_txid =
			self.funding.current_counterparty_commitment_txid.take();
	...
	fn provide_latest_holder_commitment_tx(
		...
		let dust_htlcs: Vec<_> = if htlc_outputs.iter().any(|(_, s, _)| s.is_some()) {
			...
			debug_assert_eq!(
				...
				holder_commitment_tx.trust().nondust_htlcs().len()
			);
			for (a, b) in htlc_outputs
				.iter()
				.filter(|(_, s, _)| s.is_some())
				.map(|(h, _, _)| h)
				.zip(holder_commitment_tx.trust().nondust_htlcs().iter())
			{
				debug_assert_eq!(a, b);
			}
			debug_assert_eq!(
			...
			let dust_htlcs = htlc_outputs
				...
				.filter_map(|(htlc, _, source)| {
					...
					if htlc.offered {
						nondust_htlc_sources
							.push(source.expect("Outbound HTLCs should have a source"));
					}
					None
				})
				.collect();
			...
			for htlc in holder_commitment_tx.trust().nondust_htlcs().iter() {
				if htlc.offered {
					let source =
						sources.next().expect("Non-dust HTLC sources didn't match commitment tx");
		...
		let mut holder_commitment =
			HolderCommitment { tx: holder_commitment_tx, nondust_htlc_sources, dust_htlcs };
		...
		L::Target: Logger,
	{
			...
			if txid == confirmed_spend_txid {
					...
					self.counterparty_commitment_txn_on_chain.get(&txid)
				{
					claim_htlcs!(
						*commitment_number,
						txid,
						self.funding.counterparty_claimable_outpoints.get(&txid)
					);
				} else {
					debug_assert!(false);
		...
		if self.broadcasted_holder_revokable_script.is_some() {
			let holder_commitment = if self.funding.current_holder_commitment.tx.trust().txid()
			...
			if let Some(holder_commitment) = holder_commitment {
				...
				let (claim_reqs, _) = self
					.get_broadcasted_holder_claims(&holder_commitment.tx, self.best_block.height);
				let conf_target = self.closure_conf_target();
				self.onchain_tx_handler.update_claims_view_from_requests(
					claim_reqs,
					self.best_block.height,
	...
	fn generate_claimable_outpoints_and_watch_outputs(
		...
		if !self.channel_type_features().supports_anchors_zero_fee_htlc_tx() {
			...
			if !new_outputs.is_empty() {
				watch_outputs
					.push((self.funding.current_holder_commitment.tx.trust().txid(), new_outputs));
			}
	...
	where
		...
		L::Target: Logger,
	{
		if self.latest_update_id == LEGACY_CLOSED_CHANNEL_UPDATE_ID
			&& updates.update_id == LEGACY_CLOSED_CHANNEL_UPDATE_ID
		{
			log_info!(
				logger,
				"Applying pre-0.1 post-force-closed update to monitor {} with {} change(s).",
			...
			log_info!(
				logger,
				"Applying pre-0.1 force close update to monitor {} with {} change(s).",
				log_funding_info!(self),
				updates.updates.len()
			);
		} else {
		...
		for update in updates.updates.iter() {
			match update {
				ChannelMonitorUpdateStep::LatestHolderCommitmentTXInfo {
					...
					self.provide_latest_holder_commitment_tx(
						commitment_tx.clone(),
						htlc_outputs.clone(),
						&claimed_htlcs,
				...
				ChannelMonitorUpdateStep::ChannelForceClosed { should_broadcast } => {
					...
					if *should_broadcast {
						...
						let detected_funding_spend = self.funding_spend_confirmed.is_some()
							|| self.onchain_events_awaiting_threshold_conf.iter().any(|event| {
								matches!(event.event, OnchainEvent::FundingSpendConfirmation { .. })
							});
		...
		for update in updates.updates.iter() {
			match update {
				...
				| ChannelMonitorUpdateStep::CommitmentSecret { .. } => is_pre_close_update = true,
			...
			if let Some(per_commitment_claimable_data) = per_commitment_option {
				for (htlc, _) in per_commitment_claimable_data {
					if let Some(transaction_output_index) = htlc.transaction_output_index {
						...
						let counterparty_spendable_height =
							if htlc.offered { htlc.cltv_expiry } else { height };
						let justice_package = PackageTemplate::build_package(
							commitment_txid,
	...
	fn get_broadcasted_holder_htlc_descriptors(
			...
			holder_tx.nondust_htlcs().iter().zip(holder_tx.counterparty_htlc_sigs.iter())
		{
			assert!(
				...
				"Expected transaction output index for non-dust HTLC"
			);
	...
	fn get_broadcasted_holder_claims(
		...
		let broadcasted_holder_revokable_script = Some((
			...
			keys.revocation_key.clone(),
		));

		let claim_requests = self
			.get_broadcasted_holder_htlc_descriptors(holder_tx)
			.into_iter()
			.map(|htlc_descriptor| {
				...
				let transaction_output_index = htlc_descriptor
					.htlc
					.transaction_output_index
	...
	fn get_broadcasted_holder_watch_outputs(
		...
		for htlc in holder_tx.nondust_htlcs() {
			if let Some(transaction_output_index) = htlc.transaction_output_index {
				watch_outputs.push((
					transaction_output_index,
					tx.built_transaction().transaction.output[transaction_output_index as usize]
						.clone(),
				));
			} else {
		...
		L::Target: Logger,
	{
		...
		let commitment_tx = {
			...
			self.funding
				.current_holder_commitment
				.tx
				.add_holder_sig(&self.funding.redeem_script, sig)
		};
		...
		L::Target: Logger,
	{
		...
		'tx_iter: for tx in &txn_matched {
			...
			if tx.input.len() == 1 {
				...
				let funding_outpoint = self.get_funding_txo();
				if prevout.txid == funding_outpoint.txid
					&& prevout.vout == funding_outpoint.index as u32
				{
		...
		L::Target: Logger,
	{
		...
		let (onchain_events_reaching_threshold_conf, onchain_events_awaiting_threshold_conf): (
			Vec<_>,
			Vec<_>,
		) = self
			.onchain_events_awaiting_threshold_conf
			.drain(..)
			.partition(|entry| entry.has_reached_confirmation_threshold(&self.best_block));
		...
		if self.no_further_updates_allowed() {
			...
			let current_counterparty_htlcs = if let Some(txid) =
			...
			let prev_counterparty_htlcs = if let Some(txid) =
				self.funding.prev_counterparty_commitment_txid
			{
				if let Some(htlc_outputs) = self.funding.counterparty_claimable_outpoints.get(&txid)
				{
			...
			for (htlc, source_opt) in htlcs {
				...
				let duplicate_event = self.pending_monitor_events.iter().any(|update| {
				...
				if !duplicate_event {
					log_error!(
						logger,
						"Failing back HTLC {} upstream to preserve the \
				...
				for ref htlc in $htlcs {
					...
					if (htlc_outbound && htlc.cltv_expiry + LATENCY_GRACE_PERIOD_BLOCKS <= height)
						|| (!htlc_outbound
							...
							&& self.payment_preimages.contains_key(&htlc.payment_hash))
					{
						log_info!(
							logger,
							"Force-closing channel due to {} HTLC timeout, HTLC expiry is {}",
							if htlc_outbound { "outbound" } else { "inbound " },
							htlc.cltv_expiry
						);
						return true;
					}
	...
	for (BlockHash, ChannelMonitor<SP::EcdsaSigner>)
{
	fn read<R: io::Read>(reader: &mut R, args: (&'a ES, &'b SP)) -> Result<Self, DecodeError> {
		...
		let prev_holder_signed_tx: Option<HolderSignedTx> = match <u8 as Readable>::read(reader)? {
			0 => None,
		...
		let onchain_tx_handler: OnchainTxHandler<SP::EcdsaSigner> = ReadableArgs::read(
			reader,
			(entropy_source, signer_provider, channel_value_satoshis, channel_keys_id),
		)?;
		...
		let channel_parameters =
			channel_parameters.unwrap_or_else(|| onchain_tx_handler.channel_parameters().clone());
			...
			&& counterparty_payment_script.is_p2wpkh()
		{
			...
			counterparty_payment_script =
				chan_utils::get_to_countersigner_keyed_anchor_redeemscript(&payment_point)
					.to_p2wsh();
		}
		...
		if counterparty_node_id.is_none() {
			panic!(
				"Found monitor for channel {} with no updates since v0.0.118.\
		...
		let prev_holder_commitment = if let Some(prev_holder_signed_tx) = prev_holder_signed_tx {
			...
			let holder_commitment = HolderCommitment::try_from((
				holder_commitment_tx.cloned().unwrap(),
				prev_holder_signed_tx,
			))
			.map_err(|_| DecodeError::InvalidValue)?;
...
mod tests {
	...
	fn test_prune_preimages() {
		...
		let monitor = ChannelMonitor::new(
			...
			true,
			0,
			HolderCommitmentTransaction::dummy(0, Vec::new()),
			best_block,
			dummy_key,
			channel_id,
		);
		...
		monitor.provide_latest_holder_commitment_tx(
			...
			nondust_htlcs
				.into_iter()
				.map(|htlc| (htlc.clone(), Some(dummy_sig), Some(dummy_source.clone())))
				.collect(),
		);
