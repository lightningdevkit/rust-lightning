...
fn write_legacy_holder_commitment_data<W: Writer>(
	writer: &mut W, commitment_tx: &HolderCommitmentTransaction,
	htlc_data: &HolderCommitmentHTLCData,
) -> Result<(), io::Error> {
...
pub(crate) struct ChannelMonitorImpl<Signer: EcdsaChannelSigner> {
	...
	current_holder_htlc_data: HolderCommitmentHTLCData,
	prev_holder_htlc_data: Option<HolderCommitmentHTLCData>,
}
...
macro_rules! holder_commitment_htlcs {
	...
	($self: expr, PREV_WITH_SOURCES) => {{
		if let Some(tx) = &$self.funding.prev_holder_commitment_tx {
			Some(holder_commitment_htlcs!(tx, $self.prev_holder_htlc_data.as_ref().unwrap()))
		} else {
			None
		}
	}};
	...
	fn check_spend_holder_transaction<L: Deref>(&mut self, tx: &Transaction, height: u32, block_hash: &BlockHash, logger: &L) -> Option<(Vec<PackageTemplate>, TransactionOutputs)> where L::Target: Logger {
		...
		if self.funding.current_holder_commitment_tx.trust().txid() == commitment_txid {
			...
			fail_unbroadcast_htlcs!(
				...
				holder_commitment_htlcs!(self, CURRENT_WITH_SOURCES), logger
			);
			...
			if holder_commitment_tx.trust().txid() == commitment_txid {
				...
				fail_unbroadcast_htlcs!(
					...
					holder_commitment_htlcs!(self, PREV_WITH_SOURCES).unwrap(), logger
				);
			}
		}
...
impl<'a, 'b, ES: EntropySource, SP: SignerProvider> ReadableArgs<(&'a ES, &'b SP)>
	...
	fn read<R: io::Read>(reader: &mut R, args: (&'a ES, &'b SP)) -> Result<Self, DecodeError> {
		...
		let (current_holder_commitment_tx, current_holder_htlc_data) = {
			...
			let holder_commitment_htlc_data = HolderCommitmentHTLCData::try_from((
				holder_commitment_tx, &current_holder_signed_tx,
			...
			#[cfg(debug_assertions)] {
				...
				if current_holder_signed_tx != <HolderSignedTx as Readable>::read(&mut cursor)? {
					return Err(DecodeError::InvalidValue);
				}
			}
		...
		let (prev_holder_commitment_tx, prev_holder_htlc_data) =
			if let Some(prev_holder_signed_tx) = prev_holder_signed_tx {
				...
				let holder_commitment_htlc_data = HolderCommitmentHTLCData::try_from((
					holder_commitment_tx.unwrap(), &prev_holder_signed_tx,
				)).map_err(|_| DecodeError::InvalidValue)?;
				...
				#[cfg(debug_assertions)] {
					...
					let mut cursor = crate::io::Cursor::new(stream.0);
					if prev_holder_signed_tx != <HolderSignedTx as Readable>::read(&mut cursor)? {
						return Err(DecodeError::InvalidValue);
					}
				}
