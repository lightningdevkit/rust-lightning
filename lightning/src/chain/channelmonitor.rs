...
pub(crate) fn write_util<Signer: EcdsaChannelSigner, W: Writer>(channel_monitor: &ChannelMonitorImpl<Signer>, is_stub: bool, writer: &mut W) -> Result<(), Error> {
	...
	channel_monitor.funding.script_pubkey.write(writer)?;
	...
	channel_monitor.funding.redeem_script.write(writer)?;
	...
	writer.write_all(&(channel_monitor.funding.counterparty_claimable_outpoints.len() as u64).to_be_bytes())?;
	for (ref txid, ref htlc_infos) in channel_monitor.funding.counterparty_claimable_outpoints.iter() {
		writer.write_all(&txid[..])?;
		writer.write_all(&(htlc_infos.len() as u64).to_be_bytes())?;
		for &(ref htlc_output, ref htlc_source) in htlc_infos.iter() {
			debug_assert!(htlc_source.is_none() || Some(**txid) == channel_monitor.funding.current_counterparty_commitment_txid
	...
	if let Some(holder_commitment_tx) = &channel_monitor.funding.prev_holder_commitment_tx {
		...
		write_legacy_holder_commitment_data(
			writer, holder_commitment_tx, &channel_monitor.prev_holder_htlc_data.as_ref().unwrap(),
		)?;
	} else {
		writer.write_all(&[0; 1])?;
	}
	...
	write_legacy_holder_commitment_data(
		writer, &channel_monitor.funding.current_holder_commitment_tx, &channel_monitor.current_holder_htlc_data,
	)?;
	...
	write_tlv_fields!(writer, {
		...
		(27, channel_monitor.first_confirmed_funding_txo, required),
