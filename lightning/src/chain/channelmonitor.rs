...
impl<Signer: EcdsaChannelSigner> ChannelMonitor<Signer> {
	...
	pub fn get_claimable_balances(&self) -> Vec<Balance> {
		...
		if let Some(txid) = confirmed_txid {
			...
			// confirmed at all.
			let amount_satoshis =
				us.current_holder_commitment_tx.to_self_value_sat + claimable_inbound_htlc_value_sat;
			if !us.is_closed_without_updates() || amount_satoshis != 0 {
...
impl<Signer: EcdsaChannelSigner> ChannelMonitorImpl<Signer> {
	...
	fn is_resolving_htlc_output<L: Deref>(
		...
		'outer_loop: for input in &tx.input {
			...
			macro_rules! scan_commitment {
				($htlcs: expr, $tx_info: expr, $holder_tx: expr, $spent_counterparty_revoked: expr) => {
			...
			if let Some(ref htlc_outputs) = self.counterparty_claimable_outpoints.get(&input.previous_output.txid) {
				...
				if self.current_counterparty_commitment_txid == Some(input.previous_output.txid) {
					spent_counterparty_revoked = false;
				}
				if self.prev_counterparty_commitment_txid == Some(input.previous_output.txid) {
					spent_counterparty_revoked = false;
				}
				scan_commitment!(htlc_outputs.iter().map(|&(ref a, ref b)| (a, b.as_ref().map(|boxed| &**boxed))),
					"counterparty commitment tx", false, spent_counterparty_revoked);
			}
