...
use crate::sign::{
	ecdsa::WriteableEcdsaChannelSigner, ChannelDerivationParameters,
...
where
	L::Target: Logger,
{
	pub(crate) fn from<S: WriteableEcdsaChannelSigner>(
		logger: &'a L, monitor: &ChannelMonitor<S>,
	) -> Self {
		Self::from_impl(logger, &*monitor.inner.lock().unwrap())
	}

	pub(crate) fn from_impl<S: WriteableEcdsaChannelSigner>(
		logger: &'a L, monitor_impl: &ChannelMonitorImpl<S>,
	) -> Self {
		let peer_id = monitor_impl.counterparty_node_id;
		let channel_id = Some(monitor_impl.channel_id());
		WithChannelMonitor { logger, peer_id, channel_id }
	}
}
...
impl<Signer: WriteableEcdsaChannelSigner> ChannelMonitorImpl<Signer> {
	...
	fn get_htlc_balance(
		&self, htlc: &HTLCOutputInCommitment, holder_commitment: bool,
			...
			if let Some(counterparty_tx_htlcs) = us.counterparty_claimable_outpoints.get(&txid) {
				...
				if txid == prev_commitment.txid {
					walk_htlcs!(
						true,
						false,
						prev_commitment.htlc_outputs.iter().map(|(a, _, _)| a)
					);
					if let Some(conf_thresh) = pending_commitment_tx_conf_thresh {
						res.push(Balance::ClaimableAwaitingConfirmations {
		...
		L::Target: Logger,
	{
		...
		macro_rules! claim_htlcs {
			($commitment_number: expr, $txid: expr) => {
				let (htlc_claim_reqs, _) =
					self.get_counterparty_output_claim_info($commitment_number, $txid, None);
				...
				ClaimEvent::BumpHTLC {
					...
					for htlc in htlcs {
						htlc_descriptors.push(HTLCDescriptor {
							...
							per_commitment_point: self
								.onchain_tx_handler
								.signer
								.get_per_commitment_point(
									htlc.per_commitment_number,
		...
		L::Target: Logger,
	{
		...
		if commitment_number >= self.get_min_seen_secret() {
			...
			let (htlc_claim_reqs, counterparty_output_info) = self
				.get_counterparty_output_claim_info(commitment_number, commitment_txid, Some(tx));
		...
		'tx_iter: for tx in &txn_matched {
			...
			if tx.input.len() == 1 {
					...
					&& prevout.vout == self.funding_info.0.index as u32
				{
					...
					{
						let (mut new_outpoints, new_outputs, counterparty_output_idx_sats) = self
						...
						if new_outpoints.is_empty() {
							if let Some((mut new_outpoints, new_outputs)) = self
								.check_spend_holder_transaction(&tx, height, &block_hash, &logger)
							{
		...
		L::Target: Logger,
	{
		...
		if should_broadcast {
			let (mut new_outpoints, mut new_outputs) =
				self.generate_claimable_outpoints_and_watch_outputs();
			claimable_outpoints.append(&mut new_outpoints);
...
impl<Signer: WriteableEcdsaChannelSigner, T: Deref, F: Deref, L: Deref> chain::Listen
...
impl<Signer: WriteableEcdsaChannelSigner, M, T: Deref, F: Deref, L: Deref> chain::Confirm
	for (M, T, F, L)
where
		...
		macro_rules! sign_input {
			($sighash_parts: expr, $idx: expr, $amount: expr, $weight: expr, $sum_actual_sigs: expr, $opt_anchors: expr) => {
				...
				let sighash = hash_to_message!(
					&$sighash_parts
						.segwit_signature_hash($idx, &redeem_script, $amount, EcdsaSighashType::All)
						.unwrap()[..]
				);
