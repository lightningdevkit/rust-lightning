...
impl From<&HolderCommitment> for HolderSignedTx {
	fn from(value: &HolderCommitment) -> Self {
		...
		let nondust_htlcs = value.tx.htlcs().iter().cloned();
		let counterparty_htlc_sigs = value.tx.counterparty_htlc_sigs.iter().cloned();
		let mut sources = value.nondust_htlc_sources.iter().cloned();
		...
		// Dust HTLCs go last.
		let dust_htlcs = value.dust_htlcs.iter().cloned();
