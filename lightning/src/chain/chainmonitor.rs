	    ...
	    P::Target: Persist<ChannelSigner>,
{
	...
	///
	/// If no matching `ChannelMonitor` is found for the provided `monitor_id`, it simply returns
	/// an `Err(())`, which should never happen if [`ClaimInfoRequest`] was generated. It should be handled
	/// by the caller as a failure/panic, most probably a wrong `monitor_id` was provided.
	///
	/// [`PersistClaimInfo`]: Event::PersistClaimInfo
	/// [`ClaimInfoRequest`]: Event::ClaimInfoRequest
	pub fn provide_claim_info(&self, monitor_id: OutPoint, claim_key: Txid, claim_info: ClaimInfo, claim_metadata: ClaimMetadata) -> Result<(), ()> {
		let monitors = self.monitors.read().unwrap();
	...
	/// 
	/// If no matching `ChannelMonitor` is found for the provided `monitor_id`, it simply returns
	/// an `Err(())`, which should never happen if [`PersistClaimInfo`] was generated. It should be handled
	/// by the caller as a failure/panic, most probably a wrong `monitor_id` was provided.
	///
	/// [`PersistClaimInfo`]: Event::PersistClaimInfo
	pub fn claim_info_persisted(&self, monitor_id: OutPoint, claim_key: Txid) -> Result<(), ()> {
		let monitors = self.monitors.read().unwrap();
		let monitor_data = if let Some(mon) = monitors.get(&monitor_id) { mon } else {
			return Err(());
		};
