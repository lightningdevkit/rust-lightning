...
impl<'a> DirectedChannelInfo<'a> {
	...
	#[inline]
	pub(super) fn source(&self) -> &'a NodeId {
	...
	#[inline]
	pub(super) fn target(&self) -> &'a NodeId {
		if self.from_node_one {
			&self.channel.node_two
		} else {
			&self.channel.node_one
...
impl Readable for NodeInfo {
	fn read<R: io::Read>(reader: &mut R) -> Result<Self, DecodeError> {
		...
		Ok(NodeInfo { announcement_info: announcement_info_wrap.map(|w| w.0), channels })
	}
}
	...
	L::Target: Logger,
{
	...
	fn add_channel_between_nodes(
		&self, short_channel_id: u64, channel_info: ChannelInfo, utxo_value: Option<u64>,
	) -> Result<(), LightningError> {
	...
	fn remove_channel_in_nodes(
		nodes: &mut IndexedMap<NodeId, NodeInfo>, chan: &ChannelInfo, short_channel_id: u64,
	) {
...
pub(crate) mod tests {
	...
	fn handling_node_announcements() {
		...
		match gossip_sync.handle_node_announcement(&NodeAnnouncement {
	...
	fn node_info_is_readable() {
		...
		let valid_node_info =
			NodeInfo { channels: Vec::new(), announcement_info: Some(valid_node_ann_info) };
