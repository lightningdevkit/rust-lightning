...
fn test_single_channel_multiple_mpp() {
	...
	// Test that claiming an MPP whose parts all arrived over the same final-hop channel batches
	// the per-part fulfills into a single `ChannelMonitorUpdate` and a single
	// `commitment_signed` (with one `update_fulfill_htlc` per part), rather than processing each
	// part serially with its own monitor update / commitment round trip.
	...
	//        8
	//
	// All six parts arrive at node 8 over the same node 7 -> node 8 channel.
	...
	// Claim the MPP payment. All six parts arrived over the same node 7 -> node 8 channel, so
	// LDK should queue all six fulfills into that channel's holding cell and release them
	// together as a single `ChannelMonitorUpdate` and a single `commitment_signed` carrying
	// all six `update_fulfill_htlc`s, instead of one round trip per part.
	nodes[8].node.claim_funds(payment_preimage);
	expect_payment_claimed!(nodes[8], payment_hash, 50_000_000);

	// Exactly one `ChannelMonitorUpdate` should have been added for all six MPP parts.
	check_added_monitors(&nodes[8], 1);

	// And there should be exactly one batched commitment update to node 7 carrying all six
	// fulfills.
	let updates = get_htlc_update_msgs(&nodes[8], &node_h_id);
	assert_eq!(updates.update_fulfill_htlcs.len(), 6);
	assert_eq!(updates.commitment_signed.len(), 1);
	assert!(updates.update_add_htlcs.is_empty());
	assert!(updates.update_fail_htlcs.is_empty());
	assert!(updates.update_fail_malformed_htlcs.is_empty());
