...
/// A validated, sorted set of custom TLVs for recipient onion fields.
#[derive(Clone)]
pub struct CustomTlvs(Vec<(u64, Vec<u8>)>);
...
impl CustomTlvs {
	...
	///
	/// This method will also error for types in the experimental range which
	/// have been standardized within the protocol, which only includes
	/// 5482373484 (keysend) for now.
...
impl RecipientOnionFields {
	...
	/// See [`Self::custom_tlvs`] for more info.
	#[rustfmt::skip]
	pub fn with_custom_tlvs(mut self, custom_tlvs: CustomTlvs) -> Self {
	...
	L::Target: Logger,
{
	...
	pub(super) fn pay_for_bolt11_invoice<R: Deref, ES: Deref, NS: Deref, IH, SP>(
		...
		optional_params: OptionalPaymentParams,
		router: &R,
		first_hops: Vec<ChannelDetails>, compute_inflight_htlcs: IH, entropy_source: &ES,
		node_signer: &NS, best_block_height: u32,
		pending_events: &Mutex<VecDeque<(events::Event, Option<EventCompletionAction>)>>, send_payment_along_path: SP,
...
mod tests {
	...
	use crate::ln::outbound_payment::{
		Bolt12PaymentError, CustomTlvs, OutboundPayments, PendingOutboundPayment, Retry,
	...
	fn test_recipient_onion_fields_with_custom_tlvs() {
		...
		let bad_type_range_tlvs = CustomTlvs::new(vec![
			(0, vec![42]),
			(1, vec![42; 32]),
		]);
		assert!(bad_type_range_tlvs.is_err());

		let keysend_tlv = CustomTlvs::new(vec![
			(5482373484, vec![42; 32]),
		]);
		assert!(keysend_tlv.is_err());

		let good_tlvs = CustomTlvs::new(vec![
			((1 << 16) + 1, vec![42]),
			((1 << 16) + 3, vec![42; 32]),
		]);
