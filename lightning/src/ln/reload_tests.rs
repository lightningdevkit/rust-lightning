...
fn test_mpp_claim_htlc_fulfills_unblocked_on_reload() {
	...
	let (update_id_a, _) = nodes[1].chain_monitor.latest_monitor_update_id.lock().unwrap().get(&chan_id_a).unwrap().clone();
	...
	// fulfill that was waiting behind it (unlike this test in 0.3, after we free the holding cell in
