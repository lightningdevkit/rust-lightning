...
fn test_retry_custom_tlvs() {
	...
	let onion = RecipientOnionFields::secret_only(payment_secret);
	let onion = onion.with_custom_tlvs(CustomTlvs::new(custom_tlvs.clone()).unwrap());
...
fn peel_payment_onion_custom_tlvs() {
	...
	let mut recipient_onion = RecipientOnionFields::spontaneous_empty()
		.with_custom_tlvs(CustomTlvs::new(vec![(414141, vec![42; 1200])]).unwrap());
...
fn max_out_mpp_path() {
	...
	let optional_params = crate::ln::channelmanager::OptionalPaymentParams::default();
