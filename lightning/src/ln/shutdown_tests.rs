...
fn updates_shutdown_wait() {
	...
	unwrap_send_err!(nodes[1], nodes[1].node.send_payment_with_route(route_2, payment_hash,
			RecipientOnionFields::secret_only(payment_secret), PaymentId(payment_hash.0)
		), true, APIError::ChannelUnavailable {..}, {});
...
fn do_htlc_fail_async_shutdown(blinded_recipient: bool) {
	...
	nodes[1].node.handle_commitment_signed_batch(nodes[0].node.get_our_node_id(), &updates.commitment_signed);
	check_added_monitors!(nodes[1], 1);
	nodes[1].node.handle_shutdown(nodes[0].node.get_our_node_id(), &node_0_shutdown);
...
fn do_outbound_update_no_early_closing_signed(use_htlc: bool) {
	...
	nodes[0].node.handle_commitment_signed_batch(nodes[1].node.get_our_node_id(), &bs_cs);
	check_added_monitors(&nodes[0], 1);
	assert_eq!(nodes[0].node.get_and_clear_pending_msg_events(), Vec::new());
