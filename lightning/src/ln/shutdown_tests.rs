...
use crate::ln::channelmanager::{
	self, ChannelDetails, ChannelShutdownState, PaymentId, PaymentSendFailure,
...
fn pre_funding_lock_shutdown_test() {
	...
	let node_0_closing_signed = get_event_msg!(
		nodes[0],
		MessageSendEvent::SendClosingSigned,
		nodes[1].node.get_our_node_id()
	);
	nodes[1].node.handle_closing_signed(&nodes[0].node.get_our_node_id(), &node_0_closing_signed);
	let node_1_closing_signed = get_event_msg!(
		nodes[1],
		MessageSendEvent::SendClosingSigned,
	...
	nodes[1].node.handle_closing_signed(
		&nodes[0].node.get_our_node_id(),
		&node_0_2nd_closing_signed.unwrap(),
	);
	let (_, node_1_none) =
...
fn expect_channel_shutdown_state_with_htlc() {
	...
	nodes[0].node.handle_update_fulfill_htlc(
		&nodes[1].node.get_our_node_id(),
		&updates_2.update_fulfill_htlcs[0],
	);
...
fn shutdown_on_unfunded_channel() {
	...
	nodes[0]
		.node
		.create_channel(nodes[1].node.get_our_node_id(), 1_000_000, 100_000, 0, None, None)
		.unwrap();
	let open_chan = get_event_msg!(
		nodes[0],
	...
	nodes[0].node.handle_shutdown(
		&nodes[1].node.get_our_node_id(),
		&msgs::Shutdown { channel_id: open_chan.temporary_channel_id, scriptpubkey: script },
	);
	check_closed_event!(
		nodes[0],
		1,
...
fn expect_channel_shutdown_state_with_force_closure() {
	...
	nodes[1]
		.node
		.force_close_broadcasting_latest_txn(&chan_1.2, &nodes[0].node.get_our_node_id())
		.unwrap();
...
fn do_htlc_fail_async_shutdown(blinded_recipient: bool) {
	...
	nodes[1]
		.node
		.handle_commitment_signed(&nodes[0].node.get_our_node_id(), &updates.commitment_signed);
	check_added_monitors!(nodes[1], 1);
	nodes[1].node.handle_shutdown(&nodes[0].node.get_our_node_id(), &node_0_shutdown);
	...
	nodes[0]
		.node
		.handle_update_fail_htlc(&nodes[1].node.get_our_node_id(), &updates_2.update_fail_htlcs[0]);
	commitment_signed_dance!(nodes[0], nodes[1], updates_2.commitment_signed, false, true);
	...
	check_closed_event!(
		...
		2,
		ClosureReason::CooperativeClosure,
		[nodes[0].node.get_our_node_id(), nodes[2].node.get_our_node_id()],
		100000
	);
	check_closed_event!(
		nodes[2],
...
fn do_test_shutdown_rebroadcast(recv_count: u8) {
	...
	let node_0_2nd_shutdown = if recv_count > 0 {
		let node_0_2nd_shutdown = get_event_msg!(
			nodes[0],
			MessageSendEvent::SendShutdown,
			nodes[1].node.get_our_node_id()
		);
		...
		let node_0_chan_update = get_event_msg!(
			nodes[0],
	...
	if recv_count > 0 {
		nodes[1]
			.node
			.handle_closing_signed(&nodes[0].node.get_our_node_id(), &node_0_closing_signed);
		let node_1_closing_signed = get_event_msg!(
			nodes[1],
			MessageSendEvent::SendClosingSigned,
			nodes[0].node.get_our_node_id()
		...
		nodes[0]
			.node
			.handle_closing_signed(&nodes[1].node.get_our_node_id(), &node_1_closing_signed);
		let (_, node_0_2nd_closing_signed) =
			get_closing_signed_broadcast!(nodes[0].node, nodes[1].node.get_our_node_id());
	...
	if recv_count == 0 {
		...
		nodes[0]
			.node
			.handle_channel_reestablish(&nodes[1].node.get_our_node_id(), &node_1_2nd_reestablish);
		...
		nodes[1]
			.node
			.handle_channel_reestablish(&nodes[0].node.get_our_node_id(), &node_0_2nd_reestablish);
		let node_1_3rd_shutdown = get_event_msg!(
			nodes[1],
			MessageSendEvent::SendShutdown,
			nodes[0].node.get_our_node_id()
		);
		...
		nodes[1]
			.node
			.handle_closing_signed(&nodes[0].node.get_our_node_id(), &node_0_2nd_closing_signed);
		let node_1_closing_signed = get_event_msg!(
			nodes[1],
			MessageSendEvent::SendClosingSigned,
			nodes[0].node.get_our_node_id()
...
fn test_upfront_shutdown_script() {
	...
	let node_0_orig_shutdown =
		get_event_msg!(nodes[0], MessageSendEvent::SendShutdown, nodes[2].node.get_our_node_id());
	...
	*nodes[0].override_init_features.borrow_mut() =
		Some(nodes[0].node.init_features().clear_upfront_shutdown_script());
...
fn test_unsupported_anysegwit_upfront_shutdown_script() {
	...
	*node_cfgs[0].override_init_features.borrow_mut() = Some(
		channelmanager::provided_init_features(&test_default_channel_config())
	...
	nodes[0]
		...
		.unwrap();
	let mut open_channel = get_event_msg!(
		nodes[0],
		MessageSendEvent::SendOpenChannel,
		nodes[1].node.get_our_node_id()
	);
	...
	*node_cfgs[1].override_init_features.borrow_mut() = Some(
		channelmanager::provided_init_features(&test_default_channel_config())
			.clear_shutdown_anysegwit(),
	);
	let node_chanmgrs = create_node_chanmgrs(2, &node_cfgs, &[None, None]);
	...
	nodes[1].node.handle_open_channel(&nodes[0].node.get_our_node_id(), &open_channel);
	let mut accept_channel = get_event_msg!(
		nodes[1],
...
fn test_invalid_upfront_shutdown_script() {
	...
	open_channel.shutdown_scriptpubkey =
...
fn test_unsupported_anysegwit_shutdown_script() {
	...
	*node_cfgs[0].override_init_features.borrow_mut() =
		Some(channelmanager::provided_init_features(&config).clear_shutdown_anysegwit());
	*node_cfgs[1].override_init_features.borrow_mut() =
		Some(channelmanager::provided_init_features(&config).clear_shutdown_anysegwit());
	let node_chanmgrs = create_node_chanmgrs(3, &node_cfgs, &user_cfgs);
	let nodes = create_network(3, &node_cfgs, &node_chanmgrs);
	...
	assert_eq!(
		&check_warn_msg!(nodes[0], nodes[1].node.get_our_node_id(), chan.2),
...
fn do_test_closing_signed_reinit_timeout(timeout_step: TimeoutStep) {
	...
	let mut node_0_closing_signed = get_event_msg!(
		nodes[0],
		MessageSendEvent::SendClosingSigned,
		nodes[1].node.get_our_node_id()
	);
	...
	if timeout_step != TimeoutStep::AfterShutdown {
		...
		if timeout_step == TimeoutStep::NoTimeout {
			nodes[1].node.handle_closing_signed(
				&nodes[0].node.get_our_node_id(),
				&node_0_2nd_closing_signed.1.unwrap(),
			);
			check_closed_event!(
				nodes[1],
				1,
	...
	if timeout_step != TimeoutStep::NoTimeout {
		assert!(
			(txn[0].output[0].script_pubkey.is_v0_p2wpkh()
...
fn do_simple_legacy_shutdown_test(high_initiator_fee: bool) {
	...
	nodes[1].node.handle_closing_signed(&nodes[0].node.get_our_node_id(), &node_0_closing_signed);
	let (_, mut node_1_closing_signed) =
		get_closing_signed_broadcast!(nodes[1].node, nodes[0].node.get_our_node_id());
	node_1_closing_signed.as_mut().unwrap().fee_range = None;

	nodes[0]
		.node
		.handle_closing_signed(&nodes[1].node.get_our_node_id(), &node_1_closing_signed.unwrap());
	let (_, node_0_none) =
		get_closing_signed_broadcast!(nodes[0].node, nodes[1].node.get_our_node_id());
...
fn simple_target_feerate_shutdown() {
	...
	nodes[1].node.handle_closing_signed(&nodes[0].node.get_our_node_id(), &node_0_closing_signed);
	let (_, node_1_closing_signed_opt) =
		get_closing_signed_broadcast!(nodes[1].node, nodes[0].node.get_our_node_id());
...
fn do_outbound_update_no_early_closing_signed(use_htlc: bool) {
	...
	if use_htlc {
		...
		nodes[1]
			.node
			.handle_update_fee(&nodes[0].node.get_our_node_id(), &updates.update_fee.unwrap());
	}
	nodes[1]
		.node
		.handle_commitment_signed(&nodes[0].node.get_our_node_id(), &updates.commitment_signed);
	...
	assert_eq!(nodes[0].node.get_and_clear_pending_msg_events(), Vec::new());
	let (outpoint, latest_update, _) = nodes[0]
		.chain_monitor
	...
	let bs_closing_signed = get_event_msg!(
		nodes[1],
		MessageSendEvent::SendClosingSigned,
		nodes[0].node.get_our_node_id()
	);
	nodes[0].node.handle_closing_signed(&nodes[1].node.get_our_node_id(), &bs_closing_signed);
	let (_, as_2nd_closing_signed) =
		get_closing_signed_broadcast!(nodes[0].node, nodes[1].node.get_our_node_id());
	nodes[1]
		.node
		.handle_closing_signed(&nodes[0].node.get_our_node_id(), &as_2nd_closing_signed.unwrap());
	let (_, node_1_none) =
		get_closing_signed_broadcast!(nodes[1].node, nodes[0].node.get_our_node_id());
	assert!(node_1_none.is_none());
...
fn batch_funding_failure() {
	...
	let close = [ExpectedCloseEvent::from_id_reason(
		ChannelId::v1_from_funding_txid(tx.txid().as_ref(), 0),
		true,
