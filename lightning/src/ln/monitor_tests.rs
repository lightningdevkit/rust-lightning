...
fn do_test_restored_packages_retry(check_old_monitor_retries_after_upgrade: bool) {
	...
	if check_old_monitor_retries_after_upgrade {
		...
		let serialized_channel = MonitorAndChannel{monitor: serialized_monitor, channel:None};
		reload_node!(nodes[0], &nodes[0].node.encode(), &[&serialized_channel], persister, new_chain_monitor, node_deserialized);
	}
...
fn do_test_anchors_aggregated_revoked_htlc_tx(p2a_anchor: bool) {
	...
	// present.
	let bob_serialized_monitor_a = get_monitor_and_channel(&nodes[1], chan_a.2);
	let bob_serialized_monitor_b = get_monitor_and_channel(&nodes[1], chan_b.2);
...
fn test_event_replay_causing_monitor_replay() {
	...
	let serialized_monitor = get_monitor_and_channel(&nodes[0], chan.2);
...
fn test_claim_event_never_handled() {
	...
	let chan_0_monitor_serialized = get_monitor_and_channel(&nodes[1], chan.2);
...
fn do_test_lost_preimage_monitor_events(on_counterparty_tx: bool, p2a_anchor: bool) {
	...
	let mon_a_ser = get_monitor_and_channel(&nodes[1], chan_a);
	let mon_b_ser = get_monitor_and_channel(&nodes[1], chan_b);
