...
use crate::chain::chaininterface::{compute_feerate_sat_per_1000_weight, LowerBoundedFeeEstimator};
...
fn do_chanmon_claim_value_coop_close(anchors: bool) {
	...
	let commitment_tx_fee =
		chan_feerate * channel::commitment_tx_base_weight(&channel_type_features) / 1000;
	...
	assert_eq!(
		vec![Balance::ClaimableOnChannelClose { amount_satoshis: 1_000 }],
		nodes[1]
			.chain_monitor
			.chain_monitor
			.get_monitor(funding_outpoint)
...
fn do_test_claim_value_force_close(anchors: bool, prev_commitment_tx: bool) {
	...
	if anchors {
		...
		nodes[1].wallet_source.add_utxo(
			bitcoin::OutPoint { txid: coinbase_tx.txid(), vout: 1 },
	...
	let commitment_tx_fee = chan_feerate as u64
		* (channel::commitment_tx_base_weight(&channel_type_features)
			+ 2 * channel::COMMITMENT_TX_WEIGHT_PER_HTLC)
		/ 1000;
	...
	assert_eq!(
		sorted_vec(vec![
			Balance::ClaimableOnChannelClose { amount_satoshis: 1_000 },
	...
	let commitment_tx_fee = chan_feerate as u64
		* (channel::commitment_tx_base_weight(&channel_type_features)
			+ if prev_commitment_tx { 1 } else { 2 } * channel::COMMITMENT_TX_WEIGHT_PER_HTLC)
		/ 1000;
	...
	assert_eq!(
		vec![Balance::ClaimableOnChannelClose { amount_satoshis: 1_000 + 3_000 + 4_000 }],
		nodes[1]
			.chain_monitor
			.chain_monitor
			.get_monitor(funding_outpoint)
	...
	assert_eq!(
		remote_txn[0].output[b_broadcast_txn[0].input[0].previous_output.vout as usize].value,
		3_000
	);
	assert_eq!(
		remote_txn[0].output[b_broadcast_txn[1].input[0].previous_output.vout as usize].value,
		4_000
	);
	...
	assert_eq!(
		remote_txn[0].output[a_broadcast_txn[0].input[0].previous_output.vout as usize].value,
		3_000
	);
	assert_eq!(
		remote_txn[0].output[a_broadcast_txn[1].input[0].previous_output.vout as usize].value,
		4_000
	);
...
fn test_no_preimage_inbound_htlc_balances() {
	...
	assert_eq!(
		sorted_vec(vec![
			Balance::ClaimableOnChannelClose { amount_satoshis: 500_000 - 20_000 },
...
fn do_test_revoked_counterparty_commitment_balances(anchors: bool, confirm_htlc_spend_first: bool) {
	...
	let mut claim_txn: Vec<_> = nodes[1]
		...
		.drain(..)
		.filter(|tx| {
			tx.input.iter().any(|inp| inp.previous_output.txid == as_revoked_txn[0].txid())
		})
		.collect();
	...
	let commitment_tx_fee = chan_feerate
		* (channel::commitment_tx_base_weight(&channel_type_features)
			+ 3 * channel::COMMITMENT_TX_WEIGHT_PER_HTLC)
		/ 1000;
	let anchor_outputs_value = if anchors { channel::ANCHOR_OUTPUT_VALUE_SATOSHI * 2 } else { 0 };
...
fn do_test_revoked_counterparty_htlc_tx_balances(anchors: bool) {
	...
	if anchors {
		...
		fuzzy_assert_eq(
			as_htlc_claim_tx[0].output[0].value,
...
fn do_test_revoked_counterparty_aggregated_claims(anchors: bool) {
	...
	assert_eq!(
		sorted_vec(vec![
			Balance::ClaimableOnChannelClose { amount_satoshis: 100_000 - 4_000 - 3_000 },
			Balance::MaybeTimeoutClaimableHTLC {
				amount_satoshis: 4_000,
				claimable_height: htlc_cltv_timeout,
...
fn test_yield_anchors_events() {
	...
	let (payment_preimage_2, payment_hash_2, ..) =
		route_payment(&nodes[1], &[&nodes[0]], 2_000_000);
...
fn test_anchors_aggregated_revoked_htlc_tx() {
	...
	*nodes[1].fee_estimator.sat_per_kw.lock().unwrap() *= 2;
	...
	for (idx, event) in events.into_iter().enumerate() {
		...
		nodes[1]
			.wallet_source
			.add_utxo(bitcoin::OutPoint { txid: coinbase_tx.txid(), vout: 0 }, utxo_value);
		match event {
	...
	// revoked outputs.
	{
		...
		let (revoked_htlc_claim_a, revoked_htlc_claim_b) =
			if txn[0].input[0].previous_output.txid == revoked_commitment_txs[0].txid() {
				(
	...
	let revoked_claim_transactions = {
		...
		let revoked_htlc_claims =
			txn.iter()
				.filter(|tx| {
					tx.input.len() == 2
						&& tx.output.len() == 1 && tx.input[0].previous_output.txid == htlc_tx.txid()
				})
				.collect::<Vec<_>>();
...
fn do_test_monitor_claims_with_random_signatures(
	...
	closing_node
		.node
		.force_close_broadcasting_latest_txn(&chan_id, &other_node.node.get_our_node_id())
		.unwrap();
