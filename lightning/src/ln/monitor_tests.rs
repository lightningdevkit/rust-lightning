...
fn do_test_balances_on_local_commitment_htlcs(keyed_anchors: bool, p2a_anchor: bool) {
	...
	if keyed_anchors || p2a_anchor {
		...
		assert_eq!(timeout_htlc_txn[0].input[1].witness.last().unwrap().len(), if p2a_anchor { chan_utils::OFFERED_HTLC_SCRIPT_WEIGHT } else { chan_utils::OFFERED_HTLC_SCRIPT_WEIGHT_ANCHORS });
	} else {
...
fn do_test_revoked_counterparty_htlc_tx_balances(keyed_anchors: bool, p2a_anchor: bool) {
	...
	let as_revoked_htlc_success_claim_fee = 253 * as_htlc_claim_tx[0].weight().to_wu() / 1000;
	if keyed_anchors || p2a_anchor {
