...
use crate::offers::async_receive_offer_cache::{
	...
	TEST_MIN_OFFER_PATHS_RELATIVE_EXPIRY_SECS,
};
...
fn pass_static_invoice_server_messages(
	...
	let num_cached_offers_before_flow = recipient.node.flow.test_get_async_receive_offers().len();
	...
	let (static_invoice, ack_path) = match events.pop().unwrap() {
		Event::PersistStaticInvoice {
			...
			recipient_id: ev_id,
			invoice_slot: _, // TODO: do something with this?
			invoice_id: _,   // TODO: do something with this?
		} => {
...
fn ignore_expired_static_invoice() {
	...
	let block = create_dummy_block(
		...
		server.node.duration_since_epoch().as_secs() as u32
			+ TEST_DEFAULT_ASYNC_RECEIVE_OFFER_EXPIRY.as_secs() as u32
			+ 1,
		Vec::new(),
	);
	connect_block(server, &block);
