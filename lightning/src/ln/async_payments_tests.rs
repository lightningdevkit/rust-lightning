...
// invoice server, returning the invoice at the end.
fn pass_static_invoice_server_messages(server: &Node, recipient: &Node) -> StaticInvoice {
	...
	assert!(offer_paths_req_msgs.len() <= crate::offers::flow::OFFERS_MESSAGE_REQUEST_LIMIT);
	...
	let (static_invoice, ack_path) = match events.pop().unwrap() {
		Event::PersistStaticInvoice { invoice, invoice_persisted_path, recipient_id_nonce: _ } => {
