...
impl Channel {
	...
	fn general_available(
		&mut self, incoming_amount_msat: u64, outgoing_channel_id: u64, node_salt: &[u8; 32],
	) -> Result<bool, ()> {
		self.general_bucket.can_add_htlc(outgoing_channel_id, incoming_amount_msat, node_salt)
	}
...
mod tests {
	...
	use crate::{
		...
		ln::{
			...
			resource_manager::{
				assign_slots_for_channel, bucket_allocations, slot_index, slot_used,
	...
	fn fill_general_bucket(rm: &DefaultResourceManager, incoming_scid: u64) {
		...
		incoming_channel.general_bucket.slots_occupied = [u64::MAX; 8];
	}
	...
	fn test_multi_channel_bucket_fallback_with_earned_reputation() {
		...
		let saturate_into_congestion = |outgoing_scid: u64, mut htlc_id: u64| -> u64 {
			loop {
				let outcome =
					add_htlc_between(INCOMING_SCID, outgoing_scid, false, htlc_id).unwrap();
				let bucket = get_htlc_bucket(&rm, INCOMING_SCID, htlc_id, outgoing_scid).unwrap();
				htlc_id += 1;
				if outcome == ForwardingOutcome::Forward(true) {
					assert_eq!(bucket, BucketAssigned::Congestion);
					break;
				}
				assert_eq!(bucket, BucketAssigned::General);
			}
			htlc_id
		};

		let mut htlc_id = 6_u64;
		htlc_id = saturate_into_congestion(OUTGOING_SCID, htlc_id);
		htlc_id = saturate_into_congestion(OUTGOING_SCID_2, htlc_id);
