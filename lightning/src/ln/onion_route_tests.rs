	...
	let update_1_0 = match test_case {
		0 | 100 => {
			...
			expect_htlc_handling_failed_destinations!(
				...
				&[expected_htlc_destination.clone().unwrap()]
			);
		...
		1 | 2 | 3 | 200 => {
			...
			if test_case == 2 || test_case == 200 {
				...
				expect_htlc_handling_failed_destinations!(
					...
					vec![expected_htlc_destination.clone().unwrap()]
				);
			}
...
fn test_onion_failure() {
	...
	run_onion_failure_test(
		...
		Some(short_channel_id),
		Some(HTLCDestination::InvalidOnion),
	);
	...
	run_onion_failure_test(
		...
		Some(NetworkUpdate::ChannelFailure { short_channel_id, is_permanent: true }),
		Some(short_channel_id),
		Some(HTLCDestination::UnknownNextHop { requested_forward_scid: short_channel_id }),
	);
