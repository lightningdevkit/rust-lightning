...
impl FundingTemplate {
	...
	/// - If the prior contribution's feerate can be adjusted to the minimum RBF feerate using
	///   the holder's balance, the adjusted contribution is returned directly. If adjustment
	///   fails or the balance is unavailable, coin selection is re-run using the prior
	///   contribution's parameters and the caller's `max_feerate`.
