...
impl FundingContribution {
	...
	/// feerate.
	pub(super) fn adjust_for_feerate(&mut self, target_feerate: FeeRate) -> Result<(), String> {
		let (new_estimated_fee, new_change) = self.compute_feerate_adjustment(target_feerate)?;
		match new_change {
			Some(value) => self.change_output.as_mut().unwrap().value = value,
			None => self.change_output = None,
		}
		self.estimated_fee = new_estimated_fee;
		self.feerate = target_feerate;
		Ok(())
	}
	...
	pub(super) fn net_value_at_feerate(
		&self, target_feerate: FeeRate,
...
mod tests {
	...
	#[test]
	fn test_adjust_for_feerate_higher_change_adjusted() {
		...
		let mut contribution = FundingContribution {
		...
		assert!(contribution.adjust_for_feerate(target_feerate).is_ok());
	...
	#[test]
	fn test_adjust_for_feerate_lower_change_increased() {
	...
	#[test]
	fn test_adjust_for_feerate_change_removed() {
	...
	#[test]
	fn test_adjust_for_feerate_too_high_rejected() {
		...
		let result = contribution.adjust_for_feerate(target_feerate);
		assert!(result.is_err());
		assert!(result.unwrap_err().contains("Feerate too high"));
	}

	#[test]
	fn test_adjust_for_feerate_splice_out_sufficient() {
	...
	#[test]
	fn test_adjust_for_feerate_splice_out_insufficient() {
	...
	#[test]
	fn test_net_value_at_feerate_splice_in() {
		// Splice-in: net_value_at_feerate returns the same value as net_value() since
		...
		// For splice-in, unpaid_fees is zero so net_value_at_feerate equals net_value.
		let net_at_feerate = contribution.net_value_at_feerate(target_feerate).unwrap();
		assert_eq!(net_at_feerate, contribution.net_value());
		assert_eq!(net_at_feerate, Amount::from_sat(50_000).to_signed().unwrap());
	}

	#[test]
	fn test_net_value_at_feerate_splice_out() {
		// Splice-out: net_value_at_feerate returns the adjusted value using the fair fee
	...
	#[test]
	fn test_net_value_at_feerate_does_not_mutate() {
		// Verify net_value_at_feerate does not modify the contribution.
		let original_feerate = FeeRate::from_sat_per_kwu(2000);
		let target_feerate = FeeRate::from_sat_per_kwu(5000);
		let input = funding_input_sats(100_000);
		let change = funding_output_sats(10_000);
		...
		let _ = contribution.net_value_at_feerate(target_feerate);
	...
	#[test]
	fn test_net_value_at_feerate_too_high() {
		// net_value_at_feerate returns Err when feerate can't be accommodated.
		let original_feerate = FeeRate::from_sat_per_kwu(2000);
		let target_feerate = FeeRate::from_sat_per_kwu(100_000);
		let input = funding_input_sats(100_000);
		let change = funding_output_sats(500);
		...
		let result = contribution.net_value_at_feerate(target_feerate);
		assert!(result.is_err());
		assert!(result.unwrap_err().contains("Feerate too high"));
	}
}
