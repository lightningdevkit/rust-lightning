...
fn test_fails_initiating_concurrent_splices() {
	...
	assert_eq!(
		nodes[0].node.splice_channel(&channel_id, &node_1_id, contribution.clone(), feerate),
		Err(APIError::APIMisuseError {
			err: format!("Channel {} cannot be spliced as it is already quiescent", channel_id),
		}),
	);
	...
	let (splice_tx, splice_locked) =
		sign_interactive_funding_tx(&nodes[0], &nodes[1], initial_commit_sig_for_acceptor, false);
	assert!(splice_locked.is_none());
