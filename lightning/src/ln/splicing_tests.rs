...
fn do_initiate_rbf_splice_in<'a, 'b, 'c, 'd>(
...
fn complete_rbf_handshake<'a, 'b, 'c, 'd>(
	initiator: &'a Node<'b, 'c, 'd>, acceptor: &'a Node<'b, 'c, 'd>,
) -> msgs::TxAckRbf {
	let node_id_initiator = initiator.node.get_our_node_id();
	let node_id_acceptor = acceptor.node.get_our_node_id();
...
fn lock_rbf_splice_after_blocks<'a, 'b, 'c, 'd>(
	...
	let splice_locked_b = get_event_msg!(node_a, MessageSendEvent::SendSpliceLocked, node_id_b);
	node_b.node.handle_splice_locked(node_id_a, &splice_locked_b);

	let mut msg_events = node_b.node.get_and_clear_pending_msg_events();

	// If the acceptor had a pending QuiescentAction, return the stfu message so that it can be
	// used for the next splice attempt.
	let node_b_stfu = msg_events
		.last()
		.filter(|event| matches!(event, MessageSendEvent::SendStfu { .. }))
		.is_some()
	...
	// Expect ChannelReady + DiscardFunding events on both nodes.
...
fn do_test_splice_rbf_tiebreak(
	...
	if acceptor_contributes {
		...
		// Clean up old watched outpoints from the chain source.
		let (orig_outpoint, orig_script) = original_funding_outpoint;
		let first_splice_funding_idx = first_splice_tx
			.output
			.iter()
			.position(|o| o.script_pubkey == new_funding_script)
			.unwrap();
		let first_splice_outpoint = OutPoint {
			txid: first_splice_tx.compute_txid(),
			index: first_splice_funding_idx as u16,
		};
		for node in &nodes {
			node.chain_source.remove_watched_txn_and_outputs(orig_outpoint, orig_script.clone());
			node.chain_source
