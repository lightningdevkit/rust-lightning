...
use crate::ln::{chan_utils, functional_test_utils::*};
...
fn complete_interactive_funding_negotiation<'a, 'b, 'c, 'd>(
...
) -> (msgs::CommitmentSigned, PublicKey) {
	...
	let mut initiator_sent_tx_complete = false;
	...
	loop {
		if !expected_initiator_inputs.is_empty() {
			...
			assert_eq!(msg_events.len(), if acceptor_sent_tx_complete { 2 } else { 1 });
			...
			if acceptor_sent_tx_complete {
				if let MessageSendEvent::UpdateHTLCs { mut updates, .. } = msg_events.remove(0) {
					return (updates.commitment_signed.remove(0), node_id_acceptor);
				}
				panic!();
			}
		...
		assert_eq!(msg_events.len(), if initiator_sent_tx_complete { 2 } else { 1 });
		if let MessageSendEvent::SendTxComplete { ref msg, .. } = msg_events.remove(0) {
		...
		if initiator_sent_tx_complete {
			if let MessageSendEvent::UpdateHTLCs { mut updates, .. } = msg_events.remove(0) {
				return (updates.commitment_signed.remove(0), node_id_initiator);
			}
			panic!();
		}
...
fn sign_interactive_funding_transaction<'a, 'b, 'c, 'd>(
	...
	initial_commitment_signed: msgs::CommitmentSigned, destination_node_id: PublicKey,
) {
	let node_id_initiator = initiator.node.get_our_node_id();
	let node_id_acceptor = acceptor.node.get_our_node_id();

	if destination_node_id == node_id_acceptor {
		assert!(initiator.node.get_and_clear_pending_msg_events().is_empty());
		acceptor.node.handle_commitment_signed(node_id_initiator, &initial_commitment_signed);
		...
		initiator.node.handle_commitment_signed(node_id_acceptor, &initial_commitment_signed);

		let mut msg_events = initiator.node.get_and_clear_pending_msg_events();
		assert_eq!(msg_events.len(), 1);
		if let MessageSendEvent::UpdateHTLCs { updates, .. } = msg_events.remove(0) {
			acceptor
				.node
				.handle_commitment_signed_batch_test(node_id_initiator, &updates.commitment_signed);
		} else {
			panic!("{msg_events:?}");
		}
		let event = get_event!(initiator, Event::FundingTransactionReadyForSigning);
		...
		if let MessageSendEvent::SendTxSignatures { ref msg, .. } = msg_events.remove(0) {
			acceptor.node.handle_tx_signatures(node_id_acceptor, msg);
		} else {
			panic!("{msg_events:?}");
		}

		let tx_signatures =
			get_event_msg!(acceptor, MessageSendEvent::SendTxSignatures, node_id_initiator);
		initiator.node.handle_tx_signatures(node_id_acceptor, &tx_signatures);
	}
...
fn splice_channel<'a, 'b, 'c, 'd>(
	...
	let (initial_commitment_signed, destination_node_id) = complete_interactive_funding_negotiation(
		initiator,
		acceptor,
		channel_id,
	...
	sign_interactive_funding_transaction(
		initiator,
		acceptor,
