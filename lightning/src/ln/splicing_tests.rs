...
#[test]
fn acceptor_with_local_contribution_can_cancel_splice_before_funding_transaction_signed() {
	let chanmon_cfgs = create_chanmon_cfgs(2);
	let node_cfgs = create_node_cfgs(2, &chanmon_cfgs);
	let node_chanmgrs = create_node_chanmgrs(2, &node_cfgs, &[None, None]);
	let nodes = create_network(2, &node_cfgs, &node_chanmgrs);
...
#[test]
fn cancel_splice_before_funding_transaction_signed() {
	let chanmon_cfgs = create_chanmon_cfgs(2);
	let node_cfgs = create_node_cfgs(2, &chanmon_cfgs);
	let node_chanmgrs = create_node_chanmgrs(2, &node_cfgs, &[None, None]);
	let nodes = create_network(2, &node_cfgs, &node_chanmgrs);
	...
	// Node 0 should have a signing event to handle, while node 1 immediately sends their initial
	// commitment_signed. Deliver it before canceling to ensure it gets discarded with the splice.
	...
	initiator.node.cancel_splice(&channel_id, &node_id_acceptor).unwrap();
	expect_splice_failed_events(initiator, &channel_id, funding_contribution);
...
#[test]
fn cannot_cancel_splice_after_funding_transaction_signed() {
	let chanmon_cfgs = create_chanmon_cfgs(2);
	let node_cfgs = create_node_cfgs(2, &chanmon_cfgs);
	let node_chanmgrs = create_node_chanmgrs(2, &node_cfgs, &[None, None]);
	let nodes = create_network(2, &node_cfgs, &node_chanmgrs);
	...
	let res = initiator.node.cancel_splice(&channel_id, &node_id_acceptor);
	match res {
