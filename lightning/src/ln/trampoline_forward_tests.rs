...
use crate::ln::channelmanager::{HTLCPreviousHopData, MppPart, MIN_CLTV_EXPIRY_DELTA};
...
fn do_test_trampoline_mpp_validation(test_case: Option<TrampolineMppValidationTestCase>) {
	...
	let expected = match test_case {
		...
		Some(TrampolineMppValidationTestCase::CltvDeltaBelowMinimum) => {
			// A node operator may configure a `cltv_expiry_delta` below
			// `MIN_CLTV_EXPIRY_DELTA` (the raw config field isn't floored on the way in),
			// but we must still require at least the minimum when forwarding. Offer a delta
			// that sits *between* the too-low configured value and the minimum.
			cltv_delta = (MIN_CLTV_EXPIRY_DELTA / 2) as u32;
			next_trampoline_cltv = update_add_cltv - (MIN_CLTV_EXPIRY_DELTA as u32 - 1);
			LocalHTLCFailureReason::TrampolineFeeOrExpiryInsufficient
		},
...
fn test_trampoline_mpp_validation() {
	...
	do_test_trampoline_mpp_validation(Some(TrampolineMppValidationTestCase::CltvDeltaBelowMinimum));
