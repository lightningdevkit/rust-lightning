...
pub struct ChannelDetails {
	...
	///
	/// When routing outbound payments, prefer [`outbound_scid_alias`] over this if it is set.
	/// The real SCID changes when a splice confirms, while the alias assigned at channel open
	/// remains stable. Use [`get_outbound_payment_scid`] to pick the appropriate value.
	///
	/// When a channel is spliced, this continues to refer to the original pre-splice channel
	/// state until the splice transaction reaches sufficient confirmations to be locked (and we
	/// exchange `splice_locked` messages with our peer).
	...
	/// usable in place of [`short_channel_id`] in outbound payment routes. When set, this should
	/// be preferred over [`short_channel_id`] for routing: the alias is assigned at channel open
	/// and remains stable across splices, whereas the real SCID changes each time a splice
	/// confirms. See [`get_outbound_payment_scid`].
	///
