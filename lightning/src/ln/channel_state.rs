...
pub struct ChannelDetails {
	...
	/// currently under negotiation with our counterparty, and any negotiated splice or RBF attempts
	/// waiting on sufficient on-chain confirmations. This will be `None` if no splice is pending.
	///
	/// This field will be `None` for objects serialized with LDK versions prior to 0.3.
...
pub struct SpliceDetails {
	...
	/// Our contribution to a splice we have committed to via
	...
	/// is reached (the counterparty may have its own splice in flight, as in
	/// [`negotiation`]), so this only reflects that a contribution is queued. Like the in-flight
	/// negotiation states, it does not survive a restart.
	///
	...
	/// The negotiated candidate that has confirmed on-chain, if any, along with its confirmation
	/// progress.
	///
