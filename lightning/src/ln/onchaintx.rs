...
impl<R: ::std::io::Read> ReadableArgs<R, Arc<Logger>> for OnchainTxHandler {
		      ...
		      F::Target: FeeEstimator
	{
		...
		for claim in new_claims {
			...
			if let Some((new_timer, new_feerate, tx)) = self.generate_claim_tx(height, &claim_material, &*fee_estimator) {
				...
				let txid = tx.txid();
				new_pending_claim_requests.insert(txid, claim_material);
		...
		for (k,v) in new_claimable_outpoints.drain() {
