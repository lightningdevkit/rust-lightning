...
fn do_test_htlc_interception_flags(flags_bitmask: u8, flag_bit: u8, modification: ForwardingMods) {
	...
	match modification {
		...
		ForwardingMods::CLTVBelowConfig => {
	...
	if interception_bit_match && modification == ForwardingMods::None {
		...
		match (modification, flag_bit) {
			...
			_ => {
				...
				let reason_from_mod = match modification {
					ForwardingMods::FeeTooLow => Some(LocalHTLCFailureReason::FeeInsufficient),
					ForwardingMods::CLTVBelowConfig => {
