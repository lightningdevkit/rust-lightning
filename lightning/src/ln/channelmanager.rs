	...
	L::Target: Logger,
{
	...
	pub fn fail_intercepted_htlc(&self, intercept_id: InterceptId) -> Result<(), APIError> {
			...
			payment.forward_info.routing
		{
			...
			let destination = HTLCHandlingFailureType::InvalidForward {
				requested_forward_scid: short_channel_id,
			};
										...
										&&logger)
								{
										...
										.and_then(Channel::as_funded_mut)
									{
										...
										failed_forwards.push((htlc_source, payment_hash,
											...
											HTLCHandlingFailureType::Forward { node_id: Some(chan.context.get_counterparty_node_id()), channel_id: forward_chan_id }
										));
									} else {
										forwarding_channel_not_found!(core::iter::once(forward_info).chain(draining_pending_forwards));
										break;
		...
		match message {
			...
			OffersMessage::Invoice(invoice) => {
				...
				if self.default_configuration.manually_handle_bolt12_invoices {
					...
					self.pending_outbound_payments
						.mark_invoice_received(&invoice, payment_id)
						.ok()?;
...
mod tests {
	...
	fn test_keysend_hash_mismatch() {
		...
		expect_htlc_handling_failed_destinations!(
			...
			&[HTLCHandlingFailureType::Receive { payment_hash: mismatch_payment_hash }]
		);
		check_added_monitors(&nodes[1], 1);
