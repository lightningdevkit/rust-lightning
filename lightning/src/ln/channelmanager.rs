...
use crate::ln::channel::{calculate_our_funding_satoshis, ChannelVariants, InboundV2Channel, MIN_CHAN_DUST_LIMIT_SATOSHIS, OutboundV2Channel, InteractivelyFunded as _, V2Channel};
#[cfg(splicing)]
			...
			ChannelPhase::RefundingV2((_, channels)) => {
				if let Some(funded_channel) = channels.get_funded_channel_mut() {
					...
					match channels.get_pending_mut() {
						Some(V2Channel::UnfundedOutboundV2(ch)) => {
							convert_chan_phase_err!($self, $err, ch, $channel_id, UNFUNDED_CHANNEL)
						},
						Some(V2Channel::UnfundedInboundV2(ch)) => {
							convert_chan_phase_err!($self, $err, ch, $channel_id, UNFUNDED_CHANNEL)
						},
	...
	L::Target: Logger,
{
	...
	pub fn create_dual_funded_channel(&self, their_network_key: PublicKey, funding_satoshis: u64,
		funding_inputs: Vec<(TxIn, Transaction)>, funding_conf_target: Option<ConfirmationTarget>, user_channel_id: u128,
		override_config: Option<UserConfig>) -> Result<ChannelId, APIError>
	{
	...
	fn create_channel_internal(&self, _is_v2: bool, their_network_key: PublicKey, funding_satoshis: u64,
		_funding_inputs: Vec<(TxIn,TransactionU16LenLimited)>, _funding_conf_target: Option<ConfirmationTarget>,
		push_msat: u64, user_channel_id: u128, temporary_channel_id: Option<ChannelId>, override_config: Option<UserConfig>,
	...
	pub fn rbf_on_pending_v2_channel_open(
		...
		let peer_state_mutex = per_peer_state.get(&counterparty_node_id)
			.ok_or_else(|| {
				debug_assert!(false);
				APIError::APIMisuseError{ err: format!("Can't find peer {}", counterparty_node_id)}
			})?;
			...
			hash_map::Entry::Occupied(mut chan_phase_entry) => {
				match chan_phase_entry.get_mut() {
					ChannelPhase::FundingV2(ref mut channels) => {
						...
						true, &funding_inputs, feerate_sat_per_1000_weight, MIN_CHAN_DUST_LIMIT_SATOSHIS
						)?;
						let outbound_scid_alias = self.create_and_insert_outbound_scid_alias(); // TODO Create new, should we reuse?

						let channel = match OutboundV2Channel::new_for_rbf(
							&channels.get_funded_channel().expect("FundingV2 must be funded").context,
							...
							Err(_e) => {
								self.outbound_scid_aliases.lock().unwrap().remove(&outbound_scid_alias);
								return Err(APIError::APIMisuseError{ err: format!("Error creating new RBF variant for pending v2 channel, id {}, node {}", channel_id, counterparty_node_id)});
							},
						};
						...
						let msg_rbf = msgs::TxInitRbf {
							...
							locktime,
							feerate_sat_per_1000_weight,
							funding_output_contribution: Some(funding_satoshis as i64),
						};
						let msg_send_event = events::MessageSendEvent::SendTxInitRbf {
							node_id: counterparty_node_id,
							msg: msg_rbf,
						};
						peer_state.pending_msg_events.push(msg_send_event);
					...
					_ => {
						return Err(APIError::APIMisuseError{ err: format!("Channel is not funded v2 channel, id {}, node {}", channel_id, counterparty_node_id)});
					},
				}
			},
	...
	/// Doc on message flow:
	/// splice_channel():
	///   --- splice ------------------>    signals intent for splice, with change amount
	///   <-------------- splice_ack ---    accepts proposed splice
	/// splice_funding_generated():
	///   --- splice_created ---------->    sends new funding transaction parameters.
	///                                     In future, this should be a set of tx_add_input, tx_add_output, etc.
	///   <------------- tx_complete ---
	///   --- splice_comm_sign -------->    send commitment signature
	///   <--------  splice_comm_ack ---    send commitment signature
	///   --- splice_signed ----------->    send signature on funding tx. In future this should be tx_signatures
	...
	/// Initiate an RBF on a splicing channel.
	///
	/// The channel must have a pending splice, and must have an already negotiated
	/// splicing funding transaction.
	///
	/// TODO(splicing): Check RBF-compliancy on feerate
	#[cfg(splicing)]
	pub fn rbf_on_pending_splice(
		...
		let per_peer_state = self.per_peer_state.read().unwrap();
		let peer_state_mutex = per_peer_state.get(counterparty_node_id)
			.ok_or_else(|| APIError::APIMisuseError{ err: format!("Not connected to node: {}", counterparty_node_id) })?;

		let mut peer_state_lock = peer_state_mutex.lock().unwrap();
		let peer_state = &mut *peer_state_lock;
		// Look for channel
		match peer_state.channel_by_id.entry(*channel_id) {
			hash_map::Entry::Vacant(_) => return Err(APIError::ChannelUnavailable{err: format!("Channel with id {} not found for the passed counterparty node_id {}", channel_id, counterparty_node_id) }),
			hash_map::Entry::Occupied(mut chan_phase_entry) => {
				if let ChannelPhase::RefundingV2((ref pre_chan, ref mut post_channels)) = chan_phase_entry.get_mut() {
					if let Some(_funded_post_channel) = post_channels.get_funded_channel() {
						let channel = V2Channel::new_spliced(
							true,
							&pre_chan.context,
							&self.signer_provider,
							&counterparty_node_id,
							our_funding_contribution_satoshis,
							0, // funding_contribution_satoshis,
							funding_inputs,
							LockTime::from_consensus(locktime),
							funding_feerate_perkw,
							&self.logger,
						).map_err(|e| APIError::APIMisuseError{ err: format!("Could not initiate RBF on pending splice, {:?}, id {}, node {}", e, channel_id, counterparty_node_id)})?;

						post_channels.set_new_pending(channel);

						let msg_rbf = msgs::TxInitRbf {
							channel_id: channel_id.clone(),
							locktime,
							feerate_sat_per_1000_weight: funding_feerate_perkw,
							funding_output_contribution: Some(our_funding_contribution_satoshis),
						};
						let msg_send_event = events::MessageSendEvent::SendTxInitRbf {
							node_id: *counterparty_node_id,
							msg: msg_rbf,
						...
						return Err(APIError::APIMisuseError{ err: format!("Channel has pending splice but it's not funded yet, RBF not possible, id {}, node {}", channel_id, counterparty_node_id)});
					}
				} else {
					return Err(APIError::ChannelUnavailable { err: format!("Channel with id {} is not funded, RBF not possible", channel_id) });
				}
		...
		let (update_opt, counterparty_node_id) = {
			...
			if let hash_map::Entry::Occupied(chan_phase_entry) = peer_state.channel_by_id.entry(channel_id.clone()) {
				...
				match chan_phase {
					...
					#[cfg(any(dual_funding, splicing))]
					ChannelPhase::FundingV2(_) => todo!("dual funding RBF"),
					#[cfg(splicing)]
		...
		let err: Result<(), _> = loop {
			...
			if let hash_map::Entry::Occupied(mut chan_phase_entry) = peer_state.channel_by_id.entry(id) {
				let chan = match chan_phase_entry.get_mut() {
					...
					// Both pre and post exist
					// TODO(splicing): Handle on both
					#[cfg(splicing)]
					ChannelPhase::RefundingV2((ref mut pre_chan, ref mut _post_chans)) => {
						pre_chan
					},
	...
	/* Note: contribute_funding_inputs() is no longer used
	...
	pub fn contribute_funding_inputs(&self, channel_id: &ChannelId, counterparty_node_id: &PublicKey,
		...
		let funding_inputs = funding_inputs.into_iter().map(|(txin, tx)| {
		...
		match peer_state.channel_by_id.entry(*channel_id) {
			...
			hash_map::Entry::Vacant(_) => return Err(APIError::ChannelUnavailable {err: format!(
				"Channel with id {} not found for the passed counterparty node_id {}",
				channel_id, counterparty_node_id) }),
		}
		Ok(())
	...
	*/
	...
	-> (Option<(u64, OutPoint, ChannelId, u128, Vec<(PendingHTLCInfo, u64)>)>, Option<(u64, Vec<msgs::UpdateAddHTLC>)>) {
		...
		if let Some(tx) = funding_broadcastable {
			log_info!(logger, "Broadcasting funding transaction with txid {}  len {}", tx.txid(), tx.encode().len());
			self.tx_broadcaster.broadcast_transactions(&[&tx]);
		}
	...
	//  Can also be called after a [`Event::OpenChannelV2Request`] when we are not contributing any funds.
	///
	...
	/// Accepts a request to open a channel after a [`Event::OpenChannelRequest`], treating it as
	/// confirmed immediately.
	// TODO(dual_funding): Make these part of doc comments when #[cfg(dual_funding)] is dropped.
	//  Can also be called after a [`Event::OpenChannelV2Request`] when we are not contributing any,
	// funds, treating it as confirmed immediately.
	///
	/// The `user_channel_id` parameter will be provided back in
	/// [`Event::ChannelClosed::user_channel_id`] to allow tracking of which events correspond
	/// with which `accept_inbound_channel`/`accept_inbound_channel_from_trusted_peer_0conf` call.
				...
				match unaccepted_channel.open_channel_msg {
					...
					OpenChannelMessage::V2(open_channel_msg) => {
						...
						match channel_res {
							...
							Err(_) => Err(MsgHandleErrInternal::from_chan_no_close(ChannelError::Close("V2 channel rejected due to sender error".into()), *temporary_channel_id)),
						}
					},
				}
			},
	...
	fn internal_open_channel(&self, counterparty_node_id: &PublicKey, msg: OpenChannelMessage) -> Result<(), MsgHandleErrInternal> {
		let (chain_hash, temporary_channel_id) = match msg.clone() {
			OpenChannelMessage::V1(msg) => (msg.common_fields.chain_hash, msg.common_fields.temporary_channel_id),
			#[cfg(any(dual_funding, splicing))]
			OpenChannelMessage::V2(msg) => (msg.common_fields.chain_hash, msg.common_fields.temporary_channel_id),
		};
		...
		match msg {
			...
			OpenChannelMessage::V2(ref msg) => {
				...
				channel.begin_interactive_funding_tx_construction(&self.signer_provider, &self.entropy_source, self.get_our_node_id())
	...
	fn internal_accept_channel_v2(&self, counterparty_node_id: &PublicKey, msg: &msgs::AcceptChannelV2) -> Result<(), MsgHandleErrInternal> {
				...
				Some(phase) => {
					match phase {
						ChannelPhase::UnfundedOutboundV2(mut chan) => {
							if let Err(err) = chan.accept_channel_v2(&msg, &self.default_configuration.channel_handshake_limits, &peer_state.latest_features) {
		...
		let tx_msg_opt = chan.begin_interactive_funding_tx_construction(&self.signer_provider,
			&self.entropy_source, self.get_our_node_id())
			.map_err(|_| MsgHandleErrInternal::from_chan_no_close(
				ChannelError::Close("V2 channel rejected due to sender error".into()), channel_id))?;
		if let Some(tx_msg) = tx_msg_opt {
			let msg_send_event = match tx_msg {
				InteractiveTxMessageSend::TxAddInput(msg) => events::MessageSendEvent::SendTxAddInput {
					node_id: *counterparty_node_id, msg },
		...
		match peer_state.channel_by_id.entry(msg.channel_id) {
			hash_map::Entry::Occupied(mut chan_phase_entry) => {
				...
				let msg_send_event = match channel_phase {
					...
					ChannelPhase::FundingV2(ref mut channels) => {
						match channels.tx_add_output(msg) {
							Ok(ia_res) => ia_res.into_msg_send_event(counterparty_node_id),
							Err(err) => {
								try_chan_phase_entry!(self, Err(ChannelError::Warn(
									format!("Error while tx_add_output: {:?}", err)
								)), chan_phase_entry)
							}
						}
					},
		...
		match peer_state.channel_by_id.entry(msg.channel_id) {
			hash_map::Entry::Occupied(mut chan_phase_entry) => {
				...
				let (msg_send_event_opt, signing_session_opt) = match channel_phase {
					...
					ChannelPhase::FundingV2(ref mut channels) => {
						match channels.tx_complete(msg) {
							Ok(ia_res) => ia_res.into_msg_send_event(counterparty_node_id),
							Err(err) => {
								try_chan_phase_entry!(self, Err(ChannelError::Warn(
									format!("Error while tx_complete: {:?}", err)
								)), chan_phase_entry)
							}
						}
					},
					...
					ChannelPhase::RefundingV2((_pre_chan, post_chans)) => {
						match post_chans.tx_complete(msg) {
							...
							Err(err) => {
								try_chan_phase_entry!(self, Err(ChannelError::Warn(
									format!("Error while tx_complete message: {:?}", err)
								)), chan_phase_entry)
							}
						}
					},
				...
				if let Some(signing_session) = signing_session_opt {
					...
					let (res, pre_splice_chan, opt_channels) = match channel_phase {
						ChannelPhase::UnfundedOutboundV2(chan) => {
							(chan.funding_tx_constructed(counterparty_node_id, signing_session, &self.logger).map_err(
								|(chan, err)| {
									(ChannelPhase::UnfundedOutboundV2(chan), err)
								}
							), None::<Channel<SP>>, None)
						},
						ChannelPhase::UnfundedInboundV2(chan) => {
							(chan.funding_tx_constructed(counterparty_node_id, signing_session, &self.logger).map_err(
								|(chan, err)| {
						...
						ChannelPhase::FundingV2(chans) => {
							match chans.funding_tx_constructed(counterparty_node_id, signing_session, &self.logger) {
								Ok((res, chans)) => (Ok(res), None, Some(chans)),
								Err((chans, err)) => (Err((ChannelPhase::FundingV2(chans), err)), None, None),
							}
						},
						#[cfg(splicing)]
						ChannelPhase::RefundingV2((pre_chan, post_chans)) => {
							match post_chans.funding_tx_constructed(counterparty_node_id, signing_session, &self.logger) {
								Ok((res, chans)) => (Ok(res), Some(pre_chan), Some(chans)),
								Err((chans, err)) => (Err((ChannelPhase::RefundingV2((pre_chan, chans)), err)), None, None),
							}
						},
					...
					match res {
						Ok((mut channel, commitment_signed, funding_ready_for_sig_event_opt)) => {
							...
							// Re-add phase
							let channels = if let Some(mut channels) = opt_channels {
								// we have channel variants
								// promote pending channel to funded
								channels.add_funded(channel);
								channels
							} else {
								// no channel variants, new channel
								ChannelVariants::new(channel)
							};
							if let Some(pre_chan) = pre_splice_chan {
								#[cfg(splicing)]
								peer_state.channel_by_id.insert(channel_id.clone(), ChannelPhase::RefundingV2((pre_chan, channels)));
							} else {
								peer_state.channel_by_id.insert(channel_id.clone(), ChannelPhase::FundingV2(channels));
							}
						},
				...
				match channel_phase {
					ChannelPhase::FundingV2(ref mut channels) => {
						let mut channel = match InboundV2Channel::new_for_rbf(
							...
							Ok(ch) => { ch },
							Err(err) => {
								return Err(MsgHandleErrInternal::send_err_msg_no_close(format!("Could not create inbound channel, {}", err), channel_id));
							}
						};

						channel.begin_interactive_funding_tx_construction(&self.signer_provider, &self.entropy_source, self.get_our_node_id())
							.map_err(|_| MsgHandleErrInternal::send_err_msg_no_close(
								"Failed to start RBF interactive transaction construction".to_owned(), channel_id))?;
					...
					#[cfg(splicing)]
					ChannelPhase::RefundingV2((ref pre_chan, ref mut post_channels)) => {
						let mut channel = V2Channel::new_spliced(
							false,
							&pre_chan.context,
							&self.signer_provider,
							&counterparty_node_id,
							0, // TODO
							msg.funding_output_contribution.unwrap_or_default(),
							Vec::new(),
						...
						let msg = if let V2Channel::UnfundedInboundV2(ref mut inchan) = channel {
							inchan.accept_inbound_dual_funded_rbf_channel()
						} else { panic!("Internal error: Channel must be inbound"); };
	...
	// __RBF__ tx_ack_rbf
	#[cfg(any(dual_funding, splicing))]
	fn internal_tx_ack_rbf(&self, counterparty_node_id: &PublicKey, msg: &msgs::TxAckRbf) -> Result<(), MsgHandleErrInternal> {
		let channel_id = msg.channel_id;
		// Note that the ChannelManager is NOT re-persisted on disk after this, so any changes are
		// likely to be lost on restart!
		let per_peer_state = self.per_peer_state.read().unwrap();
		...
		let mut entry = peer_state.channel_by_id.entry(channel_id);
		match entry {
			hash_map::Entry::Vacant(_) => {
				return Err(MsgHandleErrInternal::send_err_msg_no_close(format!("Can't find a channel with id {} with node {}", channel_id, counterparty_node_id), channel_id));
			},
			hash_map::Entry::Occupied(ref mut chan_phase_entry) => {
				let channel_phase = chan_phase_entry.get_mut();
				let channels = match channel_phase {
					ChannelPhase::FundingV2(ref mut channels) => { channels }
					#[cfg(splicing)]
					ChannelPhase::RefundingV2((ref _pre_chan, ref mut post_channels)) => { post_channels }
					_ => {
						return Err(MsgHandleErrInternal::send_err_msg_no_close(format!("Got an unexpected tx_ack_rbf message from peer with counterparty_node_id {}", counterparty_node_id), channel_id));
					}
				};
				if let Some(ref mut chan) = channels.get_pending_out_mut() {
					match chan.ack_rbf(msg) {
						Ok(_) => {
							let tx_msg_opt = chan.begin_interactive_funding_tx_construction(&self.signer_provider, &self.entropy_source, self.get_our_node_id())
								.map_err(|_| MsgHandleErrInternal::from_chan_no_close(
									ChannelError::Close("V2 channel rejected due to sender error".into()), channel_id))?;
							if let Some(tx_msg) = tx_msg_opt {
								let msg_send_event = match tx_msg {
									InteractiveTxMessageSend::TxAddInput(msg) => events::MessageSendEvent::SendTxAddInput { node_id: *counterparty_node_id, msg },
									InteractiveTxMessageSend::TxAddOutput(msg) => events::MessageSendEvent::SendTxAddOutput { node_id: *counterparty_node_id, msg },
									InteractiveTxMessageSend::TxComplete(msg) => events::MessageSendEvent::SendTxComplete { node_id: *counterparty_node_id, msg },
								};
								peer_state.pending_msg_events.push(msg_send_event);
							}
						},
						Err(_e) => return Err(MsgHandleErrInternal::send_err_msg_no_close(format!("Error in ack_rbf"), channel_id))
					};
				} else {
					return Err(MsgHandleErrInternal::send_err_msg_no_close(format!("Got an unexpected tx_ack_rbf message, no pending RBF. counterparty_node_id {}", counterparty_node_id), channel_id));
				}
			},
		};
	...
	fn internal_channel_ready(&self, counterparty_node_id: &PublicKey, msg: &msgs::ChannelReady) -> Result<(), MsgHandleErrInternal> {
		{
			...
			match peer_state.channel_by_id.entry(msg.channel_id) {
				hash_map::Entry::Occupied(mut chan_phase_entry) => {
					// Get the channel or channels (in case of RBF variants) to check
					let channels = match chan_phase_entry.get_mut() {
					...
					if channels.len() > 1 {
						log_trace!(self.logger, "Considering {} channel variants", channels.len());
					}
				...
				hash_map::Entry::Occupied(mut chan_phase_entry) => {
					match chan_phase_entry.get_mut() {
						...
						ChannelPhase::RefundingV2((_, post_chans)) => {
							// Handle ALL variants
							for chan in post_chans.all_funded_mut() {
								let logger = WithChannelContext::from(&self.logger, &chan.context);
	...
	fn internal_commitment_signed(&self, counterparty_node_id: &PublicKey, msg: &msgs::CommitmentSigned) -> Result<(), MsgHandleErrInternal> {
		...
		match peer_state.channel_by_id.entry(msg.channel_id) {
			hash_map::Entry::Occupied(mut chan_phase_entry) => {
				match chan_phase_entry.get_mut() {
					// First level branching: special case for RefundingV2
					// Both pre and post exist
					#[cfg(splicing)]
					ChannelPhase::RefundingV2((pre_chan, post_chans)) => {
						// Try handling on post
						match post_chans.commitment_signed_initial_v2(&msg, best_block, &self.signer_provider, &self.logger) {
							Ok(res) => {
								if let Some((post_chan, monitor)) = res {
									let post_funding_txo = post_chan.context.get_funding_txo().unwrap();
									if let Ok(persist_status) = self.chain_monitor.watch_channel(post_funding_txo, monitor) {
									...
									// Handle on pre
									let logger = WithContext::from(&self.logger, None, Some(msg.channel_id));
									log_trace!(logger, "Handling commitment_signed on pre-splice channel");

									let monitor_update_opt = try_chan_phase_entry!(self, pre_chan.commitment_signed(&msg, &&logger), chan_phase_entry);
									if let Some(monitor_update) = monitor_update_opt {
										let pre_funding_txo = pre_chan.context.get_funding_txo().unwrap();
										handle_new_monitor_update!(self, pre_funding_txo, monitor_update, peer_state_lock, peer_state, per_peer_state, pre_chan);
									}

									// TODO(splicing): Handle on post, as not initial

									Ok(())
								}
							}
							Err(err) => try_chan_phase_entry!(self, Err(err), chan_phase_entry),
						}
					},
					_ => {
			...
			match peer_state.channel_by_id.entry(msg.channel_id) {
				hash_map::Entry::Occupied(mut chan_phase_entry) => {
					let chan = match chan_phase_entry.get_mut() {
						ChannelPhase::Funded(chan) => { chan }
						// Both post and pre exist
						// TODO(splicing) handle on both pre and post
						#[cfg(splicing)]
						ChannelPhase::RefundingV2((pre_chan, _post_chans)) => {
							pre_chan
						}
		...
		match peer_state.channel_by_id.entry(post_chan_id) {
			...
			hash_map::Entry::Occupied(mut chan_entry) => {
				if let ChannelPhase::RefundingV2((_pre_chan, post_chans)) = chan_entry.get_mut() {
					...
					Err(MsgHandleErrInternal::send_err_msg_no_close("Internal consistency error: splice_init while not renegotiating".to_string(), post_chan_id))
				}
			}
		}
	}
		...
		self.do_chain_event(None, |channel| {
			if let Some(funding_txo) = channel.context.get_funding_txo() {
				if funding_txo.txid == *txid {
					channel.funding_transaction_unconfirmed(&&WithChannelContext::from(&self.logger, &channel.context)).map(|()| (None, None, Vec::new(), None))
				} else { Ok((None, None, Vec::new(), None)) }
			} else { Ok((None, None, Vec::new(), None)) }
		});
	}
}
	...
	L::Target: Logger,
{
	...
	/// the function.
	fn do_chain_event<FN: Fn(&mut Channel<SP>) -> Result<(Option<msgs::ChannelReady>, Option<msgs::SpliceLocked>, Vec<(HTLCSource, PaymentHash)>, Option<msgs::AnnouncementSignatures>), ClosureReason>>
		...
		let mut timed_out_htlcs = Vec::new();
		{
			...
			for (_cp_id, peer_state_mutex) in per_peer_state.iter() {
				...
				peer_state.channel_by_id.retain(|_, phase| {
					// Get the channel or channels (in case of RBF variants) to check
					let channels = match phase {
						// Retain unfunded channels.
						ChannelPhase::UnfundedOutboundV1(_) | ChannelPhase::UnfundedInboundV1(_) => { return true },
						// TODO(dual_funding): Combine this match arm with above.
						#[cfg(any(dual_funding, splicing))]
	...
	/// Handle freshly confirmed channel, and if needed, transition them to Funded state
	pub fn transition_confirmed_channels(&self) {
		// Move any Ready FundingV2 channel to Funded (it it has locked just now)
		let per_peer_state = self.per_peer_state.read().unwrap();
		// let mut just_locked_channel_id = None;
		let mut just_locked_channels = Vec::new();
		...
		if msg.channel_id.is_zero() {
		...
		} else {
			{
				...
				match peer_state.channel_by_id.get_mut(&msg.channel_id) {
					...
					#[cfg(any(dual_funding, splicing))]
					Some(ChannelPhase::FundingV2(_)) => todo!("dual funding RBF"),
					#[cfg(splicing)]
...
mod tests {
	...
	fn test_v2_channel_establishment_only_initiator_contributes() {
		...
		nodes[0].node.create_dual_funded_channel(
			nodes[1].node.get_our_node_id(), funding_satoshis, funding_inputs,
		...
		// Since `manually_accept_inbound_channels` is false by default, nodes[1]'s node will accept the dual-
		// funded channel immediately without allowing nodes[1] to contribute any inputs. 
		...
		/* Note: FundingInputsContributionReady event no longer used
		// nodes[0] should get an event notifying her that channel establishment is awaiting funding inputs
		// and that she should provide them.
		if let Event::FundingInputsContributionReady {
			channel_id,
			counterparty_node_id,
			holder_funding_satoshis,
			counterparty_funding_satoshis,
			..
		} = get_event!(nodes[0], Event::FundingInputsContributionReady) {
			assert_eq!(holder_funding_satoshis, funding_satoshis);
		...
		*/
		...
		assert_eq!(input_value, 100_000);

		// We have two outputs being added: the P2WSH funding output, and the P2PKH change output
