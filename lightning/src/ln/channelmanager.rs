				...
				let tx_msg_opt = match phase.get_mut() {
					ChannelPhase::UnfundedOutboundV2(chan) => {
						chan.begin_interactive_funding_tx_construction(&self.signer_provider,
							&self.entropy_source, funding_inputs)?
					},
	...
	fn internal_open_channel(&self, counterparty_node_id: &PublicKey, msg: OpenChannelMessage) -> Result<(), MsgHandleErrInternal> {
		...
		match msg {
			...
			OpenChannelMessage::V2(ref msg) => {
				...
				channel.begin_interactive_funding_tx_construction(&self.signer_provider, &self.entropy_source, Vec::new())
							...
							Ok((tx_msg_opt, tx_opt)) => {
								...
								if let Some(tx) = tx_opt {
									...
									let res = match channel_phase {
										ChannelPhase::UnfundedOutboundV2(chan) => {
											chan.funding_tx_constructed(tx, best_block, &self.signer_provider, &self.logger).map_err(
												|(chan, err)| {
			...
			hash_map::Entry::Occupied(mut chan_phase_entry) => {
				match chan_phase_entry.get_mut() {
					ChannelPhase::Funded(chan) => {
						...
						let mut has_dual_funding_channel_context = false;
						#[cfg(dual_funding)]
						{
							has_dual_funding_channel_context = chan.dual_funding_channel_context.is_some();
						}

						if has_dual_funding_channel_context {
							#[cfg(dual_funding)]
							{
