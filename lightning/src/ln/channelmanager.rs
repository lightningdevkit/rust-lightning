...
fn check_mpp_timeout(sender_intended_total: u64, htlcs: &mut Vec<ClaimableHTLC>) -> bool {
	if htlcs.is_empty() {
		debug_assert!(false, "check_mpp_timeout called with no HTLCs");
		return false;
	}
	if htlcs[0].total_msat <= sender_intended_total {
		return false;
	}
	...
	> ChannelManager<M, T, ES, NS, SP, F, R, MR, L>
{
	...
	// Checks whether an incoming htlc can be added to our [`claimable_payments`], and handles
	// MPP accumulation. On successful add, returns Ok() with a boolean indicating whether all
	// MPP parts have arrrived. Callers *MUST NOT* fail htlcs if Ok(..) is returned.
	fn check_claimable_incoming_htlc(
		...
		if total_value >= msgs::MAX_VALUE_MSAT {
			...
			let counterparty_skimmed_fee_msat = claimable_payment
				.htlcs
				.iter()
				.map(|htlc| htlc.counterparty_skimmed_fee_msat.unwrap_or(0))
				.sum();
			debug_assert!(total_value.saturating_sub(amount_msat) <= counterparty_skimmed_fee_msat);
	...
	// Handles the addition of a HTLC associated with a payment we're receiving. Err(bool) indicates
			...
			.map_err(|_| committed_to_claimable)?
		{
			new_events.push_back((
				events::Event::PaymentClaimable {
					...
					payment_hash,
					purpose,
					amount_msat: claimable_payment.htlcs.iter().map(|htlc| htlc.value).sum(),
					...
					claim_deadline: Some(
						claimable_payment.htlcs.iter().map(|h| h.cltv_expiry).min().unwrap() // TODO: don't unwrap
		...
		let result = self.pending_outbound_payments.send_payment_for_trampoline_forward(
			...
			TrampolineForwardInfo {
				...
				previous_hop_data: claimable_payment
					...
					.collect(),
				incoming_trampoline_shared_secret,
				forwading_fee_msat: our_forwarding_fee_msat,
			},
			retry_strategy,
				...
				HTLCForwardInfo::AddHTLC(payment) => {
					...
					match claimable_htlc.onion_payload {
						OnionPayload::Invoice { .. } => {
							...
							if let Err(committed_to_claimable) = self.handle_claimable_htlc(
								purpose,
								claimable_htlc,
								onion_fields,
								payment_hash,
...
impl Writeable for ClaimableHTLC {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
		let (payment_data, keysend_preimage, trampoline_next_hop, trampoline_next_node) =
			match &self.onion_payload {
				OnionPayload::Invoice { _legacy_hop_data } => {
					(_legacy_hop_data.as_ref(), None, None, None)
				},
...
impl Readable for ClaimableHTLC {
	...
	fn read<R: Read>(reader: &mut R) -> Result<Self, DecodeError> {
		...
		let onion_payload = match (keysend_preimage, trampoline_next_hop) {
			(Some(p), None) => {
			...
			(None, Some(trampoline_next_hop)) => {
				// TODO: check which values we'll have for trampoline!
				if total_msat.is_none() {
					match payment_data {
						Some(data) => total_msat = Some(data.total_msat),
						None => return Err(DecodeError::InvalidValue),
					}
				}

				OnionPayload::Trampoline {
				next_hop_info: trampoline_next_hop,
				next_trampoline: trampoline_next_node.ok_or(DecodeError::InvalidValue)?,
			}
		},
...
fn prev_hop_needs_claim_replay<Signer: EcdsaChannelSigner, L: Logger>(
	...
	inbound_edge_monitor: Option<&ChannelMonitor<Signer>>,
	short_to_chan_info: &HashMap<u64, (PublicKey, ChannelId)>, logger: &L,
) -> Option<bool> {
	// Note that for channels which have gone to chain, `get_all_current_outbound_htlcs` is never
	// pruned and always returns a constant set until the monitor is removed/archived. Thus, we
	// want to skip replaying claims that have definitely been resolved on-chain.

	// If the inbound monitor is not present, we assume it was fully resolved and properly archived,
	// implying this payment had plenty of time to get claimed and we can safely skip any further
	// attempts to claim it (they wouldn't succeed anyway as we don't have a monitor against which
	// to do so).
	let inbound_edge_monitor = inbound_edge_monitor?;

	// Second, if the inbound edge of the payment's monitor has been fully claimed we've had at
	// least `ANTI_REORG_DELAY` blocks to  get any PaymentForwarded event(s) to the user and assume
	// that there's no need to try to replay the claim just for that.
	...
	if prev_hop.counterparty_node_id.is_none() {
		...
		let balance_could_incl_htlc = |bal| match bal {
			&Balance::ClaimableOnChannelClose { .. } => {
				// The channel is still open, assume we can still claim against it
				true
			},
		...
		// At this point we're confident we need the claim, but the inbound edge channel is still
		// live. As long as this remains the case, we can conceivably proceed, but we run some risk
		// of panicking at runtime. The user ideally should have read the release notes and we
		// wouldn't be here, but we go ahead and let things run in the hope that it'll all just
		// work out.
		log_error!(logger,
					...
					.filter_map(|(htlc_source, (htlc, preimage_opt))| {
						...
						for prev_hop in prev_htlcs {
							let inbound_edge_monitor =
								args.channel_monitors.get(&prev_hop.channel_id).copied();
							let logger = WithChannelMonitor::from(
								&args.logger,
								monitor,
								Some(payment_preimage.into()),
