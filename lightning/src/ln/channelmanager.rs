		...
		self.do_chain_event(None, |channel| {
			if let Some(funding_txo) = channel.get_funding_txo() {
				if funding_txo.txid == *txid {
					channel.funding_transaction_unconfirmed(&self.logger).map(|()| ChainActionUpdates::no_updates())
				} else { Ok(ChainActionUpdates::no_updates()) }
			} else { Ok(ChainActionUpdates::no_updates()) }
		});
	}
}
	...
	L::Target: Logger,
{
	...
	/// the function.
	fn do_chain_event<FN: Fn(&mut Channel<<SP::Target as SignerProvider>::Signer>) -> Result<ChainActionUpdates, ClosureReason>>
		...
		let mut timed_out_htlcs = Vec::new();
		{
			...
			for (_cp_id, peer_state_mutex) in per_peer_state.iter() {
				...
				peer_state.channel_by_id.retain(|_, channel| {
					let res = f(channel);
					if let Ok(mut chain_action_updates) = res {
						for (source, payment_hash) in chain_action_updates.timed_out_htlcs.drain(..) {
							let (failure_code, data) = self.get_htlc_inbound_temp_fail_err_and_data(0x1000|14 /* expiry_too_soon */, &channel);
							timed_out_htlcs.push((source, payment_hash, HTLCFailReason::reason(failure_code, data),
								HTLCDestination::NextHopChannel { node_id: Some(channel.get_counterparty_node_id()), channel_id: channel.channel_id() }));
						}
						if let Some(channel_ready) = chain_action_updates.channel_ready_msg {
							send_channel_ready!(self, pending_msg_events, channel, channel_ready);
