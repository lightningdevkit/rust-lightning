	...
	> ChannelManager<M, T, ES, NS, SP, F, R, MR, L>
{
	...
	fn check_claimable_incoming_htlc(
		...
		if total_intended_recvd_value >= msgs::MAX_VALUE_MSAT {
			return Err(());
		} else if total_intended_recvd_value - claimable_htlc.sender_intended_value
			>= total_mpp_value
		{
			log_trace!(
				self.logger,
			...
			HTLCSource::TrampolineForward { previous_hop_data, outbound_payment, .. } => {
				let trampoline_error = match outbound_payment {
					Some(_) => self
						...
						.map(|decoded| {
							if let Some(onion_error_code) = decoded.onion_error_code {
								log_trace!(
									WithContext::from(&self.logger, None, None, Some(*payment_hash)),
									"Received trampoline error intended for us: {:?}, returning TemporaryTrampolineFailure to original sender",
									onion_error_code,
								);
								HTLCFailReason::reason(
								...
								log_trace!(
									WithContext::from(&self.logger, None, None, Some(*payment_hash)),
									"Received trampoline error intended for original sender, returning as-is",
								);
								HTLCFailReason::from_onion_error_packet(peeled_packet)
							} else {
								log_trace!(
									WithContext::from(&self.logger, None, None, Some(*payment_hash)),
									"Received a malformed trampoline error, returning TemporaryTrampolineFailure to original sender",
								);
	...
	pub(super) fn from_channel_manager_data(
		...
		if reconstruct_manager_from_monitors {
			...
			for (src, payment_hash, _, _, _, _) in failed_htlcs.iter() {
				if let HTLCSource::PreviousHopData(_) | HTLCSource::TrampolineForward { .. } = src {
					dedup_decode_update_add_htlcs(
						&mut decode_update_add_htlcs,
			...
			for claimable_htlcs in claimable_payments.values().map(|pmt| &pmt.htlcs) {
				dedup_decode_update_add_htlcs(
					&mut decode_update_add_htlcs,
					claimable_htlcs.iter().map(|h| &h.prev_hop),
