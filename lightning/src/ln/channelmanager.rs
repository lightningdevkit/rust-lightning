			...
			HTLCSource::TrampolineForward { previous_hop_data, outbound_payment, .. } => {
				let trampoline_error = match outbound_payment {
					Some(_) => self
						...
						.map(|decoded| {
							if let Some(peeled_packet) = decoded.trampoline_peeled_packet {
								log_trace!(
									WithContext::from(&self.logger, None, None, Some(*payment_hash)),
								...
								// Either the failure decoded with our session keys (it was for
								// us as forwarder) or we couldn't peel any outer layers (e.g.,
								// packet too short). Either way, replace with our own error.
								log_trace!(
									WithContext::from(&self.logger, None, None, Some(*payment_hash)),
									"Received trampoline error for us as forwarder, returning TemporaryTrampolineFailure to original sender",
								);
								HTLCFailReason::reason(
