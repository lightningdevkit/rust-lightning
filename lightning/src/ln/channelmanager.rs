	...
	($self: ident, $err: expr, $channel: expr, $channel_id: expr) => {
		match $err {
			...
			ChannelError::Close(msg) => {
				log_error!($self.logger, "Closing channel {} due to close-required error: {}", log_bytes!($channel_id.bytes()[..][..]), msg);
	...
	($self: ident, $err: expr, $channel_context: expr, $channel_id: expr, UNFUNDED) => {
		match $err {
			...
			ChannelError::Warn(msg) | ChannelError::Ignore(msg) | ChannelError::Close(msg) => {
				log_error!($self.logger, "Closing unfunded channel {} due to an error: {}", log_bytes!($channel_id.bytes()[..][..]), msg);
		...
		match $update_res {
			ChannelMonitorUpdateStatus::InProgress => {
				log_debug!($self.logger, "ChannelMonitor update for {} in flight, holding messages until the update completes.",
					log_bytes!($chan.context.channel_id().bytes()[..][..]));
				Ok(false)
			},
	...
	L::Target: Logger,
{
	...
	-> Result<PublicKey, APIError> {
		...
		let (update_opt, counterparty_node_id) = {
			...
			if let hash_map::Entry::Occupied(chan) = peer_state.channel_by_id.entry(channel_id.clone()) {
				log_error!(self.logger, "Force-closing channel {}", log_bytes!(channel_id.bytes()[..][..]));
				...
				return Err(APIError::ChannelUnavailable{ err: format!("Channel with id {} not found for the passed counterparty node_id {}", log_bytes!((*channel_id).bytes()[..]), peer_node_id) });
			}
		};
	...
	fn get_channel_update_for_broadcast(&self, chan: &Channel<<SP::Target as SignerProvider>::Signer>) -> Result<msgs::ChannelUpdate, LightningError> {
		...
		log_trace!(self.logger, "Attempting to generate broadcast channel update for channel {}", log_bytes!(chan.context.channel_id().bytes()[..]));
	...
	fn get_channel_update_for_unicast(&self, chan: &Channel<<SP::Target as SignerProvider>::Signer>) -> Result<msgs::ChannelUpdate, LightningError> {
		log_trace!(self.logger, "Attempting to generate channel update for channel {}", log_bytes!(chan.context.channel_id().bytes()[..]));
	...
	fn get_channel_update_for_onion(&self, short_channel_id: u64, chan: &Channel<<SP::Target as SignerProvider>::Signer>) -> Result<msgs::ChannelUpdate, LightningError> {
		log_trace!(self.logger, "Generating channel update for channel {}", log_bytes!(chan.context.channel_id().bytes()[..]));
			...
			None => {
				return Err(APIError::ChannelUnavailable {
					err: format!(
						...
						log_bytes!((*temporary_channel_id).bytes()[..]), counterparty_node_id),
				})
			},
		};
	...
	pub fn update_partial_channel_config(
		...
		for channel_id in channel_ids {
			if !peer_state.has_channel(channel_id) {
				return Err(APIError::ChannelUnavailable {
					err: format!("Channel with ID {} was not found for the passed counterparty_node_id {}", log_bytes!((*channel_id).bytes()[..]), counterparty_node_id),
				});
			};
		}
		for channel_id in channel_ids {
			...
			let context = if let Some(channel) = peer_state.inbound_v1_channel_by_id.get_mut(channel_id) {
				...
				return Err(APIError::ChannelUnavailable {
					err: format!(
						...
						log_bytes!((*channel_id).bytes()[..]), counterparty_node_id),
				});
			};
			...
			match peer_state.channel_by_id.get(&next_hop_channel_id) {
				Some(chan) => {
					if !chan.context.is_usable() {
						return Err(APIError::ChannelUnavailable {
							err: format!("Channel with id {} not fully established", log_bytes!((*next_hop_channel_id).bytes()[..]))
						})
					}
				...
				None => return Err(APIError::ChannelUnavailable {
					err: format!("Funded channel with id {} not found for the passed counterparty node_id {}. Channel may still be opening.",
						log_bytes!((*next_hop_channel_id).bytes()[..]), next_node_id)
				})
			}
		};
	...
	fn update_channel_fee(&self, chan_id: &ChannelId, chan: &mut Channel<<SP::Target as SignerProvider>::Signer>, new_feerate: u32) -> NotifyOption {
		...
		if new_feerate <= chan.context.get_feerate_sat_per_1000_weight() && new_feerate * 2 > chan.context.get_feerate_sat_per_1000_weight() {
			log_trace!(self.logger, "Channel {} does not qualify for a feerate change from {} to {}.",
				log_bytes!(chan_id.bytes()[..]), chan.context.get_feerate_sat_per_1000_weight(), new_feerate);
			return NotifyOption::SkipPersist;
		}
				...
				for (counterparty_node_id, peer_state_mutex) in per_peer_state.iter() {
					...
					peer_state.channel_by_id.retain(|chan_id, chan| {
						...
						if chan.should_disconnect_peer_awaiting_response() {
							log_debug!(self.logger, "Disconnecting peer {} due to not making any progress on channel {}",
									counterparty_node_id, log_bytes!((*chan_id).bytes()[..]));
					...
					| {
						...
						if unfunded_chan_context.should_expire_unfunded_channel() {
							log_error!(self.logger, "Force-closing pending outbound channel {} for not establishing in a timely manner", log_bytes!(&chan_id.bytes()[..]));
					...
					if let UpdateFulfillCommitFetch::NewClaim { htlc_value_msat, monitor_update } = fulfill_res {
						if let Some(action) = completion_action(Some(htlc_value_msat)) {
							log_trace!(self.logger, "Tracking monitor update completion action for channel {}: {:?}",
								log_bytes!(chan_id.bytes()[..]), action);
	...
	-> Option<(u64, OutPoint, u128, Vec<(PendingHTLCInfo, u64)>)> {
		log_trace!(self.logger, "Handling channel resumption for channel {} with {} RAA, {} commitment update, {} pending forwards, {}broadcasting funding, {} channel ready, {} announcement",
			log_bytes!(channel.context.channel_id().bytes()[..]),
	...
	fn do_accept_inbound_channel(&self, temporary_channel_id: &ChannelId, counterparty_node_id: &PublicKey, accept_0conf: bool, user_channel_id: u128) -> Result<(), APIError> {
		...
		match peer_state.inbound_v1_channel_by_id.entry(temporary_channel_id.clone()) {
			...
			hash_map::Entry::Vacant(_) => {
				return Err(APIError::ChannelUnavailable { err: format!("Channel with id {} not found for the passed counterparty node_id {}", log_bytes!((*temporary_channel_id).bytes()[..]), counterparty_node_id) });
			}
		}
		Ok(())
	}
		...
		match peer_state.channel_by_id.entry(msg.channel_id) {
			hash_map::Entry::Occupied(mut chan) => {
				...
				if let Some(announcement_sigs) = announcement_sigs_opt {
					log_trace!(self.logger, "Sending announcement_signatures for channel {}", log_bytes!(chan.get().context.channel_id().bytes()[..]));
					...
					log_trace!(self.logger, "Sending private initial channel_update for our counterparty on channel {}", log_bytes!(chan.get().context.channel_id().bytes()[..]));
	...
	fn internal_shutdown(&self, counterparty_node_id: &PublicKey, msg: &msgs::Shutdown) -> Result<(), MsgHandleErrInternal> {
		...
		let result: Result<(), _> = loop {
			...
			if let hash_map::Entry::Occupied(chan_entry) = peer_state.outbound_v1_channel_by_id.entry(msg.channel_id.clone()) {
				log_error!(self.logger, "Immediately closing unfunded channel {} as peer asked to cooperatively shut it down (which is unnecessary)", log_bytes!(&msg.channel_id.bytes()[..][..]));
		...
		match peer_state.channel_by_id.entry(chan_id) {
			hash_map::Entry::Occupied(mut chan) => {
				...
				if were_node_one == msg_from_node_one {
					...
					log_debug!(self.logger, "Received channel_update for channel {}.", log_bytes!(chan_id.bytes()[..]));
			...
			if let Some(peer_state_mtx) = per_peer_state.get(&counterparty_node_id) {
				...
				if self.raa_monitor_updates_held(&peer_state.actions_blocking_raa_monitor_updates,
					...
					log_trace!(self.logger, "Delaying monitor unlock for channel {} as another channel's mon update needs to complete first",
						log_bytes!(&channel_funding_outpoint.to_channel_id().bytes()[..][..]));
					break;
				}
					...
					if let Ok((channel_ready_opt, mut timed_out_pending_htlcs, announcement_sigs)) = res {
						...
						if let Some(channel_ready) = channel_ready_opt {
							...
							if channel.context.is_usable() {
								log_trace!(self.logger, "Sending channel_ready with private initial channel_update for our counterparty on channel {}", log_bytes!(channel.context.channel_id().bytes()[..]));
								...
								log_trace!(self.logger, "Sending channel_ready WITHOUT channel_update for {}", log_bytes!(channel.context.channel_id().bytes()[..]));
							}
						}
						...
						if let Some(announcement_sigs) = announcement_sigs {
							log_trace!(self.logger, "Sending announcement_signatures for channel {}", log_bytes!(channel.context.channel_id().bytes()[..]));
							pending_msg_events.push(events::MessageSendEvent::SendAnnouncementSignatures {
								node_id: channel.context.get_counterparty_node_id(),
								msg: announcement_sigs,
							});
			...
			if let Some(ref mut monitor) = args.channel_monitors.get_mut(&funding_txo) {
				if channel.get_cur_holder_commitment_transaction_number() > monitor.get_cur_holder_commitment_number() ||
					...
					log_error!(args.logger, " The ChannelMonitor for channel {} is at update_id {} but the ChannelManager is at update_id {}.",
						log_bytes!(channel.context.channel_id().bytes()[..]), monitor.get_latest_update_id(), channel.context.get_latest_monitor_update_id());
					...
					for (channel_htlc_source, payment_hash) in channel.inflight_htlc_sources() {
						...
						if !found_htlc {
							...
							log_info!(args.logger,
								...
								log_bytes!(channel.context.channel_id().bytes()[..]), log_bytes!(payment_hash.0));
					...
					log_info!(args.logger, "Successfully loaded channel {} at update_id {} against monitor at update id {}",
						log_bytes!(channel.context.channel_id().bytes()[..]), channel.context.get_latest_monitor_update_id(),
				...
				log_error!(args.logger, "Missing ChannelMonitor for channel {} needed by ChannelManager.", log_bytes!(channel.context.channel_id().bytes()[..]));
		...
		for (funding_txo, _) in args.channel_monitors.iter() {
			if !funding_txo_set.contains(funding_txo) {
				log_info!(args.logger, "Queueing monitor update to ensure missing channel {} is force closed",
					log_bytes!(funding_txo.to_channel_id().bytes()[..]));
			 ...
			 $monitor: expr, $peer_state: expr, $channel_info_log: expr
				...
				for update in $chan_in_flight_upds.iter() {
					log_trace!(args.logger, "Replaying ChannelMonitorUpdate {} for {}channel {}",
						update.update_id, $channel_info_log, log_bytes!($funding_txo.to_channel_id().bytes()[..]));
			...
			for (_, chan) in peer_state.channel_by_id.iter() {
				...
				if chan.get_latest_unblocked_monitor_update_id() > max_in_flight_update_id {
					...
					log_error!(args.logger, " The ChannelMonitor for channel {} is at update_id {} with update_id through {} in-flight",
						log_bytes!(chan.context.channel_id().bytes()[..]), monitor.get_latest_update_id(), max_in_flight_update_id);
										...
										if let HTLCForwardInfo::AddHTLC(htlc_info) = forward {
											if pending_forward_matches_htlc(&htlc_info) {
												log_info!(args.logger, "Removing pending to-forward HTLC with hash {} as it was forwarded to the closed channel {}",
													log_bytes!(htlc.payment_hash.0), log_bytes!(monitor.get_funding_txo().0.to_channel_id().bytes()[..]));
												false
											} else { true }
										} else { true }
									});
