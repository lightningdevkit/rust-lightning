...
#[cfg(async_payments)]
use {
	crate::blinded_path::message::{BlindedMessagePath, VerifiedAsyncPaymentContext},
	...
	L::Target: Logger,
{
	...
	pub fn request_refund_payment(
		...
		self.flow.enqueue_invoice(entropy, invoice.clone(), refund, invoice.payment_hash(), self.get_peers_for_blinded_path())?;
	...
	L::Target: Logger,
{
	fn handle_held_htlc_available(
		...
		#[cfg(async_payments)] {
			match self.flow.verify_async_payments_context(_context) {
				Ok(VerifiedAsyncPaymentContext::InboundPayment) => {},
				_ => return None,
			}
	...
	fn handle_release_held_htlc(&self, _message: ReleaseHeldHtlc, _context: AsyncPaymentsContext) {
		#[cfg(async_payments)] {
			let payment_id = match self.flow.verify_async_payments_context(_context) {
				Ok(VerifiedAsyncPaymentContext::OutboundPayment(payment_id)) => payment_id,
				_ => return
			};
