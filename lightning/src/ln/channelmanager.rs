...
pub type SimpleChannelManager<M, T, ES, NS, SP, F, R, MR, L> = ChannelManager<M, T, ES, NS, SP, F, R, MR, L, Arc<DefaultHTLCInterceptHandler>>;
	...
	HIH::Target: HTLCInterceptHandler,
{
	...
	fn close_channel_internal(&self, channel_id: &ChannelId, counterparty_node_id: &PublicKey, target_feerate_sats_per_1000_weight: Option<u32>, override_shutdown_script: Option<ShutdownScript>) -> Result<(), APIError> {
		...
		for htlc_source in failed_htlcs.drain(..) {
			...
			let receiver = HTLCHandlingFailureType::Forward { node_id: Some(*counterparty_node_id), channel_id: *channel_id };
	...
	fn fail_holding_cell_htlcs(
		...
		for (htlc_src, payment_hash) in htlcs_to_fail.drain(..) {
			...
			let receiver = HTLCHandlingFailureType::Forward { node_id: Some(counterparty_node_id.clone()), channel_id };
...
pub mod bench {
	...
	fn test_generic_htlc_intercept_handler_setting() {
		...
		impl TestHTLCInterceptor {
			fn new() -> Self {
				Self {
					intercept_called: AtomicBool::new(false),
				}
			}
		}
		...
		let chain_monitor = test_utils::TestChainMonitor::new(None, &test_utils::TestBroadcaster::new(bitcoin::Network::Testnet), &test_utils::TestLogger::new(), &fee_estimator, &test_utils::TestPersister::new(), &test_utils::TestKeysInterface::new(&[42u8; 32], bitcoin::Network::Testnet));
		let node_signer = test_utils::TestKeysInterface::new(&[42u8; 32], bitcoin::Network::Testnet);
		let keys_manager = test_utils::TestKeysInterface::new(&[42u8; 32], bitcoin::Network::Testnet);
		let logger = test_utils::TestLogger::new();
		let network_graph = Arc::new(crate::routing::gossip::NetworkGraph::new(bitcoin::Network::Testnet, &logger));
		...
		let cm = ChannelManager::new(
			&fee_estimator, &chain_monitor, &test_utils::TestBroadcaster::new(bitcoin::Network::Testnet),
			&router, &message_router, &logger, &keys_manager, &node_signer, &keys_manager,
	...
	fn test_default_htlc_intercept_handler() {
		...
		let intercept_info = InterceptedHTLCInfo {
			...
			prev_counterparty_node_id: Some(PublicKey::from_slice(&[
				3, 23, 183, 225, 206, 31, 159, 132, 211, 9, 19, 200, 182, 31, 227, 185,
				103, 111, 115, 188, 21, 186, 16, 155, 5, 150, 11, 17, 32, 104, 178, 85, 209
			]).unwrap()),
			cltv_expiry: 500000,
		};
		...
		match action {
			HTLCInterceptAction::Forward { next_hop_channel_id, next_node_id, amount_to_forward_msat } => {
