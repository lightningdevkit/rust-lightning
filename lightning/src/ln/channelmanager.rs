	...
	L::Target: Logger,
{
	...
	pub fn new(
		...
		let flow = OffersMessageFlow::new(
			...
			current_timestamp, node_signer.get_inbound_payment_key(), secp_ctx.clone(),
			message_router
		);
	...
	/// Similar to [`ChannelManager::new`], but allows the user to provide a custom
	/// [`OffersMessageFlow`] for the `ChannelManager`.
	///
	/// TODO: Expand this documentation to clearly describe usage once all `OffersMessageFlow`
	/// config parameters are implemented.
	...
	L::Target: Logger,
{
	...
	pub fn create_static_invoice_builder<'a>(
		...
		self.flow.create_static_invoice_builder(
			&self.router, entropy, offer, offer_nonce, amount_msat,
			payment_secret, relative_expiry_secs, self.list_usable_channels(),
	...
	fn pay_for_offer_intern<CPP: FnOnce(&InvoiceRequest, Nonce) -> Result<(), Bolt12SemanticError>>(
		...
		self.flow.enqueue_invoice_request(invoice_request.clone(), payment_id, nonce, self.get_peers_for_blinded_path())?;
	...
	pub fn pay_for_offer_from_human_readable_name(
		...
		self.pending_outbound_payments.add_new_awaiting_offer(
			payment_id, expiration, retry_strategy, route_params_config, amount_msats
		)?;

		self.flow
			.enqueue_dns_onion_message(onion_message, context, dns_resolvers, self.get_peers_for_blinded_path())
			.map_err(|_| ())
	}
	...
	fn best_block_updated(&self, header: &Header, height: u32) {
		...
		*self.best_block.write().unwrap() = BestBlock::new(block_hash, height);
		...
		#[cfg(feature = "dnssec")] {
			...
			self.flow.hrn_resolver.new_best_block(height, timestamp);
		}
	...
	L::Target: Logger,
{
	...
	fn message_received(&self) {
			...
			.release_invoice_requests_awaiting_invoice()
		{
			...
			if self.flow.enqueue_invoice_request(invoice_request, payment_id, nonce, self.get_peers_for_blinded_path()).is_err() {
				log_warn!(
					self.logger,
	...
	L::Target: Logger,
{
	...
	fn handle_release_held_htlc(&self, _message: ReleaseHeldHtlc, _context: AsyncPaymentsContext) {
		#[cfg(async_payments)] {
			let Some(payment_id) = self.flow.verify_outbound_async_payment_context(_context).ok() else {
				return
			};
