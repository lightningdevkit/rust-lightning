...
impl core::hash::Hash for SentHTLCId {
    fn hash<H: core::hash::Hasher>(&self, state: &mut H) {
		match self {
			Self::PreviousHopData { short_channel_id, htlc_id } => {
                short_channel_id.hash(state);
                htlc_id.hash(state);
            },
			Self::OutboundRoute { session_priv } => {
                session_priv.as_ref().hash(state);
            }
        }
    }
}
	...
	L::Target: Logger,
{
	...
	pub fn batch_funding_transaction_generated(&self, temporary_channels: &[(&ChannelId, &PublicKey)], funding_transaction: Transaction) -> Result<(), APIError> {
		...
		}
		{
			...
			if !funding_transaction.input.iter().all(|input| input.sequence == Sequence::MAX) && absolute::LockTime::from(funding_transaction.lock_time).is_block_height() && funding_transaction.lock_time.to_consensus_u32() > height + 1 {
				result = result.and(Err(APIError::APIMisuseError {
...
pub mod bench {
	...
	use bitcoin::{block, Block, Transaction, TxMerkleNode, TxOut};
	...
	pub fn bench_two_sends<P: Persist<InMemorySigner>>(bench: &mut Criterion, bench_name: &str, persister_a: P, persister_b: P) {
		...
		if let Event::FundingGenerationReady { temporary_channel_id, output_script, .. } = get_event!(node_a_holder, Event::FundingGenerationReady) {
			tx = Transaction { version: 2, lock_time: absolute::LockTime::ZERO, input: Vec::new(), output: vec![TxOut {
