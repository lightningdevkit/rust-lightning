...
mod tests {
	...
	impl TestCommitmentTxBuilder {
		...
		fn build(&self, to_broadcaster_sats: u64, to_countersignatory_sats: u64) -> CommitmentTransaction {
			CommitmentTransaction::new(
				...
				self.nondust_htlcs.clone(), &self.channel_parameters.as_holder_broadcastable(), &self.secp_ctx
			)
		}
	}
