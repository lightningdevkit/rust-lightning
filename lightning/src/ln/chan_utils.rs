...
pub(crate) fn commit_and_htlc_tx_fees_sat(
...
pub(crate) fn get_keyed_anchor_output<'a>(
...
impl HolderCommitmentTransaction {
	...
	pub fn dummy(channel_value_satoshis: u64, nondust_htlcs: Vec<HTLCOutputInCommitment>) -> Self {
		...
		let inner = CommitmentTransaction::new(
			0,
			&dummy_key,
			0,
			0,
...
impl CommitmentTransaction {
	...
	fn is_left_greater(
		i: usize, txouts: &Vec<TxOut>, nondust_htlcs: &Vec<HTLCOutputInCommitment>,
	) -> bool {
		txouts[i - 1]
			.value
			.cmp(&txouts[i].value)
	...
	fn rebuild_transaction(
		...
		let (obscured_commitment_transaction_number, txins) =
			Self::build_inputs(self.commitment_number, channel_parameters);

		// First rebuild the htlc outputs, note that `outputs` is now the same length as `self.nondust_htlcs`
		let mut outputs = Self::build_htlc_outputs(
			keys,
			&self.nondust_htlcs,
		...
		if (1..outputs.len())
			.into_iter()
			.any(|i| Self::is_left_greater(i, &outputs, &self.nondust_htlcs))
		{
			return Err(());
		}
		...
		let insert_non_htlc_output = |non_htlc_output: TxOut| {
			let idx = match outputs.binary_search_by(|output| {
				output
					.value
	...
	fn build_outputs_and_htlcs(
		...
		to_countersignatory_value_sat: Amount, nondust_htlcs: &mut Vec<HTLCOutputInCommitment>,
		channel_parameters: &DirectedChannelTransactionParameters,
	) -> Vec<TxOut> {
		// First build and sort the HTLC outputs.
		// Also sort the HTLC output data in `nondust_htlcs` in the same order.
		let mut outputs = Self::build_sorted_htlc_outputs(
			keys,
			nondust_htlcs,
			channel_parameters.channel_type_features(),
		);
	...
	fn insert_non_htlc_outputs<F>(
		keys: &TxCreationKeys, to_broadcaster_value_sat: Amount,
		to_countersignatory_value_sat: Amount,
		channel_parameters: &DirectedChannelTransactionParameters, tx_has_htlc_outputs: bool,
		...
		if to_countersignatory_value_sat > Amount::ZERO {
			let script = if channel_type.supports_anchors_zero_fee_htlc_tx() {
				...
				ScriptBuf::new_p2wpkh(
					&Hash160::hash(&countersignatory_payment_point.serialize()).into(),
				)
			};
	...
	fn build_htlc_outputs(
		keys: &TxCreationKeys, nondust_htlcs: &Vec<HTLCOutputInCommitment>,
	...
	fn build_sorted_htlc_outputs(
		keys: &TxCreationKeys, nondust_htlcs: &mut Vec<HTLCOutputInCommitment>,
		channel_type: &ChannelTypeFeatures,
	) -> Vec<TxOut> {
	...
	pub fn verify<T: secp256k1::Signing + secp256k1::Verification>(
		&self, channel_parameters: &DirectedChannelTransactionParameters, secp_ctx: &Secp256k1<T>,
...
mod tests {
	...
	impl TestCommitmentTxBuilder {
		fn new() -> Self {
			...
			let signer = keys_provider
				.derive_channel_signer(keys_provider.generate_channel_keys_id(false, 0));
			let counterparty_signer = keys_provider
				.derive_channel_signer(keys_provider.generate_channel_keys_id(true, 1));
		...
		fn build(
			&self, to_broadcaster_sats: u64, to_countersignatory_sats: u64,
			nondust_htlcs: Vec<HTLCOutputInCommitment>,
		) -> CommitmentTransaction {
	...
	fn test_anchors() {
		...
		assert_eq!(
			tx.built.transaction.output[1].script_pubkey,
			bitcoin::address::Address::p2wpkh(
				&CompressedPublicKey(builder.counterparty_pubkeys.payment_point),
				Network::Testnet
			)
			.script_pubkey()
		);
		...
		assert_eq!(
			tx.built.transaction.output[0].script_pubkey,
		...
		assert_eq!(
			tx.built.transaction.output[2].script_pubkey,
			get_htlc_redeemscript(
				&received_htlc,
				&ChannelTypeFeatures::anchors_zero_htlc_fee_and_dependencies(),
				&keys
	...
	fn test_building_to_local_justice_tx() {
		...
		.unwrap();
		let pubkey_hash =
			BitcoinPublicKey::new(PublicKey::from_secret_key(&Secp256k1::new(), &secret_key))
				.wpubkey_hash()
				.unwrap();
		let destination_script = ScriptBuf::new_p2wpkh(&pubkey_hash);

		let justice_tx =
			tx.trust().build_to_local_justice_tx(253, destination_script.clone()).unwrap();
		assert_eq!(justice_tx.input.len(), 1);
		assert_eq!(justice_tx.input[0].previous_output.txid, tx.built.transaction.compute_txid());
		assert_eq!(
			justice_tx.input[0].previous_output.vout,
			tx.trust().revokeable_output_index().unwrap() as u32
		);
	...
	fn test_verify_sorted_htlcs() {
		...
		macro_rules! swap_htlcs {
			($small_htlc: expr, $big_htlc: expr) => {
				...
				let txid = transaction.compute_txid();
				let built = BuiltCommitmentTransaction { transaction, txid };
