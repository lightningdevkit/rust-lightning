...
impl CommitmentTransaction {
	...
	// This function splits the txout-HTLC pairs into two vectors, one for the txouts, and another for the HTLCs.
	// As it pushes each HTLC into the HTLC vector, this function also populates the output index of that HTLC.
	...
	fn build_txout_htlc_pairs<'a>(keys: &TxCreationKeys, to_broadcaster_value_sat: Amount, to_countersignatory_value_sat: Amount, nondust_htlcs: &'a Vec<HTLCOutputInCommitment>, channel_parameters: &DirectedChannelTransactionParameters) -> Vec<(TxOut, Option<&'a HTLCOutputInCommitment>)> {
		...
		let mut txout_htlc_pairs: Vec<(TxOut, Option<&HTLCOutputInCommitment>)> = Vec::with_capacity(nondust_htlcs.len() + 4);
		...
		sort_outputs(&mut txout_htlc_pairs, |a: &Option<&HTLCOutputInCommitment>, b: &Option<&HTLCOutputInCommitment>| {
			if let Some(a) = a {
				if let Some(b) = b {
					a.cltv_expiry.cmp(&b.cltv_expiry)
						// Note that due to hash collisions, we have to have a fallback comparison
						// here for fuzzing mode (otherwise at least chanmon_fail_consistency
						// may fail)!
						.then(a.payment_hash.cmp(&b.payment_hash))
	...
	impl TestCommitmentTxBuilder {
		...
		fn build(&mut self, to_broadcaster_sats: u64, to_countersignatory_sats: u64) -> CommitmentTransaction {
			CommitmentTransaction::new(
				...
				&self.nondust_htlcs, &self.channel_parameters.as_holder_broadcastable(), &self.secp_ctx
			)
		}
	}
