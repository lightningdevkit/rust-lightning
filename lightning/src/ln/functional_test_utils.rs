...
use crate::chain::{
	chainmonitor::Persist, BestBlock, ChannelMonitorUpdateStatus, Confirm, Listen, Watch,
};
use crate::events::bump_transaction::{
	BumpTransactionEvent, BumpTransactionEventHandler, Wallet, WalletSource,
};
use crate::events::{
	ClaimedHTLC, ClosureReason, Event, HTLCDestination, MessageSendEvent,
	MessageSendEventsProvider, PathFailure, PaymentFailureReason, PaymentPurpose,
};
use crate::ln::channelmanager::{
	AChannelManager, ChainParameters, ChannelManager, ChannelManagerReadArgs, PaymentId,
	PaymentSendFailure, RAACommitmentOrder, RecipientOnionFields, MIN_CLTV_EXPIRY_DELTA,
};
...
use crate::util::test_utils::{panicking, TestChainMonitor, TestKeysInterface, TestScorer};
...
fn do_connect_block_without_consistency_checks<'a, 'b, 'c, 'd>(
	...
	{
		eprintln!(
			"Connecting block using Block Connection Style: {:?}",
...
impl<'a, 'b, 'c> Node<'a, 'b, 'c> {
	...
	#[cfg(test)]
	pub fn set_channel_signer_available(
		&self, peer_id: &PublicKey, chan_id: &ChannelId, available: bool,
	) {
		let per_peer_state = self.node.per_peer_state.read().unwrap();
		let chan_lock = per_peer_state.get(peer_id).unwrap().lock().unwrap();
		let signer = (|| match chan_lock.channel_by_id.get(chan_id) {
	...
	fn drop(&mut self) {
		if !panicking() {
			...
			for deserialized_monitor in deserialized_monitors.drain(..) {
				...
				if chain_monitor.watch_channel(funding_outpoint, deserialized_monitor)
...
) -> (msgs::RevokeAndACK, msgs::CommitmentSigned) {
		...
		for index in 0..2 {
			if let Ok(mon) = $node.chain_monitor.chain_monitor.get_monitor(
				$crate::chain::transaction::OutPoint {
					txid: bitcoin::Txid::from_slice(&$channel_id.0[..]).unwrap(),
					index,
				},
			) {
				monitor = Some(mon);
...
pub fn _reload_node<'a, 'b, 'c>(
	...
	for monitor in monitors_read.drain(..) {
		...
		assert_eq!(
			node.chain_monitor.watch_channel(funding_outpoint, monitor),
			Ok(ChannelMonitorUpdateStatus::Completed)
		);
...
pub fn sign_funding_transaction<'a, 'b, 'c>(
	...
	let (temporary_channel_id, tx, funding_output) =
		create_funding_transaction(node_a, &node_b.node.get_our_node_id(), channel_value, 42);
...
pub fn open_zero_conf_channel<'a, 'b, 'c, 'd>(
		...
		Event::OpenChannelRequest { temporary_channel_id, .. } => {
			receiver
				...
				.accept_inbound_channel_from_trusted_peer_0conf(
					&temporary_channel_id,
					&initiator.node.get_our_node_id(),
					0,
				)
				.unwrap();
		},
	...
	let (temporary_channel_id, tx, _) =
		create_funding_transaction(&initiator, &receiver.node.get_our_node_id(), 100_000, 42);
	initiator
		.node
		.funding_transaction_generated(
			&temporary_channel_id,
...
pub fn exchange_open_accept_chan<'a, 'b, 'c>(
	...
	let create_chan_id = node_a
		.node
		.create_channel(node_b.node.get_our_node_id(), channel_value, push_msat, 42, None, None)
		.unwrap();
	...
	if node_b.node.get_current_default_configuration().manually_accept_inbound_channels {
		...
		match &events[0] {
			Event::OpenChannelRequest { temporary_channel_id, counterparty_node_id, .. } => node_b
				.node
				.accept_inbound_channel(temporary_channel_id, counterparty_node_id, 42)
				.unwrap(),
			_ => panic!("Unexpected event"),
		};
	}
...
pub fn create_chan_between_nodes_with_value_b<'a, 'b, 'c>(
	...
	node_a
		.node
		.handle_announcement_signatures(&node_b.node.get_our_node_id(), &bs_announcement_sigs);
...
pub fn create_unannounced_chan_between_nodes_with_value<'a, 'b, 'c, 'd>(
	...
	let accept_channel = get_event_msg!(
		nodes[b],
		MessageSendEvent::SendAcceptChannel,
		nodes[a].node.get_our_node_id()
	);
	nodes[a].node.handle_accept_channel(&nodes[b].node.get_our_node_id(), &accept_channel);

	let (temporary_channel_id, tx, _) =
		create_funding_transaction(&nodes[a], &nodes[b].node.get_our_node_id(), channel_value, 42);
	nodes[a]
		.node
		.funding_transaction_generated(
			&temporary_channel_id,
...
pub fn do_check_spends<F: Fn(&bitcoin::blockdata::transaction::OutPoint) -> Option<TxOut>>(
	tx: &Transaction, get_output: F,
) {
	for outp in tx.output.iter() {
		assert!(
			outp.value >= outp.script_pubkey.dust_value().to_sat(),
...
impl SendEvent {
	pub fn from_commitment_update(
		node_id: PublicKey, updates: msgs::CommitmentUpdate,
	) -> SendEvent {
		assert!(updates.update_fulfill_htlcs.is_empty());
		assert!(updates.update_fail_htlcs.is_empty());
		assert!(updates.update_fail_malformed_htlcs.is_empty());
...
pub fn expect_pending_htlcs_forwardable_conditions(
	events: Vec<Event>, expected_failures: &[HTLCDestination],
) {
	match events[0] {
...
pub fn do_main_commitment_signed_dance(
	...
	let (bs_revoke_and_ack, extra_msg_option) = {
		...
		assert!(events.len() <= 2);
		let node_a_event =
			remove_first_msg_event_to_node(&node_a.node.get_our_node_id(), &mut events);
		(
			match node_a_event {
				MessageSendEvent::SendRevokeAndACK { ref node_id, ref msg } => {
...
pub fn do_commitment_signed_dance(
	...
	commitment_signed: &msgs::CommitmentSigned, fail_backwards: bool, skip_last_step: bool,
) {
	check_added_monitors!(node_a, 0);
	assert!(node_a.node.get_and_clear_pending_msg_events().is_empty());
	node_a.node.handle_commitment_signed(&node_b.node.get_our_node_id(), commitment_signed);
	...
	let got_claim = node_a
		.node
		.test_raa_monitor_updates_held(node_b.node.get_our_node_id(), commitment_signed.channel_id);
	if fail_backwards {
		assert!(!got_claim);
	}
...
pub fn get_route(
	send_node: &Node, route_params: &RouteParameters,
) -> Result<Route, msgs::LightningError> {
	let scorer = TestScorer::new();
	let keys_manager =
		TestKeysInterface::new(&[0u8; 32], bitcoin::network::constants::Network::Testnet);
		...
		Event::PaymentForwarded {
			...
			if !upstream_force_closed {
				...
				assert!(node
					...
					.list_channels()
					.iter()
					.any(|x| x.counterparty.node_id == prev_node.node().get_our_node_id()
...
pub fn do_pass_along_path<'a, 'b, 'c>(
	...
	payment_claimable_expected: bool, clear_recipient_events: bool,
	expected_preimage: Option<PaymentPreimage>, is_probe: bool,
) -> Option<Event> {
				...
				match &events_2[0] {
					Event::PaymentClaimable {
						...
						assert!(node
							...
							.list_channels()
							.iter()
							.any(|details| details.channel_id == via_channel_id.unwrap()));
						assert!(node
							.node
							.list_channels()
							.iter()
							.any(|details| details.user_channel_id == via_user_channel_id.unwrap()));
...
pub fn do_claim_payment_along_route_with_extra_penultimate_hop_fees<'a, 'b, 'c>(
	origin_node: &Node<'a, 'b, 'c>, expected_paths: &[&[&Node<'a, 'b, 'c>]],
	expected_extra_fees: &[u32], skip_last: bool, our_payment_preimage: PaymentPreimage,
) -> u64 {
...
pub fn pass_claimed_payment_along_route<'a, 'b, 'c>(
	origin_node: &Node<'a, 'b, 'c>, expected_paths: &[&[&Node<'a, 'b, 'c>]],
	expected_extra_fees: &[u32], skip_last: bool, our_payment_preimage: PaymentPreimage,
) -> u64 {
	...
	match claim_event[0] {
		...
		| Event::PaymentClaimed {
			...
			expected_paths
				.iter()
				.zip(htlcs)
				.for_each(|(path, htlc)| check_claimed_htlc_channel(origin_node, path, htlc));
		},
		Event::PaymentClaimed {
	...
	if events.len() == 1 {
		...
		for expected_path in expected_paths.iter() {
			...
			let ev = remove_first_msg_event_to_node(
				&expected_path[0].node.get_our_node_id(),
				&mut events,
			);
...
pub fn route_over_limit<'a, 'b, 'c>(
	...
	let route = router::get_route(
		&origin_node.node.get_our_node_id(),
		&route_params,
		&network_graph,
		None,
		origin_node.logger,
...
pub fn fail_payment_along_route<'a, 'b, 'c>(
	...
	let expected_destinations: Vec<HTLCDestination> =
		repeat(HTLCDestination::FailedPayment { payment_hash: our_payment_hash })
...
pub fn pass_failed_payment_back<'a, 'b, 'c>(
	...
	let mut per_path_msgs: Vec<((msgs::UpdateFailHTLC, msgs::CommitmentSigned), PublicKey)> =
		Vec::with_capacity(expected_paths.len());
	...
	for i in 0..node_count {
		...
		nodes.push(Node {
			...
			bump_tx_handler: BumpTransactionEventHandler::new(
				cfgs[i].tx_broadcaster,
				Arc::new(Wallet::new(Arc::clone(&wallet_source), cfgs[i].logger)),
				&cfgs[i].keys_manager,
				cfgs[i].logger,
			),
		})
...
macro_rules! get_channel_value_stat {
	($node: expr, $counterparty_node: expr, $channel_id: expr) => {{
		...
		let chan = chan_lock
			.channel_by_id
			.get(&$channel_id)
			.map(|phase| if let ChannelPhase::Funded(chan) = phase { Some(chan) } else { None })
			.flatten()
			.unwrap();
		chan.get_value_stat()
	}};
...
pub fn reconnect_nodes<'a, 'b, 'c, 'd>(args: ReconnectArgs<'a, 'b, 'c, 'd>) {
	...
	node_a
		...
		.peer_connected(
			&node_b.node.get_our_node_id(),
			&msgs::Init {
				features: node_b.node.init_features(),
				networks: None,
				remote_network_address: None,
			},
			true,
	...
	node_b
		...
		.peer_connected(
			&node_a.node.get_our_node_id(),
			&msgs::Init {
				features: node_a.node.init_features(),
				networks: None,
				remote_network_address: None,
			},
			false,
	...
	for chan_msgs in resp_1.drain(..) {
		...
		if pending_raa.0 {
			...
			node_a
				.node
				.handle_revoke_and_ack(&node_b.node.get_our_node_id(), &chan_msgs.1.unwrap());
			assert!(node_a.node.get_and_clear_pending_msg_events().is_empty());
			check_added_monitors!(node_a, 1);
		} else {
			...
			|| pending_responding_commitment_signed.0
		{
			...
			for update_fulfill in commitment_update.update_fulfill_htlcs {
				node_a
					.node
					.handle_update_fulfill_htlc(&node_b.node.get_our_node_id(), &update_fulfill);
			}
	...
	for chan_msgs in resp_2.drain(..) {
		...
		if pending_raa.1 {
			...
			node_b
				.node
				.handle_revoke_and_ack(&node_a.node.get_our_node_id(), &chan_msgs.1.unwrap());
			assert!(node_b.node.get_and_clear_pending_msg_events().is_empty());
			check_added_monitors!(node_b, 1);
		} else {
			assert!(chan_msgs.1.is_none());
			...
			|| pending_responding_commitment_signed.1
		{
			...
			for update_fulfill in commitment_update.update_fulfill_htlcs {
				node_b
					.node
					.handle_update_fulfill_htlc(&node_a.node.get_our_node_id(), &update_fulfill);
			}
			for update_fail in commitment_update.update_fail_htlcs {
			...
			if !pending_responding_commitment_signed.1 {
				...
				node_b.node.handle_commitment_signed(
					&node_a.node.get_our_node_id(),
					&commitment_update.commitment_signed,
				);
				check_added_monitors!(node_b, 1);
...
pub fn create_batch_channel_funding<'a, 'b, 'c>(
	...
	for (other_node, channel_value_satoshis, push_msat, user_channel_id, override_config) in params
	{
		...
		let temp_chan_id = funding_node
			.node
			.create_channel(
				other_node.node.get_our_node_id(),
		...
		other_node
			.node
			.handle_open_channel(&funding_node.node.get_our_node_id(), &open_channel_msg);
		let accept_channel_msg = get_event_msg!(
			other_node,
			MessageSendEvent::SendAcceptChannel,
			funding_node.node.get_our_node_id()
		);
		funding_node
			.node
			.handle_accept_channel(&other_node.node.get_our_node_id(), &accept_channel_msg);
	...
	let tx =
		Transaction { version: 2, lock_time: LockTime::ZERO, input: Vec::new(), output: tx_outs };
	assert!(funding_node
		.node
