...
pub fn pass_claimed_payment_along_route(args: ClaimAlongRouteArgs) -> u64 {
	...
	let expected_dummy_hops_skimmed_fee_msat_and_path_amounts = |htlcs: &[ClaimedHTLC]| {
		assert_eq!(htlcs.len(), args.expected_paths.len()); // One per path.
		assert_eq!(args.dummy_tlvs.len(), args.expected_paths.len());
		// `PaymentClaimed.htlcs` are event-ordered, while `dummy_tlvs` are path-ordered, so
		// recover claimed amounts in expected-path order before recomputing dummy-hop fees.
		check_claimed_htlcs_match_route(args.origin_node, args.expected_paths, htlcs);
		let per_path_claim_amt_msats =
			claimed_htlc_value_msats_for_paths(args.origin_node, args.expected_paths, htlcs);
		let expected_dummy_hops_skimmed_fee_msat = per_path_claim_amt_msats
			.iter()
			.zip(args.dummy_tlvs.iter())
			.map(|(claim_amt_msat, path_dummy_tlvs)| {
				dummy_hops_total_fee_msat(*claim_amt_msat, path_dummy_tlvs)
			})
			.sum::<u64>();
		(expected_dummy_hops_skimmed_fee_msat, per_path_claim_amt_msats)
	};
	let per_path_claim_amt_msats = match claim_event[0] {
		Event::PaymentClaimed {
			...
			let (expected_dummy_hops_skimmed_fee_msat, per_path_claim_amt_msats) =
				expected_dummy_hops_skimmed_fee_msat_and_path_amounts(htlcs);
