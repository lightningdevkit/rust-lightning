...
enum PeerDataProcessingDecision {
	CompleteHandshake(bool, PublicKey),
	Continue,
			...
			&mut PeerState::Authenticating(ref mut handshake) => {
				match handshake.process_act(data) {
					...
					Ok((response_vec_opt, conduit_and_remote_pubkey_opt)) => {
						...
						let requires_response = response_vec_opt.is_some();
						...
						if let Some((conduit, remote_pubkey)) = conduit_and_remote_pubkey_opt {
							(Some(PeerState::Connected(conduit)), PeerDataProcessingDecision::CompleteHandshake(requires_response, remote_pubkey))
						} else {
			...
			let pause_read = match peers.peers.get_mut(peer_descriptor) {
				...
				Some(peer) => {
					...
					match data_processing_decision {
						...
						PeerDataProcessingDecision::CompleteHandshake(needs_to_send_init_message, remote_pubkey) => {
