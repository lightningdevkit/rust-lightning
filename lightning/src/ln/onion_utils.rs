...
/// Process failure we got back from upstream on a payment we sent (implying htlc_source is an
/// OutboundRoute).
pub(crate) fn process_onion_success<T: secp256k1::Signing, L: Deref>(
	secp_ctx: &Secp256k1<T>, logger: &L, path: &Path, outer_session_priv: &SecretKey,
	...
	L::Target: Logger,
{
	...
	// Only consider hops in the regular path for attribution data. Failures in a blinded path are not attributable.
	let regular_hops = construct_onion_keys_generic(secp_ctx, &path.hops, None, outer_session_priv)
	...
	for (route_hop_idx, shared_secret) in regular_hops.enumerate() {
		...
		// will not be attributable.
		let position = attributable_hop_count - route_hop_idx - 1;
		let hold_time = attribution_data.verify(&Vec::new(), shared_secret.as_ref(), position);
		if let Some(hold_time) = hold_time {
...
impl AttributionData {
	...
	pub(crate) fn update(&mut self, message: &[u8], shared_secret: &[u8], hold_time: u32) {
...
mod tests {
	...
	fn test_success_hold_times() {
		...
		let ctx_full = Secp256k1::new();
		let path = build_test_path();
		let hold_times = process_onion_success(
			&ctx_full,
			&logger,
			&path,
