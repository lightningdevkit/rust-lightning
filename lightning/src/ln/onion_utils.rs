...
pub(crate) fn add_hmacs(
	...
	let hold_times = &packet[..MAX_HOPS * HOLD_TIME_LEN];
	let hmacs: &[u8] = &packet[MAX_HOPS * HOLD_TIME_LEN..];
	...
	let hmacs = &mut packet[MAX_HOPS * HOLD_TIME_LEN..];
...
fn verify_attribution_data(
	...
	hmac.input(&message);
	hmac.input(&hold_times[..(MAX_HOPS - route_hop_idx) * HOLD_TIME_LEN]);
