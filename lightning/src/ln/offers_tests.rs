...
use crate::onion_message::messenger::{DefaultMessageRouter, Destination, MessageSendInstructions, NullMessageRouter, PeeledOnion};
...
fn creates_short_lived_offer() {
	...
	let router = DefaultMessageRouter::compact_blinded_paths(alice.network_graph, alice.node.entropy_source);
	let offer = alice.offers_handler
		.create_offer_builder_using_router(router).unwrap()
...
fn fails_creating_or_paying_for_offer_without_connected_peers() {
	...
	let absolute_expiry = alice.offers_handler.duration_since_epoch() + MAX_SHORT_LIVED_RELATIVE_EXPIRY;
	match alice.offers_handler.create_offer_builder() {
