...
fn creates_and_pays_for_offer_using_two_hop_blinded_path() {
	...
	*node_cfgs[1].override_init_features.borrow_mut() = Some(features);
	...
	let payment_id = PaymentId([1; 32]);
	david.node.pay_for_offer(&offer, None, payment_id, OptionalOfferPaymentParams::default(), None)
		.unwrap();
...
fn creates_and_pays_for_offer_using_one_hop_blinded_path() {
	...
	let payment_id = PaymentId([1; 32]);
	bob.node.pay_for_offer(&offer, None, payment_id, OptionalOfferPaymentParams::default(), None).unwrap();
...
fn fails_creating_or_paying_for_offer_without_connected_peers() {
	...
	match david.node.pay_for_offer(&offer, None, payment_id, OptionalOfferPaymentParams::default(), None) {
		Ok(_) => panic!("Expected error"),
		Err(e) => assert_eq!(e, Bolt12SemanticError::MissingPaths),
	}
	...
	assert!(
		david.node.pay_for_offer(
			&offer, None, payment_id, OptionalOfferPaymentParams::default(), None
		).is_ok()
	);
...
fn fails_creating_invoice_request_for_unsupported_chain() {
	...
	let payment_id = PaymentId([1; 32]);
	match bob.node.pay_for_offer(&offer, None, payment_id, OptionalOfferPaymentParams::default(), None) {
		Ok(_) => panic!("Expected error"),
...
fn rejects_keysend_to_non_static_invoice_path() {
	...
	let amt_msat = 5000;
	let payment_id = PaymentId([1; 32]);
	nodes[0].node.pay_for_offer(&offer, Some(amt_msat), payment_id, OptionalOfferPaymentParams::default(), None).unwrap();
...
fn no_double_pay_with_stale_channelmanager() {
	...
	let payment_id = PaymentId([1; 32]);
	nodes[0].node.pay_for_offer(&offer, None, payment_id, OptionalOfferPaymentParams::default(), None).unwrap();
	expect_recent_payment!(nodes[0], RecentPaymentDetails::AwaitingInvoice, payment_id);
