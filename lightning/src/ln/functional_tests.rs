...
fn test_htlc_on_chain_success() {
	...
	assert_eq!(node_txn[0].lock_time.to_consensus_u32(), 0);
	assert_eq!(node_txn[1].lock_time.to_consensus_u32(), 0);
	...
	macro_rules! check_tx_local_broadcast {
		($node: expr, $htlc_offered: expr, $commitment_tx: expr) => { {
			...
			assert_ne!(node_txn[0].lock_time.to_consensus_u32(), 0);
			assert_ne!(node_txn[1].lock_time.to_consensus_u32(), 0);
			if $htlc_offered {
...
fn test_force_close_fail_back() {
	...
	assert_eq!(node_txn[0].lock_time.to_consensus_u32(), 0); // Must be an HTLC-Success
...
fn test_static_spendable_outputs_justice_tx_revoked_htlc_timeout_tx() {
	...
	assert_ne!(revoked_htlc_txn[0].lock_time.to_consensus_u32(), 0); // HTLC-Timeout
...
fn test_onchain_to_onchain_claim() {
	...
	assert_eq!(c_txn[0].lock_time.to_consensus_u32(), 0); // Success tx
...
fn test_invalid_funding_tx() {
	...
	let mut spend_tx = Transaction {
		version: 2i32, lock_time: absolute::LockTime::ZERO,
...
fn test_non_final_funding_tx() {
	...
	let events = nodes[0].node.get_and_clear_pending_events();
	let input = TxIn { previous_output: BitcoinOutPoint::null(), script_sig: bitcoin::ScriptBuf::new(), sequence: Sequence(1), witness: Witness::from_slice(vec!(vec!(1))) };
	assert_eq!(events.len(), 1);
	let mut tx = match events[0] {
		Event::FundingGenerationReady { ref channel_value_satoshis, ref output_script, .. } => {
			// Timelock the transaction _beyond_ the best client height + 1.
			Transaction { version: chan_id as i32, lock_time: absolute::LockTime::from_consensus(best_height + 2), input: vec![input], output: vec![TxOut {
...
fn test_non_final_funding_tx_within_headroom() {
	...
	let mut tx = match events[0] {
		Event::FundingGenerationReady { ref channel_value_satoshis, ref output_script, .. } => {
			...
			Transaction { version: chan_id as i32, lock_time: absolute::LockTime::from_consensus(best_height + 1), input: vec![input], output: vec![TxOut {
				value: *channel_value_satoshis, script_pubkey: output_script.clone(),
			}]}
		},
		_ => panic!("Unexpected event"),
