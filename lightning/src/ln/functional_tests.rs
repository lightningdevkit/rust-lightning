...
fn do_channel_reserve_test(test_recv: bool) {
...

	{
		...
		if test_recv {
			...
			assert_eq!(nodes[1].node.list_channels().len(), 1);
			let handle_error = &check_closed_broadcast!(nodes[1], true)[1];
...
fn test_update_add_htlc_bolt2_receiver_check_amount_received_more_than_min() {
	...
	if let MessageSendEvent::HandleError { ref action, .. } = *handle_error {
		match action {
			&ErrorAction::SendErrorMessage { ref msg } => {
				assert_eq!(msg.data, "Remote side tried to send less than our minimum HTLC value");
			},
			_ => panic!("Unexpected event!"),
		}
	}
...
fn test_update_add_htlc_bolt2_receiver_check_max_htlc_limit() {
	...
	if let &MessageSendEvent::HandleError { ref action, .. } = handle_error {
		match action {
			&ErrorAction::SendErrorMessage { ref msg } => {
				assert_eq!(msg.data, "Remote tried to push more than our max accepted HTLCs");
			},
			_ => panic!("Unexpected event!"),
		}
	}
...
fn test_update_add_htlc_bolt2_receiver_check_max_in_flight_msat() {
	...
	if let &MessageSendEvent::HandleError { ref action, .. } = handle_error {
		match action {
			&ErrorAction::SendErrorMessage { ref msg } => {
				assert_eq!(msg.data,"Remote HTLC add would put them over our max HTLC value");
			},
			_ => panic!("Unexpected event!"),
		}
	}
...
fn test_update_add_htlc_bolt2_receiver_check_cltv_expiry() {
	...
	if let &MessageSendEvent::HandleError { ref action, .. } = handle_error {
		match action {
			&ErrorAction::SendErrorMessage { ref msg } => {
				assert_eq!(msg.data,"Remote provided CLTV expiry in seconds instead of block height");
			},
			_ => panic!("Unexpected event!"),
		}
	}
...
fn test_update_add_htlc_bolt2_receiver_check_repeated_id_ignore() {
	...
	if let &MessageSendEvent::HandleError { ref action, .. } = handle_error {
		match action {
			&ErrorAction::SendErrorMessage { ref msg } => {
				assert_eq!(msg.data, "Remote skipped HTLC ID");
			},
			_ => panic!("Unexpected event!"),
		}
	}
...
fn test_update_fulfill_htlc_bolt2_update_fulfill_htlc_before_commitment() {
	...
	assert!(nodes[0].node.list_channels().is_empty());
	let handle_error = &check_closed_broadcast!(nodes[0], true)[1];
	if let &MessageSendEvent::HandleError { ref action, .. } = handle_error {
		match action {
			&ErrorAction::SendErrorMessage { ref msg } => {
				assert_eq!(msg.data, "Remote tried to fulfill/fail HTLC before it had been committed");
			},
			_ => panic!("Unexpected event!"),
		}
	} else { assert!(false); }
...
fn test_update_fulfill_htlc_bolt2_incorrect_htlc_id() {
	...
	if let &MessageSendEvent::HandleError { ref action, .. } = handle_error {
		match action {
			&ErrorAction::SendErrorMessage { ref msg } => {
				assert_eq!(msg.data, "Remote tried to fulfill/fail an HTLC we couldn't find");
			},
			_ => panic!("Unexpected event!"),
		}
	} else { assert!(false); }
...
fn test_update_fulfill_htlc_bolt2_wrong_preimage() {
	...
	if let &MessageSendEvent::HandleError { ref action, .. } = handle_error {
		match action {
			&ErrorAction::SendErrorMessage { ref msg } => {
				assert_eq!(msg.data, "Remote tried to fulfill HTLC with an incorrect preimage");
			},
			_ => panic!("Unexpected event!"),
		}
	} else { assert!(false); }
...
fn test_update_fulfill_htlc_bolt2_missing_badonion_bit_for_malformed_htlc_message() {
	...
	if let &MessageSendEvent::HandleError { ref action, .. } = handle_error {
		match action {
			&ErrorAction::SendErrorMessage { ref msg } => {
				assert_eq!(msg.data, "Got update_fail_malformed_htlc with BADONION not set");
			},
			_ => panic!("Unexpected event!"),
		}
	} else { assert!(false); }
