...
fn do_test_counterparty_no_reserve(send_from_initiator: bool) {
	...
	let channel_type = ChannelType::Legacy;
	push_amt -= feerate_per_kw as u64 * (commitment_tx_base_weight(&channel_type) + 4 * COMMITMENT_TX_WEIGHT_PER_HTLC) / 1000 * 1000;
	...
	if send_from_initiator {
		send_payment(&nodes[0], &[&nodes[1]], 100_000_000
			...
			- FEE_SPIKE_BUFFER_FEE_INCREASE_MULTIPLE * commit_tx_fee_msat(feerate_per_kw, 2, &channel_type));
	} else {
...
fn test_update_fee_that_funder_cannot_afford() {
	...
	// remote limit.
	let feerate = ((channel_value - bs_channel_reserve_sats - push_sats) * 1000 / (commitment_tx_base_weight(&channel_type) + CONCURRENT_INBOUND_HTLC_FEE_BUFFER as u64 * COMMITMENT_TX_WEIGHT_PER_HTLC)) as u32;
	let non_buffer_feerate = ((channel_value - bs_channel_reserve_sats - push_sats) * 1000 / commitment_tx_base_weight(&channel_type)) as u32;
	{
		let mut feerate_lock = chanmon_cfgs[0].fee_estimator.sat_per_kw.lock().unwrap();
	...
	// Confirm that the new fee based on the last local commitment txn is what we expected based on the feerate set above.
	{
		...
		let total_fee: u64 = commit_tx_fee_msat(feerate, 0, &channel_type) / 1000;
	...
	let res = {
		...
		let commitment_tx = CommitmentTransaction::new_with_auxiliary_htlc_data(
			...
			push_sats,
			channel_value - push_sats - commit_tx_fee_msat(non_buffer_feerate + 4, 0, &channel_type) / 1000,
...
fn test_chan_reserve_violation_outbound_htlc_inbound_chan() {
	...
	let mut push_amt = 100_000_000;
	push_amt -= commit_tx_fee_msat(feerate_per_kw, MIN_AFFORDABLE_HTLC_COUNT as u64, &channel_type);
...
fn test_chan_reserve_dust_inbound_htlcs_outbound_chan() {
	...
	let dust_amt = crate::ln::channel::MIN_CHAN_DUST_LIMIT_SATOSHIS * 1000
		+ feerate_per_kw as u64 * htlc_success_tx_weight(&channel_type) / 1000 * 1000 - 1;
...
fn test_chan_reserve_violation_inbound_htlc_inbound_chan() {
	...
	let channel_type = get_opt_anchors!(nodes[0], nodes[1], chan.2);

	// Add a 2* and +1 for the fee spike reserve.
	let commit_tx_fee_2_htlc = 2*commit_tx_fee_msat(feerate, 2 + 1, &channel_type);
	...
	let commit_tx_fee_2_htlcs = commit_tx_fee_msat(feerate, 2, &channel_type);
...
fn commit_tx_fee_msat(feerate: u32, num_htlcs: u64, channel_type: &ChannelType) -> u64 {
	(commitment_tx_base_weight(channel_type) + num_htlcs * COMMITMENT_TX_WEIGHT_PER_HTLC) * feerate as u64 / 1000 * 1000
}
...
fn test_channel_reserve_holding_cell_htlcs() {
	...
	let channel_type = get_opt_anchors!(nodes[0], nodes[1], chan_1.2);
	...
	loop {
		...
		let commit_tx_fee_all_htlcs = 2*commit_tx_fee_msat(feerate, 3 + 1, &channel_type);
	...
	// policy.
	let commit_tx_fee_2_htlcs = 2*commit_tx_fee_msat(feerate, 2 + 1, &channel_type);
	...
	let commit_tx_fee_3_htlcs = 2*commit_tx_fee_msat(feerate, 3 + 1, &channel_type);
	...
	let commit_tx_fee_0_htlcs = 2*commit_tx_fee_msat(feerate, 1, &channel_type);
	...
	let commit_tx_fee_1_htlc = 2*commit_tx_fee_msat(feerate, 1 + 1, &channel_type);
...
fn test_fail_holding_cell_htlc_upon_free() {
	...
	let max_can_send = 5000000 - channel_reserve - 2*commit_tx_fee_msat(feerate, 1 + 1, &channel_type);
...
fn test_free_and_fail_holding_cell_htlcs() {
	...
	// 2* and +1 HTLCs on the commit tx fee calculation for the fee spike reserve.
	let amt_1 = 20000;
	let amt_2 = 5000000 - channel_reserve - 2*commit_tx_fee_msat(feerate, 2 + 1, &channel_type) - amt_1;
...
fn test_fail_holding_cell_htlc_upon_free_multihop() {
	...
	let channel_type = get_opt_anchors!(nodes[0], nodes[1], chan_0_1.2);

	// Send a payment which passes reserve checks but gets stuck in the holding cell.
	let feemsat = 239;
	let total_routing_fee_msat = (nodes.len() - 2) as u64 * feemsat;
	let max_can_send = 5000000 - channel_reserve - 2*commit_tx_fee_msat(feerate, 1 + 1, &channel_type) - total_routing_fee_msat;
...
fn test_update_add_htlc_bolt2_receiver_sender_can_afford_amount_sent() {
	...
	let channel_type = get_opt_anchors!(nodes[0], nodes[1], chan.2);
	// The 2* and +1 are for the fee spike reserve.
	let commit_tx_fee_outbound = 2 * commit_tx_fee_msat(feerate, 1 + 1, &channel_type);
...
fn do_test_max_dust_htlc_exposure(dust_outbound_balance: bool, exposure_breach_event: ExposureEvent, on_holder_tx: bool) {
	...
	let dust_outbound_htlc_on_holder_tx_msat: u64 = (dust_buffer_feerate * htlc_timeout_tx_weight(&channel_type) / 1000 + open_channel.dust_limit_satoshis - 1) * 1000;
	...
	let dust_inbound_htlc_on_holder_tx_msat: u64 = (dust_buffer_feerate * htlc_success_tx_weight(&channel_type) / 1000 + open_channel.dust_limit_satoshis - 1) * 1000;
