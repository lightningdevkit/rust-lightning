...
use crate::ln::chan_utils::{
	commitment_tx_base_weight, htlc_success_tx_weight, COMMITMENT_TX_WEIGHT_PER_HTLC,
	OFFERED_HTLC_SCRIPT_WEIGHT,
};
use crate::ln::channel::{
	get_holder_selected_channel_reserve_satoshis, second_stage_tx_fees_sat, Channel, ChannelError,
	InboundV1Channel, OutboundV1Channel, COINBASE_MATURITY,
	DISCONNECT_PEER_AWAITING_RESPONSE_TICKS, MIN_CHAN_DUST_LIMIT_SATOSHIS,
};
...
fn do_test_max_dust_htlc_exposure(
	...
	commitment_tx_cost_msat += per_htlc_cost * nondust_htlc_count_in_limit;
	{
		let mut feerate_lock = chanmon_cfgs[0].fee_estimator.sat_per_kw.lock().unwrap();
	...
	let (
		...
		let dust_buffer_feerate = chan.context().get_dust_buffer_feerate(None) as u64;
