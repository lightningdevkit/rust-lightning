...
fn test_invalid_channel_announcement() {
	...
	macro_rules! dummy_unsigned_msg {
		() => {
			msgs::UnsignedChannelAnnouncement {
				...
				chain_hash: genesis_block(Network::Testnet).block_hash(),
	...
	unsigned_msg.chain_hash = genesis_block(Network::Bitcoin).block_hash();
	sign_msg!(unsigned_msg);
...
fn bolt2_open_channel_sending_node_checks_part2() {
	...
	let chain_hash=genesis_block(Network::Testnet).block_hash();
