...
pub(super) trait InteractivelyFunded<SP: Deref>: HasChannelContext<SP> + HasDualFundingChannelContext where SP::Target: SignerProvider {
		...
		L::Target: Logger
	{
		...
		for (idx, outp) in signing_session.unsigned_tx.outputs().enumerate() {
			if outp.tx_out().script_pubkey == expected_spk && outp.tx_out().value == context.get_value_satoshis() {
		...
		let commitment_signed = get_initial_commitment_signed(context, signing_session.unsigned_tx.clone(), logger);
		...
		if our_funding_satoshis == 0 {
			...
			funding_ready_for_sig_event = Some(Event::FundingTransactionReadyForSigning {
				...
				unsigned_transaction: signing_session.unsigned_tx.clone().into_unsigned_tx(),
			});
		}
...
impl<SP: Deref> ChannelContext<SP> where SP::Target: SignerProvider  {
	...
	where ES::Target: EntropySource
	{
		...
		let (tx_constructor, msg) = InteractiveTxConstructor::new(
			...
			dual_funding_context.our_funding_inputs.clone(), funding_outputs
		);
