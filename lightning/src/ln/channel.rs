...
impl<SP: Deref> OutboundV1Channel<SP> where SP::Target: SignerProvider {
		...
		L::Target: Logger
	{
		...
		if self.unfunded_context.holder_commitment_point.is_none() {
			return Err((self, ChannelError::close("Received funding_signed before our first commitment point was available".to_owned())));
		}
		...
		let holder_signer = self.context.build_holder_transaction_keys(holder_commitment_point.current_point());
		let current_transaction_number = self.unfunded_context.transaction_number();
		let initial_commitment_tx = self.context.build_commitment_transaction(current_transaction_number, &holder_signer, true, false, logger).tx;
		{
		...
		if holder_commitment_point.advance(&self.context.holder_signer, &self.context.secp_ctx, logger).is_err() {
			// We only fail to advance our commitment point/number if we're currently
			// waiting for our signer to unblock and provide a commitment point.
...
impl<SP: Deref> InboundV1Channel<SP> where SP::Target: SignerProvider {
	...
	fn check_funding_created_signature<L: Deref>(&mut self, sig: &Signature, logger: &L) -> Result<CommitmentTransaction, ChannelError> where L::Target: Logger {
		...
		let current_point = self.unfunded_context.holder_commitment_point
			.expect("Holder commitment must be Some by the time we receive funding_created").current_point();
		let keys = self.context.build_holder_transaction_keys(current_point);
		let current_transaction_number = self.unfunded_context.transaction_number();
		let initial_commitment_tx = self.context.build_commitment_transaction(current_transaction_number, &keys, true, false, logger).tx;
		let trusted_tx = initial_commitment_tx.trust();
		let initial_commitment_bitcoin_tx = trusted_tx.built_transaction();
		let sighash = initial_commitment_bitcoin_tx.get_sighash_all(&funding_script, self.context.channel_value_satoshis);
		...
		L::Target: Logger
	{
		...
		if self.unfunded_context.holder_commitment_point.is_none() {
			return Err((self, ChannelError::close("Received funding_created before our first commitment point was available".to_owned())));
		}
