...
impl<SP: Deref> ChannelContext<SP> where SP::Target: SignerProvider  {
	...
	pub fn has_first_holder_per_commitment_point(&self) -> bool {
		self.next_per_commitment_point.is_some()
	}
	...
	fn build_next_holder_transaction_keys(&self) -> TxCreationKeys {
		...
		TxCreationKeys::derive_new(
			&self.secp_ctx, &self.next_per_commitment_point.expect("channel isn't ready"), delayed_payment_base, htlc_basepoint,
...
impl<SP: Deref> OutboundV1Channel<SP> where SP::Target: SignerProvider {
	      ...
	      F::Target: FeeEstimator
	{
		...
		let next_per_commitment_point = holder_signer.get_per_commitment_point(INITIAL_COMMITMENT_NUMBER, &secp_ctx).ok();
	...
	pub fn get_open_channel(&self, chain_hash: BlockHash) -> msgs::OpenChannel {
		...
		msgs::OpenChannel {
			...
			first_per_commitment_point: self.context.next_per_commitment_point.expect("channel isn't ready"),
...
impl<SP: Deref> InboundV1Channel<SP> where SP::Target: SignerProvider {
		...
		L::Target: Logger
	{
		...
		// Retrieve the next commitment point: if this results in a transient failure we'll unwind here
		// and rely on retry to complete the funding_created operation.
		let res = self.context.get_next_holder_per_commitment_point(logger);
		if let Err(e) = res {
			log_error!(logger, "Unable to handle funding_created: {}", e);
			return Err((self, e));
		}
		let (next_holder_commitment_transaction_number, next_per_commitment_point) = res.unwrap();
