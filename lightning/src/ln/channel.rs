...
{ // Channel
	...
	pub fn unfunded_context_mut(&mut self) -> Option<&mut UnfundedChannelContext> {
		match &mut self.phase {
			...
			#[cfg(splicing)]
			ChannelPhase::RefundingV2(_) => { debug_assert!(false); None }, // TODO check/change
		}
	}

	pub fn is_funded(&self) -> bool {
		match &self.phase {
			ChannelPhase::Funded(_) => true,
			#[cfg(splicing)]
			ChannelPhase::RefundingV2(_) => true,
			_ => false,
		}
	}
	...
	pub fn as_funded(&self) -> Option<&FundedChannel<SP>> {
		match &self.phase {
			...
			#[cfg(splicing)]
			ChannelPhase::RefundingV2(channel) => Some(&channel.pre_funded),
			_ => None,
		}
	}
	...
	pub fn as_funded_mut(&mut self) -> Option<&mut FundedChannel<SP>> {
		match &mut self.phase {
			ChannelPhase::Funded(channel) => Some(channel),
			#[cfg(splicing)]
			ChannelPhase::RefundingV2(channel) => Some(&mut channel.pre_funded),
			_ => None,
		}
	}
	...
	pub fn signer_maybe_unblocked<L: Deref>(
		...
		match &mut self.phase {
			...
			#[cfg(splicing)]
			ChannelPhase::RefundingV2(_chan) => panic!("TODO(splicing)"), // TODO change Some(chan.signer_maybe_unblocked(logger)),
		}
	}
	...
	pub fn peer_disconnected_is_resumable<L: Deref>(&mut self, logger: &L) -> bool where L::Target: Logger {
		match &mut self.phase {
			...
			#[cfg(splicing)]
			ChannelPhase::RefundingV2(_chan) => panic!("TODO(splicing)"), // TODO change chan.remove_uncommitted_htlcs_and_mark_paused(logger).is_ok(), // TODO check
		}
	}
	...
	pub fn peer_connected_get_handshake<L: Deref>(
		...
		match &mut self.phase {
			...
			#[cfg(splicing)]
			ChannelPhase::RefundingV2(_chan) => panic!("TODO(splicing)"), // TODO change ReconnectionMsg::Reestablish(chan.get_channel_reestablish(logger)),
		}
	}
	...
	/// TODO Simplify once unified using traits
	pub fn tx_add_input(&mut self, msg: &msgs::TxAddInput) -> InteractiveTxMessageSendResult {
		...
		panic!("Got tx_add_input in an invalid phase");
	}
	...
	pub fn tx_add_output(&mut self, msg: &msgs::TxAddOutput) -> InteractiveTxMessageSendResult {
		...
		panic!("Got tx_add_output in an invalid phase");
	}
	...
	pub fn tx_complete(&mut self, msg: &msgs::TxComplete) -> HandleTxCompleteResult {
		...
		panic!("Got tx_complete in an invalid phase");
	}
		...
		L::Target: Logger
	{
		match self.phase {
			ChannelPhase::UnfundedV2(ref mut chan) => {
				let logger = WithChannelContext::from(logger, &chan.context, None);
				let (commitment_signed, event) = chan.funding_tx_constructed(counterparty_node_id, signing_session, &&logger)?;
				Ok((commitment_signed, event))
			}
			#[cfg(splicing)]
			ChannelPhase::RefundingV2(ref mut chan) => {
				let logger = WithChannelContext::from(logger, &chan.context(), None);
...
/// Struct holding together various state during splicing negotiation
#[cfg(splicing)]
pub(super) struct SplicingChannel<SP: Deref> where SP::Target: SignerProvider {
	...
	/// TODO remove
	/// Info about an in-progress, pending splice, on the post-splice channel
...
impl<SP: Deref> SplicingChannel<SP> where SP::Target: SignerProvider {
	...
	where ES::Target: EntropySource, L::Target: Logger
	{
		...
		let splice_ack_msg = self.pre_funded.get_splice_ack(our_funding_contribution);

		// Start interactive funding negotiation. No extra input, as we are not the splice initiator
		let _msg = self.begin_interactive_funding_tx_construction(signer_provider, entropy_source, holder_node_id.clone(), None, None)
	...
	where ES::Target: EntropySource, L::Target: Logger
	{
		...
		let pre_channel_value = self.pre_funded.funding.get_value_satoshis();
		let _post_channel_value = PendingSplicePre::compute_post_value(pre_channel_value, our_funding_contribution, their_funding_contribution_satoshis);
		let _post_balance = PendingSplicePre::add_checked(self.pre_funded.funding.value_to_self_msat, our_funding_contribution);
		...
		let prev_funding_input = self.pending_splice_post.get_input_of_previous_funding()?;
		...
		let tx_msg_opt = self.begin_interactive_funding_tx_construction(signer_provider, entropy_source, holder_node_id.clone(), None, Some(prev_funding_input))
	...
	pub(crate) fn splice_start<L: Deref>(&mut self, is_outgoing: bool, logger: &L) where L::Target: Logger {
		...
		log_info!(logger, "Splicing process started, new channel value {}, outgoing {}, channel_id {}",
			self.post_funding.get_value_satoshis(), is_outgoing, self.context().channel_id);
	}

	/// Copy of PendingV2Channel::begin_interactive_funding_tx_construction
	...
	{
		// debug_assert!(matches!(self.pre_funded.context.channel_state, ChannelState::NegotiatingFunding(_)));
		...
		let shared_funding_output = TxOut {
			value: Amount::from_sat(self.post_funding.get_value_satoshis()),
			script_pubkey: self.post_funding.get_funding_redeemscript().to_p2wsh(),
		};

		if self.pre_funded.funding.is_outbound() {
		...
		let change_script = if let Some(script) = change_destination_opt {
			...
			signer_provider.get_destination_script(self.pre_funded.context.channel_keys_id)
				.map_err(|_err| AbortReason::InternalError("Error getting destination script"))?
		};
		let change_value_opt = calculate_change_output_value(
			self.pre_funded.funding.is_outbound(), self.dual_funding_context.our_funding_satoshis,
		...
		let constructor_args = InteractiveTxConstructorArgs {
			...
			is_initiator: self.pre_funded.funding.is_outbound(),
	...
	/// TODO: remove once PendingV2Channel is a trait
	pub fn tx_add_input(&mut self, msg: &msgs::TxAddInput) -> InteractiveTxMessageSendResult {
	...
	pub fn tx_complete(&mut self, msg: &msgs::TxComplete) -> HandleTxCompleteResult {
		...
		// TODO: Should next_funding_txid be in FundingScope?
	...
	) -> Result<(InteractiveTxSigningSession, HolderCommitmentPoint, msgs::CommitmentSigned, Option<Event>), ChannelError>
	where
		L::Target: Logger
	{
		let our_funding_satoshis = self.dual_funding_context.our_funding_satoshis;
		let transaction_number = self.unfunded_context.transaction_number();
		let is_splice_pending = true;

		let mut output_index = None;
		let expected_spk = self.post_funding.get_funding_redeemscript().to_p2wsh();
		...
		self.post_funding.channel_transaction_parameters.funding_outpoint = Some(outpoint);
		// self.context_mut().holder_signer.as_mut_ecdsa().provide_channel_parameters(&self.post_funding.channel_transaction_parameters);

		// Assert for no commitment, unless splicing
		if !is_splice_pending {
			self.context().assert_no_commitment_advancement(transaction_number, "initial commitment_signed");
		}
		let commitment_signed = self.pre_funded.context.get_initial_commitment_signed(&self.post_funding, is_splice_pending, logger);
		let commitment_signed = match commitment_signed {
			Ok(commitment_signed) => {
				self.post_funding.funding_transaction = Some(signing_session.unsigned_tx.build_unsigned_tx());
				commitment_signed
			},
			Err(err) => {
				self.post_funding.channel_transaction_parameters.funding_outpoint = None;
				return Err(ChannelError::Close((err.to_string(), ClosureReason::HolderForceClosed { broadcasted_latest_txn: Some(false) })));
			},
		};

		#[cfg(not(splicing))]
		let partly_signed_transaction =signing_session.unsigned_tx.clone().build_unsigned_tx();
		#[cfg(splicing)]
		let mut partly_signed_transaction =signing_session.unsigned_tx.clone().build_unsigned_tx();
		#[cfg(splicing)]
		if is_splice_pending {
			// Add signature for prev funding input
			// Note: here the transaction is used for signing, input&output order matters
			let (partly_signed_tx, holder_signature) = self.prev_funding_tx_sign(&partly_signed_transaction, None, logger)?;
			signing_session.shared_signature = Some(holder_signature);
			partly_signed_transaction = partly_signed_tx;
		}
		...
		if signing_session.local_inputs_count() == 0 {
			...
			if signing_session.provide_holder_witnesses(self.context().channel_id, Vec::new(), signing_session.shared_signature.clone()).is_err() {
				debug_assert!(
					false,
		...
		*/
		...
		if our_funding_satoshis == 0 {
			let _res = signing_session.provide_holder_witnesses(self.context().channel_id, Vec::new(), signing_session.shared_signature.clone());
		} else {
	...
	fn prev_funding_tx_sign<L: Deref>(
		...
		let (prev_funding_input_index, pre_channel_value) =
			(
				self.pending_splice_post.find_input_of_previous_funding(&transaction)?,
				self.pending_splice_post.pre_channel_value
			);
		debug_assert!(prev_funding_input_index < transaction.input.len());

		// #SPLICE-SIG
		// the redeem script
		let sig_order_ours_first = self.post_funding.get_holder_pubkeys().funding_pubkey.serialize() < self.post_funding.counterparty_funding_pubkey().serialize();
		log_info!(logger, "Pubkeys used for redeem script: {} {} {}", &self.post_funding.get_holder_pubkeys().funding_pubkey, &self.post_funding.counterparty_funding_pubkey(), sig_order_ours_first);

		let redeem_script = self.post_funding.get_funding_redeemscript();
	...
	#[cfg(splicing)]
	fn prev_funding_tx_create_holder_sig(&self, transaction: &Transaction, input_index: usize, input_value: u64/*, _redeem_script: &ScriptBuf*/) -> Result<Signature, ChannelError> {
		// #SPLICE-SIG
		match &self.context().holder_signer {
			ChannelSignerType::Ecdsa(ecdsa) => {
				ecdsa.sign_splicing_funding_input(&self.post_funding.channel_transaction_parameters, transaction, input_index, input_value, /*&redeem_script, */&self.context().secp_ctx)
					.map_err(|_e| {
						let msg = "Failed to sign the previous funding input in the new splicing funding tx";
						ChannelError::Close((msg.to_owned(), ClosureReason::ProcessingError { err: msg.to_owned() }))
					})
			},
			// TODO (taproot|arik)
			#[cfg(taproot)]
...
impl PendingSplicePost {
	pub(crate) fn new(
		...
		let post_channel_value = PendingSplicePre::compute_post_value(pre_channel_value, our_funding_contribution, their_funding_contribution);
		Self {
	...
	fn get_input_of_previous_funding(&self) -> Result<(TxIn, TransactionU16LenLimited), ChannelError> {
	...
	fn find_input_of_previous_funding(&self, tx: &Transaction) -> Result<usize, ChannelError> {
		if let Some(pre_funding_txo) = &self.pre_funding_txo {
...
impl<SP: Deref> PendingV2Channel<SP> where SP::Target: SignerProvider {
		...
		L::Target: Logger
	{
		...
		let commitment_signed = self.context.get_initial_commitment_signed(&self.funding, is_splice_pending, logger);
		let commitment_signed = match commitment_signed {
			Ok(commitment_signed) => {
	...
	fn prev_funding_tx_sign<L: Deref>(
		...
		let (prev_funding_input_index, pre_channel_value) = if let Some(pending_splice) = &self.pending_splice_post {
			...
			return Err(ChannelError::Warn(format!("Cannot sign splice transaction, channel is not in active splice, channel_id {}", self.context.channel_id)))
		};
		debug_assert!(prev_funding_input_index < transaction.input.len());

		// #SPLICE-SIG
		// the redeem script
		let sig_order_ours_first = self.funding.get_holder_pubkeys().funding_pubkey.serialize() < self.funding.counterparty_funding_pubkey().serialize();
		log_info!(logger, "Pubkeys used for redeem script: {} {} {}", &self.funding.get_holder_pubkeys().funding_pubkey, &self.funding.counterparty_funding_pubkey(), sig_order_ours_first);

		let redeem_script = self.funding.get_funding_redeemscript();
		let holder_signature = self.prev_funding_tx_create_holder_sig(&transaction, prev_funding_input_index, pre_channel_value)?; // , &redeem_script)?;
		let mut holder_sig = holder_signature.serialize_der().to_vec();
		holder_sig.push(EcdsaSighashType::All as u8);
		// counterparty signature
	...
	fn prev_funding_tx_create_holder_sig(&self, transaction: &Transaction, input_index: usize, input_value: u64/*, _redeem_script: &ScriptBuf*/) -> Result<Signature, ChannelError> {
		...
		match &self.context.holder_signer {
			ChannelSignerType::Ecdsa(ecdsa) => {
				ecdsa.sign_splicing_funding_input(&self.funding.channel_transaction_parameters, transaction, input_index, input_value, /*&redeem_script, */&self.context.secp_ctx)
					.map_err(|_e| {
						let msg = "Failed to sign the previous funding input in the new splicing funding tx";
						ChannelError::Close((msg.to_owned(), ClosureReason::ProcessingError { err: msg.to_owned() }))
					})
...
impl<SP: Deref> ChannelContext<SP> where SP::Target: SignerProvider {
		...
		where L::Target: Logger
	{
		...
		let anchors_val = if funding.get_channel_type().supports_anchors_zero_fee_htlc_tx() { ANCHOR_OUTPUT_VALUE_SATOSHI * 2 } else { 0 };
	...
	fn get_initial_commitment_signed<L: Deref>(
		&mut self, funding: &FundingScope, is_splice: bool, logger: &L
	) -> Result<msgs::CommitmentSigned, ChannelError>
	where
		SP::Target: SignerProvider,
		L::Target: Logger
	{
		// TODO reset state?, put back check always
		if !is_splice {
			if !matches!(
				self.channel_state, ChannelState::NegotiatingFunding(flags)
	...
	<SP::Target as SignerProvider>::EcdsaSigner: EcdsaChannelSigner
{
	...
	where F::Target: FeeEstimator, L::Target: Logger
	{
		...
		let buffer_fee_msat = commit_tx_fee_sat(feerate_per_kw, commitment_data.stats.tx.htlcs().len() + htlc_stats.on_holder_tx_outbound_holding_cell_htlcs_count as usize + CONCURRENT_INBOUND_HTLC_FEE_BUFFER as usize, self.funding.get_channel_type()) * 1000;
	...
	pub fn splice_channel(&mut self, our_funding_contribution_satoshis: i64,
		...
		self.pending_splice_pre = Some(PendingSplicePre {
	...
	pub fn splice_init_checks(&mut self, msg: &msgs::SpliceInit) -> Result<(), ChannelError> {
		...
		let _post_channel_value = PendingSplicePre::compute_post_value(pre_channel_value, their_funding_contribution_satoshis, our_funding_contribution_satoshis);
		let _post_balance = PendingSplicePre::add_checked(self.funding.value_to_self_msat, our_funding_contribution_satoshis);
	...
	) -> Result<(PendingSplicePost, FundingScope, DualFundingChannelContext, UnfundedChannelContext), ChannelError>
	{
		...
		let pre_funding_transaction = self.funding.funding_transaction.clone();
		let pre_funding_txo = self.funding.get_funding_txo().clone();
	...
	#[cfg(splicing)]
	fn splice_ack_checks(&mut self) -> Result<PendingSplicePre, ChannelError> {
		// check if splice is pending
		if let Some(pending_splice) = &self.pending_splice_pre {
			Ok(pending_splice.clone())
		} else {
			Err(ChannelError::Warn(format!("Channel is not in pending splice")))
		}
	}

	/// See also [`splice_ack_checks`]
	#[cfg(splicing)]
	fn splice_ack(
		&mut self, msg: &msgs::SpliceAck,
	) -> Result<(PendingSplicePost, FundingScope, DualFundingChannelContext, UnfundedChannelContext, i64), ChannelError>
	{
		...
		let _post_balance = PendingSplicePre::add_checked(self.funding.value_to_self_msat, our_funding_contribution);
...
mod tests {
	...
	fn get_pre_and_post(pre_channel_value: u64, our_funding_contribution: i64, their_funding_contribution: i64) -> (u64, u64) {
		use crate::ln::channel::PendingSplicePre;
