...
{ // impl Channel
	...
	pub fn signer_maybe_unblocked<L: Deref>(
		...
		match &mut self.phase {
			...
			#[cfg(splicing)]
			ChannelPhase::RefundingV2(chan) => Some(chan.pre_funded.signer_maybe_unblocked(logger)),
		}
	}
	...
	pub fn peer_disconnected_is_resumable<L: Deref>(&mut self, logger: &L) -> bool where L::Target: Logger {
		match &mut self.phase {
			...
			#[cfg(splicing)]
			ChannelPhase::RefundingV2(chan) => chan.pre_funded.remove_uncommitted_htlcs_and_mark_paused(logger).is_ok(),
		}
	}
	...
	pub fn peer_connected_get_handshake<L: Deref>(
		...
		match &mut self.phase {
			...
			ChannelPhase::RefundingV2(chan) =>
				ReconnectionMsg::Reestablish(chan.pre_funded.get_channel_reestablish(logger)),
		}
	}
	...
	#[cfg(splicing)]
	fn phase_to_splice(&mut self, post_chan: PendingV2Channel<SP>) -> Result<(), ChannelError>
	{
		let phase = core::mem::replace(&mut self.phase, ChannelPhase::Undefined);
		let result = if let ChannelPhase::Funded(prev_chan) = phase {
			self.phase = ChannelPhase::RefundingV2(SplicingChannel::new(prev_chan, post_chan));
			Ok(())
		} else {
			// revert phase
			self.phase = phase;
		...
		L::Target: Logger
	{
		...
		if let ChannelPhase::Funded(prev_chan) = &mut self.phase {
			let _res = prev_chan.splice_init_checks(msg)?;
		...
		L::Target: Logger
	{
		// Explicit check for Funded, not as_funded; RefundingV2 not allowed
		if let ChannelPhase::Funded(prev_chan) = &mut self.phase {
			let pending_splice = prev_chan.splice_ack_checks()?;
			...
			let post_chan = PendingV2Channel::new_spliced(
				...
				msg.funding_contribution_satoshis,
				pending_splice.our_funding_inputs.clone(),
				LockTime::from_consensus(pending_splice.locktime),
				pending_splice.funding_feerate_per_kw,
				logger,
			)?;
...
impl PendingSplicePost {
	pub(crate) fn new(
		pre_channel_value: u64, _our_funding_contribution: i64, _their_funding_contribution: i64,
...
impl<SP: Deref> ChannelContext<SP> where SP::Target: SignerProvider {
	...
	fn new_for_splice<L: Deref>(
		...
		pre_splice_funding: &FundingScope,
		is_outgoing: bool,
		...
		holder_signer: <SP::Target as SignerProvider>::EcdsaSigner,
		logger: &L,
	) -> Result<(ChannelContext<SP>, FundingScope), ChannelError> where L::Target: Logger
	{
		let pre_channel_value = pre_splice_funding.get_value_satoshis();
		let post_channel_value = PendingSplicePre::compute_post_value(pre_channel_value, our_funding_contribution, their_funding_contribution);

		// Compute our new balance
		let old_to_self = pre_splice_funding.value_to_self_msat;
		...
		if delta_in_value_to_self < 0 && delta_in_value_to_self.abs() as u64 > old_to_self {
			...
			return Err(ChannelError::Warn(format!("Cannot decrease channel value to requested amount, too low, {} {} {} {} {}",
		...
		let value_to_cp = post_channel_value.saturating_sub(value_to_self_msat);
		let mut post_channel_transaction_parameters = pre_splice_funding.channel_transaction_parameters.clone();
		post_channel_transaction_parameters.channel_value_satoshis = post_channel_value;
		post_channel_transaction_parameters.funding_outpoint = None;
		let funding = FundingScope {
			value_to_self_msat: value_to_self_msat,

			counterparty_selected_channel_reserve_satoshis: pre_splice_funding.counterparty_selected_channel_reserve_satoshis,
			holder_selected_channel_reserve_satoshis: pre_splice_funding.holder_selected_channel_reserve_satoshis,
			#[cfg(debug_assertions)]
			holder_max_commitment_tx_output: Mutex::new((value_to_self_msat, value_to_cp)),
			#[cfg(debug_assertions)]
			counterparty_max_commitment_tx_output: Mutex::new((value_to_self_msat, value_to_cp)),
			#[cfg(any(test, fuzzing))]
			next_local_commitment_tx_fee_info_cached: Mutex::new(None),
			#[cfg(any(test, fuzzing))]
			next_remote_commitment_tx_fee_info_cached: Mutex::new(None),
		...
		log_debug!(logger, "Splicing channel context: value {} old {}, dir {}, value to self {}",
			funding.get_value_satoshis(), pre_channel_value,
	...
	#[cfg(splicing)]
	pub(crate) fn splice_start<L: Deref>(&mut self, is_outgoing: bool, channel_value_satoshis: u64, logger: &L) where L::Target: Logger {
		...
		log_info!(logger, "Splicing process started, old channel value {}, outgoing {}, channel_id {}",
			channel_value_satoshis, is_outgoing, self.channel_id);
	}
}
...
{ // impl FundedChannel
	...
	#[cfg(splicing)]
	pub fn splice_ack_checks(&mut self) -> Result<PendingSplicePre, ChannelError> {
...
impl<SP: Deref> PendingV2Channel<SP> where SP::Target: SignerProvider {
	...
	{
		if pre_splice_channel.pending_splice_post.is_some() {
		...
		let pre_funding_transaction = pre_splice_channel.funding.funding_transaction.clone();
		let pre_funding_txo = pre_splice_channel.funding.get_funding_txo().clone();
		...
		let holder_signer = signer_provider.derive_channel_signer(pre_splice_channel.context.channel_keys_id);
	...
	where ES::Target: EntropySource, L::Target: Logger
	{
		...
		// let post_balance = PendingSplicePre::add_checked(self.funding.value_to_self_msat, our_funding_contribution);
		...
		self.context.splice_start(true, pre_channel_value, logger);

		// Start interactive funding negotiation, with the previous funding transaction as an extra shared input
		let tx_msg_opt = self.begin_interactive_funding_tx_construction(signer_provider, entropy_source, holder_node_id.clone(), Some(prev_funding_input))
	...
	where ES::Target: EntropySource, L::Target: Logger
	{
		...
		// Apply start of splice change in the state
		self.context.splice_start(false, self.funding.get_value_satoshis(), logger);
		...
		let _msg = self.begin_interactive_funding_tx_construction(signer_provider, entropy_source, holder_node_id.clone(), None)
