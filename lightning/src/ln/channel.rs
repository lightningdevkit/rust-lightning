...
use bitcoin::{secp256k1, sighash, Sequence, TxIn};
...
use crate::ln::funding::{FundingTemplate, FundingContribution, FundingTxInput, SpliceContribution};
...
impl FundingScope {
	...
	pub fn get_funding_output(&self) -> Option<TxOut> {
		self.channel_transaction_parameters
			...
			.and_then(|redeem_script| Some(TxOut {
	...
	<SP::Target as SignerProvider>::EcdsaSigner: EcdsaChannelSigner,
{
	...
	pub fn funding_contributed<L: Deref>(
		&mut self, contribution: FundingContribution, locktime: u32, logger: &L,
	) -> Result<msgs::SpliceInit, SpliceFundingFailed>
	where
		L::Target: Logger,
	{
		let adjusted_funding_contribution = match contribution.validate() {
			Ok(adjusted_contribution) => adjusted_contribution,
			Err(e) => {
				log_error!(
					logger,
					"Channel {} cannot be {}; {}",
					self.context.channel_id(),
					if contribution.net_value().is_positive() { "spliced in" } else { "spliced out" },
					e
				);

				let (contributed_inputs, contributed_outputs) = contribution.into_contributed_inputs_and_outputs();
		...
		if is_holder_quiescence_initiator {
			match self.quiescent_action.take() {
				...
				Some(QuiescentAction::Splice(contribution)) => {
					...
					let previous_utxo = self.funding.get_funding_output().expect("funding_output should be set");
					...
					let funding_template = FundingTemplate::for_splice(contribution, shared_input);
					return Ok(Some(StfuResponse::FundingNeeded(funding_template)));
				},
				#[cfg(any(test, fuzzing))]
