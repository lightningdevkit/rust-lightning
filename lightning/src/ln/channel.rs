...
impl<SP: Deref> ChannelVariants<SP> where SP::Target: SignerProvider {
	...
	/// Return all the funded channels (mut ref)
	pub fn all_funded_mut(&mut self) -> Vec<&mut Channel<SP>> {
	...
	) -> Result<((Channel<SP>, msgs::CommitmentSigned, Option<Event>), Self), (Self, ChannelError)>
	where
		L::Target: Logger
	{
		if let Some(pending) = self.take_pending() {
			match pending.funding_tx_constructed(counterparty_node_id, signing_session, logger) {
				Ok(r) => Ok((r, self)),
				Err((_ch, err)) => Err((self, err)),
			}
		} else {
...
#[cfg(any(dual_funding, splicing))]
pub(super) trait HasDualFundingChannelContext {
	fn dual_funding_context(&self) -> &DualFundingChannelContext;
	fn dual_funding_context_mut(&mut self) -> &mut DualFundingChannelContext;
}

#[cfg(any(dual_funding, splicing))]
pub(super) trait InteractivelyFunded<SP: Deref>: HasChannelContext<SP> + HasDualFundingChannelContext where SP::Target: SignerProvider {
	...
	fn internal_funding_tx_constructed<L: Deref>(&mut self, counterparty_node_id: &PublicKey,
		signing_session: &mut InteractiveTxSigningSession, logger: &L)
		...
		L::Target: Logger
	{
		...
		let expected_spk = context.get_funding_redeemscript().to_v0_p2wsh();
		let funding_outputs = signing_session.unsigned_tx.find_output_by_script(&expected_spk, Some(context.get_value_satoshis()));
		let funding_outpoint_index = if funding_outputs.len() == 1 {
			funding_outputs[0].0 as u16
		} else if funding_outputs.len() == 0 {
			return Err(ChannelError::Close("No output matched the script_pubkey and value in the FundingGenerationReady event".to_owned()));
		} else { // > 1
			return Err(ChannelError::Close("Multiple outputs matched the expected script and value".to_owned()));
		};
		...
		let commitment_signed = get_initial_commitment_signed(context, signing_session.unsigned_tx.clone(),
			is_splice_pending, logger);
		let commitment_signed = match commitment_signed {
			Ok(commitment_signed) => commitment_signed,
			Err(err) => return Err(ChannelError::Close(err.to_string())),
		};

		let mut partly_signed_transaction = signing_session.unsigned_tx.clone().into_unsigned_tx();
		#[cfg(splicing)]
		if is_splice_pending {
			...
			let (partly_signed_tx, holder_signature) = context.prev_funding_tx_sign(&partly_signed_transaction, None, logger)?;
		...
		if our_funding_satoshis == 0 {
			signing_session.provide_holder_witnesses(context.channel_id, Vec::new(), signing_session.shared_signature.clone());
		} else {
			funding_ready_for_sig_event = Some(Event::FundingTransactionReadyForSigning {
				channel_id: context.channel_id,
				counterparty_node_id: *counterparty_node_id,
				user_channel_id: context.user_id,
				// Note: here the transaction is used for signing, input&output order matters
...
#[cfg(any(dual_funding, splicing))]
impl<SP: Deref> HasDualFundingChannelContext for InboundV2Channel<SP> where SP::Target: SignerProvider {
    fn dual_funding_context(&self) -> &DualFundingChannelContext {
        &self.dual_funding_context
    }
...
impl<SP: Deref> ChannelContext<SP> where SP::Target: SignerProvider  {
	...
	/// __RBF__ Clone a channel context for a new RBF variant
	#[cfg(any(dual_funding, splicing))]
	fn clone_for_rbf(
		prev_context: &Self,
		is_outbound: bool,
		...
		let mut context = Self::clone(prev_context, holder_signer);

		context.reset_common_rbf_fields(funding_feerate_sat_per_1000_weight);
		// reset state
		context.channel_state = if is_outbound {
			ChannelState::NegotiatingFunding(NegotiatingFundingFlags::OUR_INIT_SENT)
		} else {
			ChannelState::NegotiatingFunding(NegotiatingFundingFlags::OUR_INIT_SENT | NegotiatingFundingFlags::THEIR_INIT_SENT)
		};
	...
	{
		if pre_splice_context.is_splice_pending() {
			return Err(ChannelError::Warn(format!("Internal error: Channel is already splicing, channel_id {}", pre_splice_context.channel_id)));
		}

		let pre_channel_value = pre_splice_context.channel_value_satoshis;

		// Save the current funding transaction
		let pre_funding_transaction = pre_splice_context.funding_transaction_saved.clone();
		let pre_funding_txo = pre_splice_context.get_funding_txo().clone();
		...
		// Compute our new balance
		let old_to_self = pre_splice_context.value_to_self_msat;
		...
		log_debug!(logger, "Splicing channel context: value {} old {}, dir {}, value to self {}, funding keys local {} cp {}",
			...
			context.channel_transaction_parameters.holder_pubkeys.funding_pubkey, counterparty_funding_pubkey
		);
	...
	fn do_accept_channel_checks(&mut self, default_limits: &ChannelHandshakeLimits,
		their_features: &InitFeatures, common_fields: &msgs::CommonAcceptChannelFields, channel_reserve_satoshis: u64,
	) -> Result<(), ChannelError> {
	...
	{
		let mut funding_inputs_with_extra = dual_funding_context.our_funding_inputs.clone();
		// #SPLICING
		#[cfg(splicing)]
		...
		if is_initiator {
			// add output
			// #SPLICING Note on splicing: channel value at this point is changed to the post-splice value, so no special action needed
	...
	pub(crate) fn splice_start<L: Deref>(&mut self, is_outgoing: bool, logger: &L) where L::Target: Logger {
		...
		log_info!(logger, "Splicing process started, new channel value {}, outgoing {}, channel_id {}", self.channel_value_satoshis, is_outgoing, self.channel_id);
	}
...
pub(super) fn calculate_our_funding_satoshis(
	...
	for (idx, input) in funding_inputs.iter().enumerate() {
		if let Some(output) = input.1.as_transaction().output.get(input.0.previous_output.vout as usize) {
			total_input_satoshis = total_input_satoshis.saturating_add(output.value);
	...
	if is_initiator {
		our_contributed_weight = our_contributed_weight
			...
			.saturating_add(get_output_weight(&ScriptBuf::new_v0_p2wsh(
				&WScriptHash::from_slice(&[2; 32]).expect("valid scrpt hash")
			)).to_wu())
	}
	...
	<SP::Target as SignerProvider>::EcdsaSigner: WriteableEcdsaChannelSigner
{
	...
	where L::Target: Logger {
		if let Some(ref mut signing_session) = self.interactive_tx_signing_session {
			let expected_witness_count = signing_session.remote_inputs_count();
			...
			if msg.tx_hash != signing_session.unsigned_tx.txid() {
...
impl<SP: Deref> OutboundV2Channel<SP> where SP::Target: SignerProvider {
	...
	/// __RBF__
	/// Create new OutboundV2Channel context for a new outgoing RBF variant
	pub fn new_for_rbf(
		pre_channel: &ChannelContext<SP>,
		signer_provider: &SP,
		funding_feerate_sat_per_1000_weight: u32,
		funding_inputs: Vec<(TxIn, TransactionU16LenLimited)>,
		locktime: u32,
	) -> Result<Self, ChannelError> {
		let dust_limit = pre_channel.holder_dust_limit_satoshis;
		let funding_satoshis = calculate_our_funding_satoshis(
			true, &funding_inputs, funding_feerate_sat_per_1000_weight,
			dust_limit
		).map_err(|_| ChannelError::Close("Failed to open RBF variant, failed to compute funding".to_string()))?;

		let holder_signer = signer_provider.derive_channel_signer(pre_channel.channel_value_satoshis, pre_channel.channel_keys_id);

		let context = ChannelContext::clone_for_rbf(pre_channel, true, holder_signer, funding_feerate_sat_per_1000_weight)?;
	...
	where ES::Target: EntropySource
	{
		self.context.begin_interactive_funding_tx_construction(&self.dual_funding_context,
			signer_provider, entropy_source, holder_node_id, true /* is_initiator */)
	}
	...
	pub fn ack_rbf(&mut self, _msg: &msgs::TxAckRbf) -> Result<(), ChannelError> {
...
impl<SP: Deref> InboundV2Channel<SP> where SP::Target: SignerProvider {
	...
	/// __RBF__ Create new InboundV2Channel context for a new incoming RBF variant
	pub fn new_for_rbf(
		pre_channel: &ChannelContext<SP>,
		signer_provider: &SP,
		msg: &msgs::TxInitRbf,
		...
		let funding_satoshis = calculate_our_funding_satoshis(
			...
			dust_limit
		).map_err(|_| ChannelError::Close("Failed to accept channel".to_string()))?;

		let holder_signer = signer_provider.derive_channel_signer(pre_channel.channel_value_satoshis, pre_channel.channel_keys_id);

		let context = ChannelContext::clone_for_rbf(pre_channel, false, holder_signer, msg.feerate_sat_per_1000_weight)?;
		...
		let chan = Self {
			...
			dual_funding_context: DualFundingChannelContext {
				our_funding_satoshis: funding_satoshis,
				their_funding_satoshis: msg.funding_output_contribution.unwrap_or(0) as u64,
				funding_tx_locktime: LockTime::from_consensus(msg.locktime),
	...
	/// __RBF__
	/// Marks an inbound channel as accepted and generates a [`msgs::TxAckRbf`] message which
	/// should be sent back to the counterparty node.
	///
	/// [`msgs::TxAckRbf`]: crate::ln::msgs::TxAckRbf
	pub fn accept_inbound_dual_funded_rbf_channel(&mut self) -> msgs::TxAckRbf {
		if self.context.is_outbound() {
			debug_assert!(false, "Tried to send tx_ack_rbf for an outbound channel?");
		}
		if !matches!(
			self.context.channel_state, ChannelState::NegotiatingFunding(flags)
			if flags == (NegotiatingFundingFlags::OUR_INIT_SENT | NegotiatingFundingFlags::THEIR_INIT_SENT)
	...
	/// This function is used to explicitly generate a [`msgs::TxAckRbf`] message for an
	/// inbound RBF channel.
	///
	/// [`msgs::TxAckRbf`]: crate::ln::msgs::TxAckRbf
	fn generate_tx_ack_rbf_message(&self) -> msgs::TxAckRbf {
		msgs::TxAckRbf {
			channel_id: self.context.channel_id(),
			funding_output_contribution: Some(self.dual_funding_context.their_funding_satoshis as i64),
		}
	}
	...
	{
		self.context.begin_interactive_funding_tx_construction(&self.dual_funding_context,
			signer_provider, entropy_source, holder_node_id, false /* is_initiator */)
	}
...
fn get_initial_commitment_signed<SP:Deref, L: Deref>(
	context: &mut ChannelContext<SP>, transaction: ConstructedTransaction, is_splice: bool, logger: &L
...
impl<SP: Deref> Writeable for Channel<SP> where SP::Target: SignerProvider {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
		...
		// TODO check BW compatibility; we may go with being non saved; if it is missing, splicing will not be possible
		#[cfg(splicing)]
