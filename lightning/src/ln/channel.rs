...
impl<SP: Deref> ChannelContext<SP> where SP::Target: SignerProvider  {
	...
	fn get_funding_created_signature<L: Deref>(&mut self, logger: &L) -> Result<Signature, ChannelError> where L::Target: Logger {
		...
		match &self.holder_signer {
			...
			ChannelSignerType::Ecdsa(ecdsa) => {
				Ok(ecdsa.sign_counterparty_commitment(&counterparty_initial_commitment_tx, Vec::new(), &self.secp_ctx)
					.map_err(|_| ChannelError::Close("Failed to get signatures for new commitment_signed".to_owned()))?.0)
			}
		}
	}
	...
	<SP::Target as SignerProvider>::Signer: WriteableEcdsaChannelSigner
{
	...
	{
		if !self.context.is_outbound() {
			return Err(ChannelError::Close("Received splice_signed for an inbound channel?".to_owned()));
		}
		if self.context.channel_state & !(ChannelState::MonitorUpdateInProgress as u32) != ChannelState::FundingCreated as u32 {
			return Err(ChannelError::Close("Received splice_signed in strange state!".to_owned()));
		}
		// TODO check
		/*
		...
		*/
		...
		log_info!(logger, "Received splice_signed from peer for channel {}", self.context.channel_id());
	...
	pub fn get_splice_created<L: Deref>(&mut self, splice_transaction: Transaction, splice_txo: OutPoint, splice_prev_funding_input_index: u16, logger: &L) -> Result<msgs::SpliceCreated, ChannelError> where L::Target: Logger {
		...
		*/
		...
		let signature = match self.context.get_funding_created_signature(logger) {
			Err(e) => {
	...
	) -> Result<(msgs::SpliceSigned, ChannelMonitor<<SP::Target as SignerProvider>::Signer>), ChannelError>
	where
		L::Target: Logger
	{
		if self.context.is_outbound() {
		...
		*/
		...
		match &mut self.context.holder_signer {
			ChannelSignerType::Ecdsa(ecdsa) =>
				ecdsa.reprovide_channel_parameters(&self.context.channel_transaction_parameters, self.context.channel_value_satoshis)
		}
		...
		match &self.context.holder_signer {
			ChannelSignerType::Ecdsa(ecdsa) =>
				ecdsa.validate_holder_commitment(&holder_commitment_tx, Vec::new())
					.map_err(|_| ChannelError::Close("Failed to validate our commitment".to_owned()))?
		}
		...
		channel_monitor.provide_latest_counterparty_commitment_tx(counterparty_initial_commitment_tx.trust().txid(), Vec::new(), self.context.cur_counterparty_commitment_transaction_number, self.context.counterparty_cur_commitment_point.unwrap(), logger);
