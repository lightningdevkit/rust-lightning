...
use crate::ln::funding::{
	FundingContribution, FundingTemplate, FundingTxInput, SpliceContribution,
};
	...
	<SP::Target as SignerProvider>::EcdsaSigner: EcdsaChannelSigner,
{
		...
		L::Target: Logger,
	{
		...
		if let Some(quiescent_action) = self.quiescent_action.as_ref() {
			// TODO: If we didn't win quiescence, then we can contribute as an acceptor instead of
			// waiting for the splice to lock.
			if matches!(
				quiescent_action,
	...
	pub fn splice_channel<L: Deref>(
		&mut self, contribution: SpliceContribution, feerate: FeeRate, logger: &L,
		...
		L::Target: Logger,
	{
		...
		if self.quiescent_action.is_some() {
			return Err(APIError::APIMisuseError {
				err: format!(
					"Channel {} cannot be spliced as there is a pending quiescent action",
					self.context.channel_id(),
				),
			});
		}
		...
		Ok(FundingTemplate::for_splice(contribution, shared_input, feerate))
	}
		...
		L::Target: Logger,
	{
			...
			|| self.context.channel_state.is_quiescent()
		{
			log_error!(
				logger,
				"Channel {} isn't expecting any funding contribution",
				self.context.channel_id(),
			);
			...
			// TODO: Make this a DiscardFunding event since a splice wasn't initiated
