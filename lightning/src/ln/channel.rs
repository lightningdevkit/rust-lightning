...
trait InitialRemoteCommitmentReceiver<SP: Deref> where SP::Target: SignerProvider {
	...
	fn check_counterparty_commitment_signature<L: Deref>(&mut self, sig: &Signature, logger: &L) -> Result<CommitmentTransaction, ChannelError> where L::Target: Logger {
	...
	fn initial_commitment_signed<L: Deref>(
		...
		counterparty_txid: Txid, counterparty_commitment_number: u64, best_block: BestBlock,
	...
	<SP::Target as SignerProvider>::EcdsaSigner: EcdsaChannelSigner
{
	...
	where L::Target: Logger
	{
		...
		let channel_monitor = self.initial_commitment_signed(
