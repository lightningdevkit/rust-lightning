...
where
	<SP as SignerProvider>::EcdsaSigner: ChannelSigner,
{
	...
	) -> Result<(&mut FundedChannel<SP>, ChannelMonitor<<SP as SignerProvider>::EcdsaSigner>), ChannelError> {
	...
	fn funding_tx_constructed<L: Logger>(
	...
	) -> Result<(Option<ChannelMonitor<<SP as SignerProvider>::EcdsaSigner>>, Option<ChannelMonitorUpdate>), ChannelError> {
		let phase = core::mem::replace(&mut self.phase, ChannelPhase::Undefined);
		match phase {
			ChannelPhase::UnfundedV2(chan) => {
...
trait InitialRemoteCommitmentReceiver<SP: SignerProvider> {
	...
	) -> Result<(ChannelMonitor<<SP as SignerProvider>::EcdsaSigner>, CommitmentTransaction), ChannelError> {
...
impl<SP: SignerProvider> ChannelContext<SP> {
	...
	fn new_for_outbound_channel<'a, ES: EntropySource, F: FeeEstimator, L: Logger>(
		...
		holder_selected_channel_reserve_satoshis: u64,
		channel_keys_id: [u8; 32],
		holder_signer: <SP as SignerProvider>::EcdsaSigner,
		_logger: L,
...
where
	<SP as SignerProvider>::EcdsaSigner: EcdsaChannelSigner,
{
	pub fn context(&self) -> &ChannelContext<SP> {
		&self.context
	}
	...
	pub fn initial_commitment_signed_v2<L: Logger>(
		...
		logger: &L,
	) -> Result<ChannelMonitor<<SP as SignerProvider>::EcdsaSigner>, ChannelError> {
	...
	pub fn funding_transaction_signed<L: Logger>(
...
impl<SP: SignerProvider> OutboundV1Channel<SP> {
	...
	pub fn funding_signed<L: Logger>(
		...
		(FundedChannel<SP>, ChannelMonitor<<SP as SignerProvider>::EcdsaSigner>),
...
impl<SP: SignerProvider> InboundV1Channel<SP> {
	...
	pub fn funding_created<L: Logger>(
		...
		(
			FundedChannel<SP>,
			Option<msgs::FundingSigned>,
			ChannelMonitor<<SP as SignerProvider>::EcdsaSigner>,
		),
		(Self, ChannelError),
	> {
		if self.funding.is_outbound() {
