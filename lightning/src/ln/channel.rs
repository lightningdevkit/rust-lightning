...
impl<Signer: Sign> Channel<Signer> {
	...
	#[inline]
	fn build_commitment_transaction<L: Deref>(&self, commitment_number: u64, keys: &TxCreationKeys, building_for_counterparty: bool, feerate_per_kw: u32, logger: &L) -> (CommitmentTransaction, usize, Vec<(HTLCOutputInCommitment, Option<&HTLCSource>)>) where L::Target: Logger {
		...
		log_trace!(logger, "Building commitment transaction number {} (really {} xor {}) for {}, generated by {} with fee {}...", commitment_number, (INITIAL_COMMITMENT_NUMBER - commitment_number), get_commitment_transaction_number_obscure_factor(&self.get_holder_pubkeys().payment_point, &self.get_counterparty_pubkeys().payment_point, self.is_outbound()), if !building_for_counterparty { "us" } else { "remote" }, if !building_for_counterparty { "us" } else { "remote" }, feerate_per_kw);
		...
		#[cfg(debug_assertions)]
		{
			...
			debug_assert!(broadcaster_max_commitment_tx_output.1 <= counterparty_value_msat as u64 || counterparty_value_msat / 1000 >= Channel::<Signer>::get_holder_selected_channel_reserve_satoshis(self.channel_value_satoshis) as i64);
	...
	pub fn funding_signed<L: Deref>(&mut self, msg: &msgs::FundingSigned, best_block: BestBlock, logger: &L) -> Result<(ChannelMonitor<Signer>, Transaction), ChannelError> where L::Target: Logger {
		...
		let initial_commitment_tx = self.build_commitment_transaction(self.cur_holder_commitment_transaction_number, &holder_signer, false, self.feerate_per_kw, logger).0;
		{
			let trusted_tx = initial_commitment_tx.trust();
			let initial_commitment_bitcoin_tx = trusted_tx.built_transaction();
			let sighash = initial_commitment_bitcoin_tx.get_sighash_all(&funding_script, self.channel_value_satoshis);
				...
				L::Target: Logger
	{
		...
		let (num_htlcs, mut htlcs_cloned, commitment_tx, commitment_txid) = {
			let commitment_tx = self.build_commitment_transaction(self.cur_holder_commitment_transaction_number, &keys, false, feerate_per_kw, logger);
			let commitment_txid = {
