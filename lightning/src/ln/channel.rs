...
impl<SP: Deref> SplicingChannel<SP> where SP::Target: SignerProvider {
	...
	where ES::Target: EntropySource, L::Target: Logger
	{
		...
		let _post_balance = PendingSplice::add_checked(self.pre_funded.funding.value_to_self_msat, our_funding_contribution);
	...
	<SP::Target as SignerProvider>::EcdsaSigner: EcdsaChannelSigner
{
	...
	#[cfg(splicing)]
	fn splice_ack_checks(&mut self) -> Result<PendingSplice, ChannelError> {
		// check if splice is pending
