...
/// TODO
pub struct DefaultTxBuilder {}

impl TxBuilder for DefaultTxBuilder {
	fn build_commitment_transaction<'a, SP: Deref, L: Deref>(&self, channel_context: &'a ChannelContext<SP>, commitment_number: u64, keys: &TxCreationKeys, local: bool, generated_by_local: bool, logger: &L) -> CommitmentStats<'a>
	where
		SP::Target: SignerProvider,
		L::Target: Logger,
	{
		channel_context.build_commitment_transaction(
			commitment_number,
			keys,
			local,
			generated_by_local,
...
pub struct ChannelContext<SP: Deref> where SP::Target: SignerProvider {
...
impl<SP: Deref> ChannelContext<SP> where SP::Target: SignerProvider  {
	...
	pub(crate) fn get_latest_monitor_update_id(&self) -> u64 {
	...
	pub(crate) fn minimum_depth(&self) -> Option<u32> {
		self.minimum_depth
	}
	...
	pub(crate) fn get_counterparty_selected_contest_delay(&self) -> Option<u16> {
	...
	pub(crate) fn get_value_satoshis(&self) -> u64 {
		self.channel_value_satoshis
	}

	pub(crate) fn get_fee_proportional_millionths(&self) -> u32 {
		self.config.options.forwarding_fee_proportional_millionths
	}

	pub(crate) fn get_cltv_expiry_delta(&self) -> u16 {
	...
	pub(crate) fn get_max_dust_htlc_exposure_msat(&self, limiting_feerate_sat_per_kw: u32) -> u64 {
	...
	pub(crate) fn get_feerate_sat_per_1000_weight(&self) -> u32 {
		self.feerate_per_kw
	}

	pub(crate) fn get_dust_buffer_feerate(&self, outbound_feerate_update: Option<u32>) -> u32 {
	...
	pub(crate) fn get_holder_counterparty_selected_channel_reserve_satoshis(&self) -> (u64, Option<u64>) {
		let context = &self;
	...
	fn get_funding_signed_msg<L: Deref, TB: Deref>(&mut self, tx_builder: &TB, logger: &L) -> (CommitmentTransaction, Option<msgs::FundingSigned>)
	where
		L::Target: Logger,
		TB::Target: TxBuilder,
	{
		let counterparty_keys = self.build_remote_transaction_keys();
		let counterparty_initial_commitment_tx = tx_builder.build_commitment_transaction(&self, self.cur_counterparty_commitment_transaction_number + 1, &counterparty_keys, false, false, logger).tx;
	...
	<SP::Target as SignerProvider>::EcdsaSigner: EcdsaChannelSigner
{
	...
	pub fn get_update_fulfill_htlc_and_commit<L: Deref, TB: Deref>(&mut self, htlc_id: u64, payment_preimage: PaymentPreimage, tx_builder: &TB, logger: &L) -> UpdateFulfillCommitFetch
	where
		L::Target: Logger,
		TB::Target: TxBuilder,
	{
		...
		match self.get_update_fulfill_htlc(htlc_id, payment_preimage, logger) {
			UpdateFulfillFetch::NewClaim { mut monitor_update, htlc_value_msat, msg } => {
				...
				if release_cs_monitor && msg.is_some() {
					let mut additional_update = self.build_commitment_no_status_check(tx_builder, logger);
					...
					if msg.is_some() {
						...
						let update = self.build_commitment_no_status_check(tx_builder, logger);
	...
	pub fn commitment_signed<L: Deref, TB: Deref>(&mut self, msg: &msgs::CommitmentSigned, tx_builder: &TB, logger: &L) -> Result<Option<ChannelMonitorUpdate>, ChannelError>
	where
		L::Target: Logger,
		TB::Target: TxBuilder,
	{
		...
		let commitment_stats = tx_builder.build_commitment_transaction(&self.context, self.context.holder_commitment_point.transaction_number(), &keys, true, false, logger);
		let commitment_txid = {
	...
	pub fn maybe_free_holding_cell_htlcs<F: Deref, L: Deref, TB: Deref>(
		&mut self, fee_estimator: &LowerBoundedFeeEstimator<F>, tx_builder: &TB, logger: &L
		...
		TB::Target: TxBuilder,
	{
		if matches!(self.context.channel_state, ChannelState::ChannelReady(_)) && self.context.channel_state.can_generate_new_commitment() {
			self.free_holding_cell_htlcs(fee_estimator, tx_builder, logger)
		} else { (None, Vec::new()) }
	}

	/// Frees any pending commitment updates in the holding cell, generating the relevant messages
	/// for our counterparty.
	fn free_holding_cell_htlcs<F: Deref, L: Deref, TB: Deref>(
		&mut self, fee_estimator: &LowerBoundedFeeEstimator<F>, tx_builder: &TB, logger: &L
	) -> (Option<ChannelMonitorUpdate>, Vec<(HTLCSource, PaymentHash)>)
	where
		F::Target: FeeEstimator,
		...
		TB::Target: TxBuilder,
	{
		...
		if self.context.holding_cell_htlc_updates.len() != 0 || self.context.holding_cell_update_fee.is_some() {
			...
			let update_fee = if let Some(feerate) = self.context.holding_cell_update_fee.take() {
				self.send_update_fee(feerate, false, fee_estimator, tx_builder, logger)
			} else {
				None
			};
	...
	pub fn revoke_and_ack<F: Deref, L: Deref, TB: Deref>(&mut self, msg: &msgs::RevokeAndACK,
		fee_estimator: &LowerBoundedFeeEstimator<F>, tx_builder: &TB, logger: &L, hold_mon_update: bool,
		...
		TB::Target: TxBuilder,
	{
		...
		match self.free_holding_cell_htlcs(fee_estimator, tx_builder, logger) {
	...
	/// commitment update.
	pub fn queue_update_fee<F: Deref, L: Deref, TB: Deref>(&mut self, feerate_per_kw: u32,
		fee_estimator: &LowerBoundedFeeEstimator<F>, tx_builder: &TB, logger: &L)
	where
		F::Target: FeeEstimator,
		L::Target: Logger,
		TB::Target: TxBuilder,
	{
		let msg_opt = self.send_update_fee(feerate_per_kw, true, fee_estimator, tx_builder, logger);
	...
	fn send_update_fee<F: Deref, L: Deref, TB: Deref>(
		...
		TB::Target: TxBuilder,
	{
		...
		let keys = self.context.build_holder_transaction_keys();
		let commitment_stats = tx_builder.build_commitment_transaction(&self.context, self.context.holder_commitment_point.transaction_number(), &keys, true, true, logger);
	...
	/// to the remote side.
	pub fn monitor_updating_restored<L: Deref, NS: Deref, TB: Deref>(
		&mut self, tx_builder: &TB, logger: &L, node_signer: &NS, chain_hash: ChainHash,
		...
		TB::Target: TxBuilder,
	{
		...
		let commitment_update = if self.context.monitor_pending_commitment_signed {
			self.get_last_commitment_update_for_send(tx_builder, logger).ok()
		} else { None };
	...
	fn get_last_commitment_update_for_send<L: Deref, TB: Deref>(&mut self, tx_builder: &TB, logger: &L) -> Result<msgs::CommitmentUpdate, ()>
	where
		L::Target: Logger,
		TB::Target: TxBuilder,
	{
		...
		let commitment_signed = if let Ok(update) = self.send_commitment_no_state_update(tx_builder, logger).map(|(cu, _)| cu) {
	...
	pub fn channel_reestablish<L: Deref, NS: Deref, TB: Deref>(
		&mut self, msg: &msgs::ChannelReestablish, tx_builder: &TB, logger: &L, node_signer: &NS,
		...
		TB::Target: TxBuilder,
	{
		...
		if msg.next_local_commitment_number == next_counterparty_commitment_number {
			...
			if self.context.channel_state.is_monitor_update_in_progress() {
				...
				let commitment_update = self.get_last_commitment_update_for_send(tx_builder, logger).ok();
	...
	fn build_commitment_no_status_check<L: Deref, TB: Deref>(&mut self, tx_builder: &TB, logger: &L) -> ChannelMonitorUpdate
	where
		L::Target: Logger,
		TB::Target: TxBuilder,
	{
		...
		let (mut htlcs_ref, counterparty_commitment_tx) =
			self.build_commitment_no_state_update(tx_builder, logger);
	...
	fn build_commitment_no_state_update<L: Deref, TB: Deref>(&self, tx_builder: &TB, logger: &L)
		...
		TB::Target: TxBuilder,
	{
		...
		let commitment_stats = tx_builder.build_commitment_transaction(&self.context, self.context.cur_counterparty_commitment_transaction_number, &counterparty_keys, false, true, logger);
	...
	fn send_commitment_no_state_update<L: Deref, TB: Deref>(&self, tx_builder: &TB, logger: &L) -> Result<(msgs::CommitmentSigned, (Txid, Vec<(HTLCOutputInCommitment, Option<&HTLCSource>)>)), ChannelError>
	where
		L::Target: Logger,
		TB::Target: TxBuilder,
	{
	...
	pub fn send_htlc_and_commit<F: Deref, L: Deref, TB: Deref>(
		...
		TB::Target: TxBuilder,
	{
		...
		match send_res? {
			Some(_) => {
				let monitor_update = self.build_commitment_no_status_check(tx_builder, logger);
				self.monitor_updating_paused(false, true, false, Vec::new(), Vec::new(), Vec::new());
...
impl<SP: Deref> OutboundV1Channel<SP> where SP::Target: SignerProvider {
	...
	fn get_funding_created_msg<L: Deref, TB: Deref>(&mut self, tx_builder: &TB, logger: &L) -> Option<msgs::FundingCreated>
	where
		L::Target: Logger,
		TB::Target: TxBuilder,
	{
		let counterparty_keys = self.context.build_remote_transaction_keys();
		let counterparty_initial_commitment_tx = tx_builder.build_commitment_transaction(&self.context, self.context.cur_counterparty_commitment_transaction_number, &counterparty_keys, false, false, logger).tx;
	...
	pub fn get_funding_created<L: Deref, TB: Deref>(&mut self, funding_transaction: Transaction, funding_txo: OutPoint, is_batch_funding: bool, tx_builder: &TB, logger: &L)
		...
		TB::Target: TxBuilder,
	{
		...
		let funding_created = self.get_funding_created_msg(tx_builder, logger);
	...
	pub fn funding_signed<L: Deref, TB: Deref>(
		mut self, msg: &msgs::FundingSigned, best_block: BestBlock, signer_provider: &SP, tx_builder: &TB, logger: &L
		...
		TB::Target: TxBuilder,
	{
		...
		let initial_commitment_tx = tx_builder.build_commitment_transaction(&self.context, self.context.holder_commitment_point.transaction_number(), &holder_signer, true, false, logger).tx;
		{
...
impl<SP: Deref> InboundV1Channel<SP> where SP::Target: SignerProvider {
	...
	fn check_funding_created_signature<L: Deref, TB: Deref>(&mut self, sig: &Signature, tx_builder: &TB, logger: &L) -> Result<CommitmentTransaction, ChannelError>
	where
		L::Target: Logger,
		TB::Target: TxBuilder,
	{
		...
		let keys = self.context.build_holder_transaction_keys();
		let initial_commitment_tx = tx_builder.build_commitment_transaction(&self.context, self.context.holder_commitment_point.transaction_number(), &keys, true, false, logger).tx;
		let trusted_tx = initial_commitment_tx.trust();
		let initial_commitment_bitcoin_tx = trusted_tx.built_transaction();
		let sighash = initial_commitment_bitcoin_tx.get_sighash_all(&funding_script, self.context.channel_value_satoshis);
	...
	pub fn funding_created<L: Deref, TB: Deref>(
		mut self, msg: &msgs::FundingCreated, best_block: BestBlock, signer_provider: &SP, tx_builder: &TB, logger: &L
		...
		TB::Target: TxBuilder,
	{
		...
		let initial_commitment_tx = match self.check_funding_created_signature(&msg.signature, tx_builder, logger) {
			Ok(res) => res,
		...
		let (counterparty_initial_commitment_tx, funding_signed) = self.context.get_funding_signed_msg(tx_builder, logger);
		...
		macro_rules! test_commitment_common {
			( $counterparty_sig_hex: expr, $sig_hex: expr, $tx_hex: expr, $opt_anchors: expr, {
				...
				let (commitment_tx, htlcs): (_, Vec<HTLCOutputInCommitment>) = {
					let mut commitment_stats = tx_builder.build_commitment_transaction(&chan.context, 0xffffffffffff - 42, &keys, true, false, &logger);
