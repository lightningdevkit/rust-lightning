...
use crate::ln::chan_utils::{
	...
	HTLCOutputInCommitment, HolderCommitmentTransaction, BASE_INPUT_WEIGHT,
...
impl<SP: SignerProvider> ChannelContext<SP> {
	...
	fn new_for_outbound_channel<'a, ES: EntropySource, F: FeeEstimator, L: Logger>(
		...
		let local_stats = SpecTxBuilder {}.get_onchain_stats(
			...
			&channel_type,
		).map_err(|()| APIError::APIMisuseError { err: format!("Funding amount ({} sats) can't even pay fee for two anchors on the initial commitment transaction", value_to_self_msat / 1000)})?;
	...
	fn get_available_balances_for_scope<F: FeeEstimator>(
		...
		available_capacity_msat = cmp::min(available_capacity_msat,
			counterparty_channel_constraints.max_htlc_value_in_flight_msat - outbound_htlcs_value_msat);
