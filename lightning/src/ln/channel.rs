...
impl<Signer: ChannelSigner> ChannelContext<Signer> {
		...
		where L::Target: Logger
	{
		...
		log_trace!(logger, "Building commitment transaction number {} (really {} xor {}) for channel {} for {}, generated by {} with fee {}...",
			...
			log_bytes!(self.channel_id.bytes()[..]), if local { "us" } else { "remote" }, if generated_by_local { "us" } else { "remote" }, feerate_per_kw);
...
impl<Signer: WriteableEcdsaChannelSigner> Channel<Signer> {
	...
	fn get_update_fulfill_htlc<L: Deref>(&mut self, htlc_id_arg: u64, payment_preimage_arg: PaymentPreimage, logger: &L) -> UpdateFulfillFetch where L::Target: Logger {
				...
				match htlc.state {
					...
					InboundHTLCState::LocalRemoved(ref reason) => {
						if let &InboundHTLCRemovalReason::Fulfill(_) = reason {
							...
							log_warn!(logger, "Have preimage and want to fulfill HTLC with payment hash {} we already failed against channel {}", log_bytes!(htlc.payment_hash.0), log_bytes!(self.context.channel_id().bytes()[..]));
		...
		if (self.context.channel_state & (ChannelState::AwaitingRemoteRevoke as u32 | ChannelState::PeerDisconnected as u32 | ChannelState::MonitorUpdateInProgress as u32)) != 0 {
			...
			for pending_update in self.context.holding_cell_htlc_updates.iter() {
				match pending_update {
					...
					&HTLCUpdateAwaitingACK::FailHTLC { htlc_id, .. } => {
						if htlc_id_arg == htlc_id {
							log_warn!(logger, "Have preimage and want to fulfill HTLC with pending failure against channel {}", log_bytes!(self.context.channel_id().bytes()[..]));
			...
			log_trace!(logger, "Adding HTLC claim to holding_cell in channel {}! Current state: {}", log_bytes!(self.context.channel_id().bytes()[..]), self.context.channel_state);
...

		{
			...
			log_trace!(logger, "Upgrading HTLC {} to LocalRemoved with a Fulfill in channel {}!", log_bytes!(htlc.payment_hash.0), log_bytes!(self.context.channel_id.bytes()[..]));
	...
	-> Result<Option<msgs::UpdateFailHTLC>, ChannelError> where L::Target: Logger {
		...
		if force_holding_cell {
			...
			log_trace!(logger, "Placing failure for HTLC ID {} in holding cell in channel {}.", htlc_id_arg, log_bytes!(self.context.channel_id().bytes()[..]));
		...
		log_trace!(logger, "Failing HTLC ID {} back with a update_fail_htlc message in channel {}.", htlc_id_arg, log_bytes!(self.context.channel_id().bytes()[..]));
		{
			let htlc = &mut self.context.pending_inbound_htlcs[pending_idx];
		...
		L::Target: Logger
	{
		...
		log_trace!(logger, "Initial counterparty tx for channel {} is: txid {} tx {}",
			log_bytes!(self.context.channel_id().bytes()[..]), counterparty_initial_bitcoin_tx.txid, encode::serialize_hex(&counterparty_initial_bitcoin_tx.transaction));
		...
		log_info!(logger, "Received funding_signed from peer for channel {}", log_bytes!(self.context.channel_id().bytes()[..]));
		...
		L::Target: Logger
	{
		...
		log_info!(logger, "Received channel_ready from peer for channel {}", log_bytes!(self.context.channel_id().bytes()[..]));
		...
		FE::Target: FeeEstimator, L::Target: Logger,
	{
		...
		if !self.context.is_outbound() {
			...
			if pending_remote_value_msat - msg.amount_msat - self.context.holder_selected_channel_reserve_satoshis * 1000 < remote_fee_cost_incl_stuck_buffer_msat {
				...
				log_info!(logger, "Attempting to fail HTLC due to fee spike buffer violation in channel {}. Rebalancing is required.", log_bytes!(self.context.channel_id().bytes()[..]));
				pending_forward_status = create_pending_htlc_status(self, pending_forward_status, 0x1000|7);
			}
		} else {
		...
		where L::Target: Logger
	{
		...
		let commitment_txid = {
			...
			log_trace!(logger, "Checking commitment tx signature {} by key {} against tx {} (sighash {}) with redeemscript {} in channel {}",
				...
				log_bytes!(sighash[..]), encode::serialize_hex(&funding_script), log_bytes!(self.context.channel_id().bytes()[..]));
		...
		for (idx, (htlc, mut source_opt)) in htlcs_cloned.drain(..).enumerate() {
			if let Some(_) = htlc.transaction_output_index {
				...
				log_trace!(logger, "Checking HTLC tx signature {} by key {} against tx {} (sighash {}) with redeemscript {} in channel {}.",
					...
					encode::serialize_hex(&htlc_tx), log_bytes!(htlc_sighash[..]), encode::serialize_hex(&htlc_redeemscript), log_bytes!(self.context.channel_id().bytes()[..]));
		...
		for htlc in self.context.pending_inbound_htlcs.iter_mut() {
			...
			if let Some(forward_info) = new_forward {
				log_trace!(logger, "Updating HTLC {} to AwaitingRemoteRevokeToAnnounce due to commitment_signed in channel {}.",
					log_bytes!(htlc.payment_hash.0), log_bytes!(self.context.channel_id.bytes()[..]));
		...
		if (self.context.channel_state & ChannelState::MonitorUpdateInProgress as u32) != 0 {
			...
			log_debug!(logger, "Received valid commitment_signed from peer in channel {}, updated HTLC state but awaiting a monitor update resolution to reply.",
				log_bytes!(self.context.channel_id.bytes()[..]));
		...
		log_debug!(logger, "Received valid commitment_signed from peer in channel {}, updating HTLC state and responding with{} a revoke_and_ack.",
			log_bytes!(self.context.channel_id().bytes()[..]), if need_commitment_signed { " our own commitment_signed and" } else { "" });
	...
	where F::Target: FeeEstimator, L::Target: Logger
	{
		...
		if self.context.holding_cell_htlc_updates.len() != 0 || self.context.holding_cell_update_fee.is_some() {
			log_trace!(logger, "Freeing holding cell with {} HTLC updates{} in channel {}", self.context.holding_cell_htlc_updates.len(),
				if self.context.holding_cell_update_fee.is_some() { " and a fee update" } else { "" }, log_bytes!(self.context.channel_id().bytes()[..]));
								...
								match e {
									ChannelError::Ignore(ref msg) => {
										log_info!(logger, "Failed to send HTLC with payment_hash {} due to {} in channel {}",
											log_bytes!(payment_hash.0), msg, log_bytes!(self.context.channel_id().bytes()[..]));
			...
			log_debug!(logger, "Freeing holding cell in channel {} resulted in {}{} HTLCs added, {} HTLCs fulfilled, and {} HTLCs failed.",
				log_bytes!(self.context.channel_id().bytes()[..]), if update_fee.is_some() { "a fee update, " } else { "" },
	...
	where F::Target: FeeEstimator, L::Target: Logger,
	{
		...
		log_trace!(logger, "Updating HTLCs on receipt of RAA in channel {}...", log_bytes!(self.context.channel_id().bytes()[..]));
		...
		if (self.context.channel_state & ChannelState::MonitorUpdateInProgress as u32) == ChannelState::MonitorUpdateInProgress as u32 {
			...
			log_debug!(logger, "Received a valid revoke_and_ack for channel {} but awaiting a monitor update resolution to reply.", log_bytes!(self.context.channel_id().bytes()[..]));
			...
			(None, htlcs_to_fail) => {
				if require_commitment {
					...
					log_debug!(logger, "Received a valid revoke_and_ack for channel {}. Responding with a commitment update with {} HTLCs failed.",
						log_bytes!(self.context.channel_id().bytes()[..]), update_fail_htlcs.len() + update_fail_malformed_htlcs.len());
					self.monitor_updating_paused(false, true, false, to_forward_infos, revoked_htlcs, finalized_claimed_htlcs);
					Ok((htlcs_to_fail, self.push_ret_blockable_mon_update(monitor_update)))
				} else {
					log_debug!(logger, "Received a valid revoke_and_ack for channel {} with no reply necessary.", log_bytes!(self.context.channel_id().bytes()[..]));
	...
	pub fn remove_uncommitted_htlcs_and_mark_paused<L: Deref>(&mut self, logger: &L)  where L::Target: Logger {
		...
		log_trace!(logger, "Peer disconnection resulted in {} remote-announced HTLC drops on channel {}", inbound_drop_count, log_bytes!(self.context.channel_id().bytes()[..]));
	}
		...
		NS::Target: NodeSigner
	{
		...
		log_debug!(logger, "Restored monitor updating in channel {} resulting in {}{} commitment update and {} RAA, with {} first",
			log_bytes!(self.context.channel_id().bytes()[..]), if funding_broadcastable.is_some() { "a funding broadcastable, " } else { "" },
	...
	fn get_last_commitment_update<L: Deref>(&self, logger: &L) -> msgs::CommitmentUpdate where L::Target: Logger {
		...
		log_trace!(logger, "Regenerated latest commitment update in channel {} with{} {} update_adds, {} update_fulfills, {} update_fails, and {} update_fail_malformeds",
				log_bytes!(self.context.channel_id().bytes()[..]), if update_fee.is_some() { " update_fee," } else { "" },
		...
		NS::Target: NodeSigner
	{
		...
		if msg.next_remote_commitment_number > 0 {
			...
			if msg.next_remote_commitment_number > INITIAL_COMMITMENT_NUMBER - self.context.cur_holder_commitment_transaction_number {
				macro_rules! log_and_panic {
					($err_msg: expr) => {
						log_error!(logger, $err_msg, log_bytes!(self.context.channel_id.bytes()[..]), log_pubkey!(self.context.counterparty_node_id));
						panic!($err_msg, log_bytes!(self.context.channel_id.bytes()[..]), log_pubkey!(self.context.counterparty_node_id));
					}
				}
		...
		if msg.next_local_commitment_number == next_counterparty_commitment_number {
			if required_revoke.is_some() {
				log_debug!(logger, "Reconnected channel {} with only lost outbound RAA", log_bytes!(self.context.channel_id().bytes()[..]));
			} else {
				log_debug!(logger, "Reconnected channel {} with no loss", log_bytes!(self.context.channel_id().bytes()[..]));
			}
			...
			if required_revoke.is_some() {
				log_debug!(logger, "Reconnected channel {} with lost outbound RAA and lost remote commitment tx", log_bytes!(self.context.channel_id().bytes()[..]));
			} else {
				log_debug!(logger, "Reconnected channel {} with only lost remote commitment tx", log_bytes!(self.context.channel_id().bytes()[..]));
			}
			...
			for &(index_in_block, tx) in txdata.iter() {
				...
				if self.context.funding_tx_confirmation_height == 0 {
					...
					if let Some(channel_ready) = self.check_get_channel_ready(height) {
						log_info!(logger, "Sending a channel_ready to our peer for channel {}", log_bytes!(self.context.channel_id.bytes()[..]));
				...
				for inp in tx.input.iter() {
					if inp.previous_output == funding_txo.into_bitcoin_outpoint() {
						log_info!(logger, "Detected channel-closing tx {} spending {}:{}, closing channel {}", tx.txid(), inp.previous_output.txid, inp.previous_output.vout, log_bytes!(self.context.channel_id().bytes()[..]));
		...
		L::Target: Logger
	{
		...
		if non_shutdown_state >= ChannelState::ChannelReady as u32 ||
		   (non_shutdown_state & ChannelState::OurChannelReady as u32) == ChannelState::OurChannelReady as u32 {
			...
			log_info!(logger, "Closing channel {} due to funding timeout", log_bytes!(self.context.channel_id.bytes()[..]));
		...
		L::Target: Logger
	{
		...
		log_trace!(logger, "Creating an announcement_signatures message for channel {}", log_bytes!(self.context.channel_id().bytes()[..]));
	...
	pub fn get_channel_reestablish<L: Deref>(&mut self, logger: &L) -> msgs::ChannelReestablish where L::Target: Logger {
		...
		let remote_last_secret = if self.context.cur_counterparty_commitment_transaction_number + 1 < INITIAL_COMMITMENT_NUMBER {
			...
			log_trace!(logger, "Enough info to generate a Data Loss Protect with per_commitment_secret {} for channel {}", log_bytes!(remote_last_secret), log_bytes!(self.context.channel_id().bytes()[..]));
			remote_last_secret
		} else {
			log_info!(logger, "Sending a data_loss_protect with no previous remote per_commitment_secret for channel {}", log_bytes!(self.context.channel_id().bytes()[..]));
			[0;32]
		};
		self.mark_awaiting_response();
	...
	fn send_commitment_no_state_update<L: Deref>(&self, logger: &L) -> Result<(msgs::CommitmentSigned, (Txid, Vec<(HTLCOutputInCommitment, Option<&HTLCSource>)>)), ChannelError> where L::Target: Logger {
...

		{
			...
			log_trace!(logger, "Signed remote commitment tx {} (txid {}) with redeemscript {} -> {} in channel {}",
				...
				log_bytes!(signature.serialize_compact()[..]), log_bytes!(self.context.channel_id().bytes()[..]));
			...
			for (ref htlc_sig, ref htlc) in htlc_signatures.iter().zip(htlcs) {
				log_trace!(logger, "Signed remote HTLC tx {} with redeemscript {} with pubkey {} -> {} in channel {}",
					...
					log_bytes!(htlc_sig.serialize_compact()[..]), log_bytes!(self.context.channel_id().bytes()[..]));
			}
		}
...
impl<Signer: WriteableEcdsaChannelSigner> InboundV1Channel<Signer> {
	...
	fn funding_created_signature<L: Deref>(&mut self, sig: &Signature, logger: &L) -> Result<(Txid, CommitmentTransaction, Signature), ChannelError> where L::Target: Logger {
		...
		let initial_commitment_tx = self.context.build_commitment_transaction(self.context.cur_holder_commitment_transaction_number, &keys, true, false, logger).tx;
		{
			...
			log_trace!(logger, "Checking funding_created tx signature {} by key {} against tx {} (sighash {}) with redeemscript {} for channel {}.",
				...
				encode::serialize_hex(&funding_script), log_bytes!(self.context.channel_id().bytes()[..]));
		...
		L::Target: Logger
	{
		...
		log_info!(logger, "Generated funding_signed for peer for channel {}", log_bytes!(self.context.channel_id().bytes()[..]));
