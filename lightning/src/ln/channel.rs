...
impl<ChanSigner: ChannelKeys> Channel<ChanSigner> {
	...
	fn build_commitment_transaction(&self, commitment_number: u64, keys: &TxCreationKeys, local: bool, generated_by_local: bool, feerate_per_kw: u64) -> (Transaction, usize, Vec<(HTLCOutputInCommitment, Option<&HTLCSource>)>) {
		...
		macro_rules! get_htlc_in_commitment {
			($htlc: expr, $offered: expr, $value: expr, $timelock: expr, $hash: expr) => {
				HTLCOutputInCommitment {
					offered: $offered,
					amount_msat: $value,
					cltv_expiry: $timelock,
					payment_hash: $hash,
		...
		macro_rules! add_htlc_output {
			($htlc: expr, $outbound: expr, $source: expr, $state_name: expr, $value: expr, $timelock: expr, $hash: expr) => {
				if $outbound == local { // "offered HTLC output"
					let htlc_in_tx = get_htlc_in_commitment!($htlc, true, $value, $timelock, $hash);
					if $value / 1000 >= dust_limit_satoshis + (feerate_per_kw * HTLC_TIMEOUT_TX_WEIGHT / 1000) {
					...
					let htlc_in_tx = get_htlc_in_commitment!($htlc, false, $value, $timelock, $hash);
					if $value / 1000 >= dust_limit_satoshis + (feerate_per_kw * HTLC_SUCCESS_TX_WEIGHT / 1000) {
						//log_trace!(self, "   ...including {} {} HTLC {} (hash {}) with value {}", if $outbound { "outbound" } else { "inbound" }, $state_name, $htlc.htlc_id, log_bytes!($htlc.payment_hash.0), $htlc.amount_msat);
		...
		for ref htlc in self.pending_inbound_htlcs.iter() {
			...
			if include {
				add_htlc_output!(htlc, false, None, state_name, htlc.value(), htlc.timelock(), htlc.hash());
				remote_htlc_total_msat += htlc.value();
			} else {
	...
	fn get_update_fulfill_htlc(&mut self, htlc_id_arg: u64, payment_preimage_arg: PaymentPreimage) -> Result<(Option<msgs::UpdateFulfillHTLC>, Option<ChannelMonitorUpdate>), ChannelError> {
...

		{
			...
			htlc.update_state(InboundHTLCState::LocalRemoved(InboundHTLCRemovalReason::Fulfill(payment_preimage_arg.clone())));
		}
	...
	pub fn get_update_fail_htlc(&mut self, htlc_id_arg: u64, err_packet: msgs::OnionErrorPacket) -> Result<Option<msgs::UpdateFailHTLC>, ChannelError> {
		...
		{
			let htlc = &mut self.pending_inbound_htlcs[pending_idx];
			htlc.update_state(InboundHTLCState::LocalRemoved(InboundHTLCRemovalReason::FailRelay(err_packet.clone())));
		}
	...
	fn get_inbound_pending_htlc_stats(&self) -> (u32, u64) {
		...
		for ref htlc in self.pending_inbound_htlcs.iter() {
			htlc_inbound_value_msat += htlc.value();
		}
	...
	pub fn update_add_htlc(&mut self, msg: &msgs::UpdateAddHTLC, pending_forward_state: PendingHTLCStatus) -> Result<(), ChannelError> {
		...
		self.pending_inbound_htlcs.push(Box::new(InboundHTLCOutput {
			htlc_id: msg.htlc_id,
	...
	pub fn commitment_signed<F: Deref>(&mut self, msg: &msgs::CommitmentSigned, fee_estimator: &F) -> Result<(msgs::RevokeAndACK, Option<msgs::CommitmentSigned>, Option<msgs::ClosingSigned>, ChannelMonitorUpdate), (Option<ChannelMonitorUpdate>, ChannelError)> where F::Target: FeeEstimator {
		...
		for htlc in self.pending_inbound_htlcs.iter_mut() {
			...
			if let Some(forward_info) = new_forward {
				htlc.update_state(InboundHTLCState::AwaitingRemoteRevokeToAnnounce(forward_info));
				need_our_commitment = true;
			}
		}
		for htlc in self.pending_outbound_htlcs.iter_mut() {
		...
		where F::Target: FeeEstimator
	{
...

		{
			...
			for htlc in pending_inbound_htlcs.iter_mut() {
				...
				if swap {
					let state = InboundHTLCState::Committed;
					htlc.update_state(InboundHTLCState::Committed);

					if let InboundHTLCState::AwaitingRemoteRevokeToAnnounce(forward_info) = state {
						htlc.update_state(InboundHTLCState::AwaitingAnnouncedRemoteRevoke(forward_info));
							...
							PendingHTLCStatus::Fail(fail_msg) => {
								...
								match fail_msg {
									HTLCFailureMsg::Relay(msg) => {
										htlc.update_state(InboundHTLCState::LocalRemoved(InboundHTLCRemovalReason::FailRelay(msg.reason.clone())));
										update_fail_htlcs.push(msg)
									},
									HTLCFailureMsg::Malformed(msg) => {
										htlc.update_state(InboundHTLCState::LocalRemoved(InboundHTLCRemovalReason::FailMalformed((msg.sha256_of_onion, msg.failure_code))));
