...
use crate::ln::msgs;
use crate::ln::msgs::{
	ChannelMessageHandler, LightningError, OnionMessageHandler, RoutingMessageHandler,
	SocketAddress,
};
...
impl RoutingMessageHandler for IgnoringMessageHandler {
	fn handle_node_announcement(
		&self, _msg: &msgs::NodeAnnouncement,
	) -> Result<bool, LightningError> {
		Ok(false)
	}
	fn handle_channel_announcement(
		&self, _msg: &msgs::ChannelAnnouncement,
	) -> Result<bool, LightningError> {
		Ok(false)
	}
...
impl<
		...
		CMH: Deref,
		NS: Deref,
	> APeerManager for PeerManager<Descriptor, CM, RM, OM, L, CMH, NS>
where
	CM::Target: ChannelMessageHandler,
	RM::Target: RoutingMessageHandler,
	OM::Target: OnionMessageHandler,
...
impl<Descriptor: SocketDescriptor, CM: Deref, OM: Deref, L: Deref, NS: Deref>
...
impl core::fmt::Display for OptionalFromDebugger<'_> {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> Result<(), core::fmt::Error> {
		if let Some((node_id, _)) = self.0 {
			write!(f, " from {}", log_pubkey!(node_id))
		} else {
			Ok(())
		}
	}
	...
	NS::Target: NodeSigner,
{
		...
		while !peer.awaiting_write_event {
			if peer.should_buffer_onion_message() {
				if let Some((peer_node_id, _)) = peer.their_node_id {
					if let Some(next_onion_message) = self
						.message_handler
						.onion_message_handler
			...
			if peer.should_buffer_gossip_backfill() {
				match peer.sync_status {
					...
					InitSyncTracker::ChannelsSyncing(c) if c == 0xffff_ffff_ffff_ffff => {
						if let Some(msg) =
							self.message_handler.route_handler.get_next_node_announcement(None)
						{
					...
					InitSyncTracker::NodesSyncing(sync_node_id) => {
						if let Some(msg) = self
							.message_handler
							.route_handler
							.get_next_node_announcement(Some(&sync_node_id))
						{
							self.enqueue_message(peer, &msg);
							peer.sync_status = InitSyncTracker::NodesSyncing(msg.contents.node_id);
						} else {
	...
	pub fn read_event(
		...
		match self.do_read_event(peer_descriptor, data) {
			...
			Err(e) => {
				log_trace!(
					self.logger,
					"Disconnecting peer due to a protocol error (usually a duplicate connection)."
				);
						...
						match next_step {
							NextNoiseStep::ActOne => {
								let act_two = try_potential_handleerror!(
									peer,
									peer.channel_encryptor.process_act_one_with_keys(
	...
	fn handle_message(
		&self, peer_mutex: &Mutex<Peer>, mut peer_lock: MutexGuard<Peer>,
		message: wire::Message<
			<<CMH as core::ops::Deref>::Target as wire::CustomMessageReader>::CustomMessage,
		>,
	) -> Result<
		Option<
			wire::Message<
	...
	> {
		...
		if let wire::Message::Init(msg) = message {
			...
			if let Err(()) = self.message_handler.route_handler.peer_connected(
				&their_node_id,
				&msg,
				peer_lock.inbound_connection,
			) {
				log_debug!(
					logger,
			...
			if let Err(()) = self.message_handler.chan_handler.peer_connected(
				&their_node_id,
				&msg,
				peer_lock.inbound_connection,
			) {
			...
			if let Err(()) = self.message_handler.onion_message_handler.peer_connected(
				&their_node_id,
				&msg,
				peer_lock.inbound_connection,
			) {
		...
		match message {
			...
			wire::Message::Custom(custom) => {
				self.message_handler
					...
					.handle_custom_message(custom, &their_node_id)?;
			},
		};
		Ok(should_forward)
	}
		...
		match msg {
			wire::Message::ChannelAnnouncement(ref msg) => {
				...
				for (_, peer_mutex) in peers.iter() {
					...
					let logger =
						WithContext::from(&self.logger, peer.their_node_id.map(|p| p.0), None);
	...
	pub fn process_events(&self) {
		...
		loop {
...

			{
				...
				let mut events_generated =
					self.message_handler.chan_handler.get_and_clear_pending_msg_events();
				events_generated.append(
					&mut self.message_handler.route_handler.get_and_clear_pending_msg_events(),
				);
				...
				for (node_id, msg) in
					self.message_handler.custom_message_handler.get_and_clear_pending_msg()
				{
					if peers_to_disconnect.get(&node_id).is_some() {
						continue;
					}
		...
		match peer_option {
			...
			Some(peer_lock) => {
				...
				if let Some((node_id, _)) = peer.their_node_id {
					log_trace!(
						WithContext::from(&self.logger, Some(node_id), None),
						"Handling disconnection of peer {}",
						log_pubkey!(node_id)
					);
...
mod tests {
	...
	use crate::ln::peer_handler::{
		filter_addresses, CustomMessageHandler, IgnoringMessageHandler, MessageHandler,
				...
				std::thread::spawn(move || {
					...
					while start_time.elapsed() < std::time::Duration::from_secs(1) {
						...
						while start_time.elapsed() < std::time::Duration::from_secs(1) {
							...
							cfgs[0].chan_handler.pending_events.lock().unwrap().push(
								crate::events::MessageSendEvent::SendShutdown {
									node_id: peers[1]
										.node_signer
							...
							cfgs[1].chan_handler.pending_events.lock().unwrap().push(
								crate::events::MessageSendEvent::SendShutdown {
									node_id: peers[0]
										.node_signer
										.get_node_id(Recipient::Node)
	...
	fn test_send_simple_msg() {
		...
		a_chan_handler
			...
			.lock()
			.unwrap()
			.push(events::MessageSendEvent::SendShutdown { node_id: their_id, msg: msg.clone() });
