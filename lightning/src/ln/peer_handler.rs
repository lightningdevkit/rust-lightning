	...
	NS::Target: NodeSigner,
{
	...
	> {
		...
		match unprocessed_message {
			...
			Some(LogicalMessage::CommitmentSignedBatch(channel_id, batch)) => {
				...
				self.message_handler.chan_handler.handle_commitment_signed_batch(
					their_node_id,
					channel_id,
					batch,
				);
	...
	> {
		...
		if let wire::Message::Init(msg) = message {
			...
			if let Err(()) = self.message_handler.custom_message_handler.peer_connected(
				their_node_id,
				&msg,
				peer_lock.inbound_connection,
			) {
		...
		if let wire::Message::CommitmentSigned(msg) = message {
			if let Some(ref batch) = msg.batch {
				...
				if buffer.len() == COMMITMENT_SIGNED_BATCH_LIMIT {
					log_debug!(
						logger,
						"Peer {} sent batched commitment_signed for channel {} exceeding the limit",
						log_pubkey!(their_node_id),
						channel_id
					);
					return Err(PeerHandleError {}.into());
				...
				if buffer.len() >= batch_size {
					let (channel_id, batch) = peer_lock
						.commitment_signed_batch
						.take()
						.expect("batch should have been inserted");
	...
	pub fn process_events(&self) {
		...
		loop {
...

			{
					...
					match event {
						...
						MessageSendEvent::BroadcastChannelAnnouncement { msg, update_msg } => {
							...
							match self
								.message_handler
								.route_handler
								.handle_channel_announcement(None, &msg)
							{
								Ok(_)
								| Err(LightningError {
							...
							if let Some(msg) = update_msg {
								match self
									.message_handler
									.route_handler
									.handle_channel_update(None, &msg)
								{
									Ok(_)
									| Err(LightningError {
										action: msgs::ErrorAction::IgnoreDuplicateGossip,
						...
						MessageSendEvent::BroadcastNodeAnnouncement { msg } => {
							...
							match self
								.message_handler
								.route_handler
								.handle_node_announcement(None, &msg)
							{
								Ok(_)
								| Err(LightningError {
									action: msgs::ErrorAction::IgnoreDuplicateGossip,
				...
				let route_events =
					self.message_handler.route_handler.get_and_clear_pending_msg_events();
				for event in route_events {
					handle_event(event, false);
				}

				let onion_msg_events =
					self.message_handler.onion_message_handler.get_and_clear_pending_msg_events();
					...
					self.message_handler.custom_message_handler.get_and_clear_pending_msg()
				{
					...
					self.enqueue_message(
						&mut *if let Some(peer) = get_peer_for_forwarding!(&node_id) {
							peer
						} else {
							continue;
						},
