...
fn allow_shutdown_while_awaiting_quiescence(local_shutdown: bool) {
	...
	remote_node.node.handle_commitment_signed_batch_test(local_node_id, &update_add.commitment_signed);
	...
	local_node.node.handle_commitment_signed_batch_test(remote_node_id, &update_fail.commitment_signed);
