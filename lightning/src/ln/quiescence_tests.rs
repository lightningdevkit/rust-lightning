...
fn allow_shutdown_while_awaiting_quiescence(local_shutdown: bool) {
	...
	remote_node.node.handle_commitment_signed_batch(local_node_id, &update_add.commitment_signed);
	...
	local_node.node.handle_commitment_signed_batch(remote_node_id, &commit_sig);
	check_added_monitors(local_node, 1);
	...
	local_node.node.handle_commitment_signed_batch(remote_node_id, &update_fail.commitment_signed);
	...
	check_added_monitors(remote_node, 1);
	remote_node.node.handle_commitment_signed_batch(local_node_id, &commit_sig);
	check_added_monitors(remote_node, 1);
...
fn test_quiescence_tracks_monitor_update_in_progress_and_waits_for_async_signer() {
	...
	nodes[0].node.handle_commitment_signed_batch(node_id_1, &update.commitment_signed);
	...
	check_added_monitors(&nodes[1], 1);
	nodes[1].node.handle_commitment_signed_batch(node_id_0, &commit_sig);
	check_added_monitors(&nodes[1], 1);
