...
use crate::ln::outbound_payment::{CustomTlvs, RecipientOnionFields, Retry, RetryableSendFailure};
use crate::prelude::*;
...
fn one_hop_blinded_path_with_custom_tlv() {
	...
	let max_sized_onion = RecipientOnionFields::spontaneous_empty().with_custom_tlvs(
		CustomTlvs::new(vec![(CUSTOM_TLV_TYPE, vec![42; max_custom_tlv_len])]).unwrap(),
	);
	let id = PaymentId(payment_hash.0);
