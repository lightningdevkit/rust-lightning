...
impl<ChanSigner: ChannelKeys> ChannelMonitor<ChanSigner> {
	...
	fn check_spend_remote_transaction(&mut self, tx: &Transaction, height: u32) -> (HashMap<Sha256dHash, Vec<ClaimRequest>>, (Sha256dHash, Vec<TxOut>), Vec<SpendableOutputDescriptor>) {
		...
		macro_rules! ignore_error {
			( $thing : expr ) => {
				match $thing {
					Ok(a) => a,
					Err(_) => return (claim_requests_per_txid, (commitment_txid, watch_outputs), spendable_outputs)
				}
			};
		}
		...
		if commitment_number >= self.get_min_seen_secret() {
			...
			let a_htlc_key = match self.their_htlc_base_key {
				None => return (claim_requests_per_txid, (commitment_txid, watch_outputs), spendable_outputs),
			...
			if !outpoints.is_empty() || per_commitment_option.is_some() { // ie we're confident this is actually ours
				// We're definitely a remote commitment transaction!
				log_trace!(self, "Got broadcast of revoked remote commitment transaction, going to generate general spend tx with {} inputs", outpoints.len());
		...
		if outpoints.len() > 0 { claim_requests_per_txid.insert(commitment_txid, outpoints); }
		(claim_requests_per_txid, (commitment_txid, watch_outputs), spendable_outputs)
	}
	...
	fn check_spend_remote_htlc(&mut self, tx: &Transaction, commitment_number: u64, height: u32) -> HashMap<Sha256dHash, Vec<ClaimRequest>> {
		...
		let secret = if let Some(secret) = self.get_secret(commitment_number) { secret } else { return HashMap::new(); };
		let per_commitment_key = ignore_error!(SecretKey::from_slice(&secret));
		let per_commitment_point = PublicKey::from_secret_key(&self.secp_ctx, &per_commitment_key);
