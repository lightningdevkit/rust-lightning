...
use chain::keysinterface::{ChannelKeys, SpendableOutputDescriptor};
use chain::transaction::OutPoint;
use ln::chan_utils;
use ln::chan_utils::{CounterpartyCommitmentSecrets, HTLCOutputInCommitment, HTLCType, LocalCommitmentTransaction};
use ln::channelmanager::{HTLCSource, PaymentHash, PaymentPreimage};
use ln::msgs::DecodeError;
use ln::onchaintx::OnchainTxHandler;
use util::logger::Logger;
use util::ser::{MaybeReadable, Readable, ReadableArgs, Writeable, Writer, U48};
...
impl Readable for ChannelMonitorUpdate {
	fn read<R: ::std::io::Read>(r: &mut R) -> Result<Self, DecodeError> {
		...
		let mut updates = Vec::with_capacity(cmp::min(
			len as usize,
			MAX_ALLOC_SIZE / ::std::mem::size_of::<ChannelMonitorUpdateStep>(),
		));
		for _ in 0..len {
...
pub trait ManyChannelMonitor<ChanSigner: ChannelKeys>: Send + Sync {
	...
	fn add_monitor(
		&self, funding_txo: OutPoint, monitor: ChannelMonitor<ChanSigner>,
	...
	/// ChannelMonitors via block_connected may result in FUNDS LOSS.
	fn update_monitor(
		&self, funding_txo: OutPoint, monitor: ChannelMonitorUpdate,
	) -> Result<(), ChannelMonitorUpdateErr>;
...
{
	fn block_connected(
		&self, header: &BlockHeader, height: u32, txn_matched: &[&Transaction], _indexes_of_txn_matched: &[u32],
	) {
		let block_hash = header.bitcoin_hash();
		{
			...
			for monitor in monitors.values_mut() {
				let txn_outputs =
					monitor.block_connected(txn_matched, height, &block_hash, &*self.broadcaster, &*self.fee_estimator);
...
impl<Key: Send + cmp::Eq + hash::Hash + 'static, ChanSigner: ChannelKeys, T: Deref, F: Deref>
	SimpleManyChannelMonitor<Key, ChanSigner, T, F>
where
	T::Target: BroadcasterInterface,
	F::Target: FeeEstimator,
{
	...
	pub fn new(
		chain_monitor: Arc<ChainWatchInterface>, broadcaster: T, logger: Arc<Logger>, feeest: F,
	) -> SimpleManyChannelMonitor<Key, ChanSigner, T, F> {
	...
	pub fn add_monitor_by_key(&self, key: Key, monitor: ChannelMonitor<ChanSigner>) -> Result<(), MonitorUpdateError> {
		...
		let entry = match monitors.entry(key) {
			hash_map::Entry::Occupied(_) => {
				return Err(MonitorUpdateError("Channel monitor for given key is already present"))
			}
			hash_map::Entry::Vacant(e) => e,
		};
		log_trace!(
			self,
			"Got new Channel Monitor for channel {}",
			log_bytes!(monitor.funding_info.0.to_channel_id()[..])
		);
		self.chain_monitor.install_watch_tx(&monitor.funding_info.0.txid, &monitor.funding_info.1);
		self.chain_monitor.install_watch_outpoint(
			(monitor.funding_info.0.txid, monitor.funding_info.0.index as u32),
			&monitor.funding_info.1,
		);
...
impl<ChanSigner: ChannelKeys, T: Deref + Sync + Send, F: Deref + Sync + Send> ManyChannelMonitor<ChanSigner>
	for SimpleManyChannelMonitor<OutPoint, ChanSigner, T, F>
where
	T::Target: BroadcasterInterface,
	F::Target: FeeEstimator,
{
...
impl<Key: Send + cmp::Eq + hash::Hash, ChanSigner: ChannelKeys, T: Deref, F: Deref> events::EventsProvider
	for SimpleManyChannelMonitor<Key, ChanSigner, T, F>
where
	T::Target: BroadcasterInterface,
	F::Target: FeeEstimator,
{
...
pub(crate) enum InputMaterial {
	Revoked { witness_script: Script, pubkey: Option<PublicKey>, key: SecretKey, is_htlc: bool, amount: u64 },
	RemoteHTLC { witness_script: Script, key: SecretKey, preimage: Option<PaymentPreimage>, amount: u64, locktime: u32 },
	LocalHTLC { preimage: Option<PaymentPreimage>, amount: u64 },
	Funding { funding_redeemscript: Script },
}
	...
	fn read<R: ::std::io::Read>(reader: &mut R) -> Result<Self, DecodeError> {
		let input_material = match <u8 as Readable>::read(reader)? {
			0 => {
				...
				InputMaterial::Revoked { witness_script, pubkey, key, is_htlc, amount }
			}
			1 => {
				let witness_script = Readable::read(reader)?;
				let key = Readable::read(reader)?;
				...
				InputMaterial::RemoteHTLC { witness_script, key, preimage, amount, locktime }
			}
			2 => {
				let preimage = Readable::read(reader)?;
				let amount = Readable::read(reader)?;
				InputMaterial::LocalHTLC { preimage, amount }
			}
			3 => InputMaterial::Funding { funding_redeemscript: Readable::read(reader)? },
...
impl Writeable for ChannelMonitorUpdateStep {
	fn write<W: Writer>(&self, w: &mut W) -> Result<(), ::std::io::Error> {
		match self {
			...
			&ChannelMonitorUpdateStep::LatestRemoteCommitmentTXInfo {
				ref unsigned_commitment_tx,
				ref htlc_outputs,
				ref commitment_number,
				ref their_revocation_point,
			} => {
				1u8.write(w)?;
...
impl Readable for ChannelMonitorUpdateStep {
	fn read<R: ::std::io::Read>(r: &mut R) -> Result<Self, DecodeError> {
		match Readable::read(r)? {
			0u8 => Ok(ChannelMonitorUpdateStep::LatestLocalCommitmentTXInfo {
			...
			1u8 => Ok(ChannelMonitorUpdateStep::LatestRemoteCommitmentTXInfo {
			...
			2u8 => Ok(ChannelMonitorUpdateStep::PaymentPreimage { payment_preimage: Readable::read(r)? }),
			3u8 => {
				Ok(ChannelMonitorUpdateStep::CommitmentSecret { idx: Readable::read(r)?, secret: Readable::read(r)? })
			}
			4u8 => Ok(ChannelMonitorUpdateStep::RescueRemoteCommitmentTXInfo {
				their_current_per_commitment_point: Readable::read(r)?,
			}),
			5u8 => Ok(ChannelMonitorUpdateStep::ChannelForceClosed { should_broadcast: Readable::read(r)? }),
...
impl<ChanSigner: ChannelKeys> ChannelMonitor<ChanSigner> {
	pub(super) fn new(
		keys: ChanSigner, shutdown_pubkey: &PublicKey, our_to_self_delay: u16, destination_script: &Script,
		funding_info: (OutPoint, Script), their_htlc_base_key: &PublicKey, their_delayed_payment_base_key: &PublicKey,
		their_to_self_delay: u16, funding_redeemscript: Script, channel_value_satoshis: u64,
		commitment_transaction_number_obscure_factor: u64, initial_local_commitment_tx: LocalCommitmentTransaction,
		logger: Arc<Logger>,
	) -> ChannelMonitor<ChanSigner> {
		assert!(commitment_transaction_number_obscure_factor <= (1 << 48));
		let our_channel_close_key_hash = WPubkeyHash::hash(&shutdown_pubkey.serialize());
		let shutdown_script = Builder::new()
			.push_opcode(opcodes::all::OP_PUSHBYTES_0)
			.push_slice(&our_channel_close_key_hash[..])
			.into_script();

		let mut onchain_tx_handler =
			OnchainTxHandler::new(destination_script.clone(), keys.clone(), their_to_self_delay, logger.clone());
		...
		ChannelMonitor {
			...
			current_local_commitment_number: 0xffff_ffff_ffff
				- ((((local_tx_sequence & 0xffffff) << 3 * 8) | (local_tx_locktime as u64 & 0xffffff))
					^ commitment_transaction_number_obscure_factor),
	...
	pub(super) fn provide_latest_remote_commitment_tx_info(
		&mut self, unsigned_commitment_tx: &Transaction,
		htlc_outputs: Vec<(HTLCOutputInCommitment, Option<Box<HTLCSource>>)>, commitment_number: u64,
		their_revocation_point: PublicKey,
	) {
		...
		log_trace!(
			self,
			"Tracking new remote commitment transaction with txid {} at commitment number {} with {} HTLC outputs",
			new_txid,
			commitment_number,
			htlc_outputs.len()
		);
		log_trace!(
			self,
			"New potential remote commitment transaction: {}",
			encode::serialize_hex(unsigned_commitment_tx)
		);
				...
				if old_points.0 == commitment_number + 1 {
					...
					if let Some(old_second_point) = old_points.2 {
						self.their_cur_revocation_points =
							Some((old_points.0 - 1, old_second_point, Some(their_revocation_point)));
					} else {
	...
	pub(super) fn provide_rescue_remote_commitment_tx_info(&mut self, their_revocation_point: PublicKey) {
		if let Ok(payment_key) = chan_utils::derive_public_key(
			&self.secp_ctx,
			&their_revocation_point,
			&self.keys.pubkeys().payment_basepoint,
		) {
			let to_remote_script = Builder::new()
				.push_opcode(opcodes::all::OP_PUSHBYTES_0)
				.push_slice(&WPubkeyHash::hash(&payment_key.serialize())[..])
				.into_script();
			if let Ok(to_remote_key) =
				chan_utils::derive_private_key(&self.secp_ctx, &their_revocation_point, &self.keys.payment_base_key())
			{
	...
	pub(super) fn provide_latest_local_commitment_tx_info(
		&mut self, commitment_tx: LocalCommitmentTransaction,
		htlc_outputs: Vec<(HTLCOutputInCommitment, Option<Signature>, Option<HTLCSource>)>,
		...
		self.current_local_commitment_number = 0xffff_ffff_ffff
			- ((((sequence & 0xffffff) << 3 * 8) | (locktime as u64 & 0xffffff))
		...
		for update in updates.updates.drain(..) {
			match update {
				...
				ChannelMonitorUpdateStep::LatestRemoteCommitmentTXInfo {
					...
					commitment_number,
					their_revocation_point,
				} => self.provide_latest_remote_commitment_tx_info(
					&unsigned_commitment_tx,
					htlc_outputs,
					commitment_number,
					their_revocation_point,
				),
				ChannelMonitorUpdateStep::PaymentPreimage { payment_preimage } => self.provide_payment_preimage(
					&PaymentHash(Sha256::hash(&payment_preimage.0[..]).into_inner()),
					&payment_preimage,
				),
				ChannelMonitorUpdateStep::CommitmentSecret { idx, secret } => self.provide_secret(idx, secret)?,
	...
	pub fn update_monitor<B: Deref>(
		&mut self, mut updates: ChannelMonitorUpdate, broadcaster: &B,
	) -> Result<(), MonitorUpdateError>
	where
		B::Target: BroadcasterInterface,
	{
	...
	fn check_spend_remote_transaction(
		&mut self, tx: &Transaction, height: u32,
	) -> (Vec<ClaimRequest>, (Txid, Vec<TxOut>)) {
		...
		let commitment_number = 0xffffffffffff
			- ((((tx.input[0].sequence as u64 & 0xffffff) << 3 * 8) | (tx.lock_time as u64 & 0xffffff))
				^ self.commitment_transaction_number_obscure_factor);
		if commitment_number >= self.get_min_seen_secret() {
			...
			let b_htlc_key = ignore_error!(chan_utils::derive_public_key(
				&self.secp_ctx,
				&per_commitment_point,
				&self.keys.pubkeys().htlc_basepoint
			));
			let local_payment_key = ignore_error!(chan_utils::derive_private_key(
				&self.secp_ctx,
				&per_commitment_point,
			...
			let delayed_key = ignore_error!(chan_utils::derive_public_key(
				...
				&self.their_delayed_payment_base_key
			));
			let a_htlc_key = ignore_error!(chan_utils::derive_public_key(
				&self.secp_ctx,
				&PublicKey::from_secret_key(&self.secp_ctx, &per_commitment_key),
				&self.their_htlc_base_key
			));

			let revokeable_redeemscript =
				chan_utils::get_revokeable_redeemscript(&revocation_pubkey, self.our_to_self_delay, &delayed_key);
			...
			self.broadcasted_remote_payment_script = {
				...
				let payment_hash160 =
					WPubkeyHash::hash(&PublicKey::from_secret_key(&self.secp_ctx, &local_payment_key).serialize());
				Some((
					Builder::new()
						.push_opcode(opcodes::all::OP_PUSHBYTES_0)
			...
			for (idx, outp) in tx.output.iter().enumerate() {
				if outp.script_pubkey == revokeable_p2wsh {
					let witness_data = InputMaterial::Revoked {
						witness_script: revokeable_redeemscript.clone(),
						pubkey: Some(revocation_pubkey),
						key: revocation_key,
						is_htlc: false,
						amount: outp.value,
					};
					claimable_outpoints.push(ClaimRequest {
						absolute_timelock: height + self.our_to_self_delay as u32,
						aggregable: true,
				...
				for (_, &(ref htlc, _)) in per_commitment_data.iter().enumerate() {
					if let Some(transaction_output_index) = htlc.transaction_output_index {
						...
						if transaction_output_index as usize >= tx.output.len()
							...
							|| tx.output[transaction_output_index as usize].script_pubkey
						...
						let witness_data = InputMaterial::Revoked {
							...
							key: revocation_key,
							is_htlc: true,
							amount: tx.output[transaction_output_index as usize].value,
						};
						claimable_outpoints.push(ClaimRequest {
							absolute_timelock: htlc.cltv_expiry,
							aggregable: true,
							outpoint: BitcoinOutPoint { txid: commitment_txid, vout: transaction_output_index },
							witness_data,
						});
					}
				}
			...
			if !claimable_outpoints.is_empty() || per_commitment_option.is_some() {
				...
				self.remote_commitment_txn_on_chain.insert(
					commitment_txid,
					(commitment_number, tx.output.iter().map(|output| output.script_pubkey.clone()).collect()),
				);
			...
			if let Some(revocation_points) = self.their_cur_revocation_points {
				...
				if let Some(revocation_point) = revocation_point_option {
					...
					let htlc_privkey = ignore_error!(chan_utils::derive_private_key(
						&self.secp_ctx,
						revocation_point,
					...
					self.broadcasted_remote_payment_script = {
						// Note that the Network here is ignored as we immediately drop the address for the
						// script_pubkey version
						let payment_hash160 = WPubkeyHash::hash(
							&PublicKey::from_secret_key(&self.secp_ctx, &local_payment_key).serialize(),
						);
						Some((
							Builder::new()
								.push_opcode(opcodes::all::OP_PUSHBYTES_0)
					...
					for (_, &(ref htlc, _)) in per_commitment_data.iter().enumerate() {
						if let Some(transaction_output_index) = htlc.transaction_output_index {
							...
							if preimage.is_some() || !htlc.offered {
								let witness_data = InputMaterial::RemoteHTLC {
									witness_script: expected_script,
									key: htlc_privkey,
									preimage,
	...
	fn check_spend_remote_htlc(
		&mut self, tx: &Transaction, commitment_number: u64, height: u32,
	) -> (Vec<ClaimRequest>, Option<(Txid, Vec<TxOut>)>) {
		...
		let claimable_outpoints = vec![ClaimRequest {
			absolute_timelock: height + self.our_to_self_delay as u32,
			aggregable: true,
			outpoint: BitcoinOutPoint { txid: htlc_txid, vout: 0 },
			witness_data,
		}];
	...
	fn broadcast_by_local_state(
		&self, commitment_tx: &Transaction, local_tx: &LocalSignedTx,
	) -> (Vec<ClaimRequest>, Vec<TxOut>, Option<(Script, SecretKey, Script)>) {
		...
		let broadcasted_local_revokable_script = if let Ok(local_delayedkey) = chan_utils::derive_private_key(
			&self.secp_ctx,
			&local_tx.per_commitment_point,
		...
		for &(ref htlc, _, _) in local_tx.htlc_outputs.iter() {
			if let Some(transaction_output_index) = htlc.transaction_output_index {
				claim_requests.push(ClaimRequest {
					absolute_timelock: ::std::u32::MAX,
					aggregable: false,
					outpoint: BitcoinOutPoint { txid: local_tx.txid, vout: transaction_output_index as u32 },
	...
	fn check_spend_local_transaction(
		&mut self, tx: &Transaction, height: u32,
	) -> (Vec<ClaimRequest>, (Txid, Vec<TxOut>)) {
		...
		if let Some(commitment_tx) = self.onchain_tx_handler.get_fully_signed_local_tx(&self.funding_redeemscript) {
			...
			for htlc in self.current_local_commitment_tx.htlc_outputs.iter() {
				if let Some(vout) = htlc.0.transaction_output_index {
					...
					if let Some(htlc_tx) =
						self.onchain_tx_handler.get_fully_signed_htlc_tx(&::bitcoin::OutPoint { txid, vout }, &preimage)
					{
						res.push(htlc_tx);
					}
				}
	...
	/// on-chain.
	fn block_connected<B: Deref, F: Deref>(
		&mut self, txn_matched: &[&Transaction], height: u32, block_hash: &BlockHash, broadcaster: B, fee_estimator: F,
	) -> Vec<(Txid, Vec<TxOut>)>
	where
		B::Target: BroadcasterInterface,
		F::Target: FeeEstimator,
	{
			...
			if tx.input.len() == 1 {
				...
				if prevout.txid == self.funding_info.0.txid && prevout.vout == self.funding_info.0.index as u32 {
					...
					if let Some(&(commitment_number, _)) = self.remote_commitment_txn_on_chain.get(&prevout.txid) {
						let (mut new_outpoints, new_outputs_option) =
							self.check_spend_remote_htlc(&tx, commitment_number, height);
						claimable_outpoints.append(&mut new_outpoints);
		...
		if should_broadcast {
			claimable_outpoints.push(ClaimRequest {
				absolute_timelock: height,
				aggregable: false,
				outpoint: BitcoinOutPoint {
					txid: self.funding_info.0.txid.clone(),
					vout: self.funding_info.0.index as u32,
				},
				witness_data: InputMaterial::Funding { funding_redeemscript: self.funding_redeemscript.clone() },
			});
		}
		if should_broadcast {
			if let Some(commitment_tx) = self.onchain_tx_handler.get_fully_signed_local_tx(&self.funding_redeemscript) {
				let (mut new_outpoints, new_outputs, _) =
					self.broadcast_by_local_state(&commitment_tx, &self.current_local_commitment_tx);
			...
			for ev in events {
				match ev {
					...
					OnchainEvent::MaturingOutput { descriptor } => {
						...
						self.pending_events.push(events::Event::SpendableOutputs { outputs: vec![descriptor] });
					}
				}
			}
		}
		self.onchain_tx_handler.block_connected(
			txn_matched,
			claimable_outpoints,
			height,
			&*broadcaster,
		...
		for &(ref txid, ref output_scripts) in watch_outputs.iter() {
			self.outputs_to_watch
				.insert(txid.clone(), output_scripts.iter().map(|o| o.script_pubkey.clone()).collect());
		}

		watch_outputs
	}

	fn block_disconnected<B: Deref, F: Deref>(
		&mut self, height: u32, block_hash: &BlockHash, broadcaster: B, fee_estimator: F,
	) where
		B::Target: BroadcasterInterface,
		F::Target: FeeEstimator,
	{
	...
	fn is_resolving_htlc_output(&mut self, tx: &Transaction, height: u32) {
		'outer_loop: for input in &tx.input {
			...
			let revocation_sig_claim = (input.witness.len() == 3
				&& HTLCType::scriptlen_to_htlctype(input.witness[2].len()) == Some(HTLCType::OfferedHTLC)
				&& input.witness[1].len() == 33)
				|| (input.witness.len() == 3
					&& HTLCType::scriptlen_to_htlctype(input.witness[2].len()) == Some(HTLCType::AcceptedHTLC)
					&& input.witness[1].len() == 33);
			let accepted_preimage_claim = input.witness.len() == 5
				&& HTLCType::scriptlen_to_htlctype(input.witness[4].len()) == Some(HTLCType::AcceptedHTLC);
			...
			if input.previous_output.txid == self.current_local_commitment_tx.txid {
				scan_commitment!(
					self.current_local_commitment_tx.htlc_outputs.iter().map(|&(ref a, _, ref b)| (a, b.as_ref())),
			...
			if let Some(ref prev_local_signed_commitment_tx) = self.prev_local_signed_commitment_tx {
				if input.previous_output.txid == prev_local_signed_commitment_tx.txid {
					scan_commitment!(
						prev_local_signed_commitment_tx.htlc_outputs.iter().map(|&(ref a, _, ref b)| (a, b.as_ref())),
			...
			if let Some(ref htlc_outputs) = self.remote_claimable_outpoints.get(&input.previous_output.txid) {
				scan_commitment!(
					htlc_outputs.iter().map(|&(ref a, ref b)| (a, (b.as_ref().clone()).map(|boxed| &**boxed))),
			...
			if let Some((source, payment_hash)) = payment_data {
				...
				if accepted_preimage_claim {
					...
					match self.onchain_events_waiting_threshold_conf.entry(height + ANTI_REORG_DELAY - 1) {
						hash_map::Entry::Occupied(mut entry) => {
							let e = entry.get_mut();
							e.retain(|ref event| match **event {
								OnchainEvent::HTLCUpdate { ref htlc_update } => return htlc_update.0 != source,
								_ => true,
							});
							e.push(OnchainEvent::HTLCUpdate { htlc_update: (source, payment_hash) });
						}
...
impl<ChanSigner: ChannelKeys + Readable> ReadableArgs<Arc<Logger>> for (BlockHash, ChannelMonitor<ChanSigner>) {
	fn read<R: ::std::io::Read>(reader: &mut R, logger: Arc<Logger>) -> Result<Self, DecodeError> {
		...
		let mut remote_claimable_outpoints =
			HashMap::with_capacity(cmp::min(remote_claimable_outpoints_len as usize, MAX_ALLOC_SIZE / 64));
		for _ in 0..remote_claimable_outpoints_len {
			...
			for _ in 0..htlcs_count {
				htlcs.push((
					...
					<Option<HTLCSource> as Readable>::read(reader)?.map(|o: HTLCSource| Box::new(o)),
				));
			}
		...
		let mut remote_commitment_txn_on_chain =
			HashMap::with_capacity(cmp::min(remote_commitment_txn_on_chain_len as usize, MAX_ALLOC_SIZE / 32));
		...
		let mut remote_hash_commitment_number =
			HashMap::with_capacity(cmp::min(remote_hash_commitment_number_len as usize, MAX_ALLOC_SIZE / 32));
		...
		let mut pending_htlcs_updated =
			Vec::with_capacity(cmp::min(pending_htlcs_updated_len as usize, MAX_ALLOC_SIZE / (32 + 8 * 3)));
		...
		let mut pending_events =
			Vec::with_capacity(cmp::min(pending_events_len as usize, MAX_ALLOC_SIZE / mem::size_of::<events::Event>()));
		...
		let mut onchain_events_waiting_threshold_conf =
			HashMap::with_capacity(cmp::min(waiting_threshold_conf_len as usize, MAX_ALLOC_SIZE / 128));
			...
			for _ in 0..events_len {
				let ev = match <u8 as Readable>::read(reader)? {
					0 => {
						...
						OnchainEvent::HTLCUpdate { htlc_update: (htlc_source, hash) }
					}
					1 => {
		...
		let mut outputs_to_watch = HashMap::with_capacity(cmp::min(
			...
			MAX_ALLOC_SIZE / (mem::size_of::<Txid>() + mem::size_of::<Vec<Script>>()),
		));
		for _ in 0..outputs_to_watch_len {
			let txid = Readable::read(reader)?;
			let outputs_len: u64 = Readable::read(reader)?;
			let mut outputs =
				Vec::with_capacity(cmp::min(outputs_len as usize, MAX_ALLOC_SIZE / mem::size_of::<Script>()));
...
mod tests {
	...
	use ln::onchaintx::{InputDescriptors, OnchainTxHandler};
	...
	fn test_prune_preimages() {
		...
		let mut monitor = ChannelMonitor::new(
			...
			46,
			0,
			LocalCommitmentTransaction::dummy(),
			logger.clone(),
		);
	...
	fn test_claim_txn_weight_computation() {
		...
		assert_eq!(
			base_weight + OnchainTxHandler::<InMemoryChannelKeys>::get_witnesses_weight(&inputs_des[..]),
			claim_tx.get_weight() + /* max_length_sig */ (73 * inputs_des.len() - sum_actual_sigs)
		);
		...
		assert_eq!(
			base_weight + OnchainTxHandler::<InMemoryChannelKeys>::get_witnesses_weight(&inputs_des[..]),
			claim_tx.get_weight() + /* max_length_isg */ (73 * inputs_des.len() - sum_actual_sigs)
		);
	}
