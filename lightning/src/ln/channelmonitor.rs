...
use util::ser::{Readable, MaybeReadable, Writer, Writeable, U48, ReadableArgs};
...
impl ::std::fmt::Display for MonitorUpdateError
{
	fn fmt(&self, f: &mut Formatter<'_>) -> ::std::fmt::Result {
		f.write_str(self.0)
	}
}
impl ::std::error::Error for MonitorUpdateError {}
...
impl<Key: Writeable, ChanSigner, T: Deref, F: Deref, L: Deref, C: Deref> Writeable for SimpleManyChannelMonitor<Key, ChanSigner, T, F, L, C>
		  ...
		  C::Target: ChainWatchInterface,
{
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), Error> {
		...
		(monitors.len() as u64).write(writer)?;
		for (key, m) in monitors.iter() {
			key.write(writer)?;
			m.write_for_disk(writer)?;
		}
		Ok(())
	}
}

/// Arguments for the creation of a SimpleManyChannelMonitor that are not deserialized.
pub struct SimpleManyChannelMonitorReadArgs<T: Deref, F: Deref, L: Deref, C: Deref>
	where T::Target: BroadcasterInterface,
	F::Target: FeeEstimator,
	L::Target: Logger,
	C::Target: ChainWatchInterface,
...
impl<Key : Send + cmp::Eq + hash::Hash + 'static, ChanSigner: ChannelKeys, T: Deref + Sync + Send, F: Deref + Sync + Send, L: Deref + Sync + Send, C: Deref + Sync + Send>
...
impl<Key : Send + cmp::Eq + hash::Hash + 'static, ChanSigner: ChannelKeys, T: Deref + Sync + Send, F: Deref + Sync + Send, L: Deref + Sync + Send, C: Deref + Sync + Send>
SimpleManyChannelMonitor<Key, ChanSigner, T, F, L, C>
	where T::Target: BroadcasterInterface,
		  F::Target: FeeEstimator,
		  L::Target: Logger,
		  C::Target: ChainWatchInterface,
