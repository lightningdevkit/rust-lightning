...
impl ChannelMonitor {
	...
	fn write<W: Writer>(&self, writer: &mut W, for_local_storage: bool) -> Result<(), ::std::io::Error> {
		...
		for (ref ancestor_claim_txid, claim_tx_data) in self.pending_claim_requests.iter() {
			...
			writer.write_all(&byte_utils::be64_to_array(claim_tx_data.feerate_previous))?;
			writer.write_all(&byte_utils::be32_to_array(claim_tx_data.soonest_timelock))?;
			...
			($amount: expr, $old_feerate: expr, $fee_estimator: expr, $predicted_weight: expr) => {
				{
					...
					log_trace!(self, "Getting FuzzEstimator input");
