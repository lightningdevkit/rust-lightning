...
pub enum Event {
	...
	///
	/// The transaction may be unsigned, in which case a further BroadcastFunding event will
	/// be generated to let the client code to sign and broadcast.  This is useful when using
	/// a validating external signer, to ensure that the signed commitment is seen first.
	///
	/// [`ChannelManager`]: crate::ln::channelmanager::ChannelManager
	...
	/// Used to indicate that the client should sign and broadcast the funding transaction.
	/// This transaction was generated by the client in response to `FundingGenerationReady`
	/// but wasn't signed at that time.
	BroadcastFunding {
		/// The funding transaction
		transaction: Transaction,
		/// The random channel_id we picked
		temporary_channel_id: [u8; 32],
...
impl Writeable for Event {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
		match self {
			...
			&Event::BroadcastFunding { ref transaction, ref temporary_channel_id, ref counterparty_node_id } => {
				21u8.write(writer)?;
				write_tlv_fields!(writer, {
					(0, transaction, required),
					(2, temporary_channel_id, required),
					(4, counterparty_node_id.serialize(), required),
				});
			}
...
impl MaybeReadable for Event {
	fn read<R: io::Read>(reader: &mut R) -> Result<Option<Self>, msgs::DecodeError> {
		match Readable::read(reader)? {
			...
			21u8 => {
				let f = || {
					let mut counterparty_node_id = [0; 33];
					...
					Ok(Some(Event::BroadcastFunding {
						transaction,
						temporary_channel_id,
						counterparty_node_id: PublicKey::from_slice(&counterparty_node_id).unwrap(),
					} ))
				};
				f()
			},
