	...
	pub async fn regenerate_and_broadcast_spend_if_necessary(&self) -> Result<(), ()> {
		...
		// Sweep the outputs.
		{
			...
			let respend_descriptors: Vec<&SpendableOutputDescriptor> =
				sweeper_state.outputs.iter().filter(|o| filter_fn(*o, cur_height)).map(|o| &o.descriptor).collect();
			...
			// above.
			let respend_outputs = sweeper_state.outputs.iter_mut().filter(|o| filter_fn(&**o, cur_height));
