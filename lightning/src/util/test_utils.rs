...
pub struct TestChainMonitor<'a> {
	...
	pub persisted_channels: Mutex<HashMap<ChannelId, Option<Vec<u8>>>>,
}
impl<'a> TestChainMonitor<'a> {
	pub fn new(
		...
		Self {
			...
			persisted_channels: Mutex::new(new_hash_map()),
		}
	}
...
impl<'a> chain::Watch<TestChannelSigner> for TestChainMonitor<'a> {
	...
	fn update_channel(
		...
		self.persisted_channels
			.lock()
			.unwrap()
			.insert(channel_id, encoded_channel.map(|data| data.to_vec()));
...
impl<Signer: sign::ecdsa::EcdsaChannelSigner> Persist<Signer> for WatchtowerPersister {
	...
	fn update_persisted_channel(
		...
		let res =
			self.persister.update_persisted_channel(monitor_name, update, encoded_channel, data);
...
impl<Signer: sign::ecdsa::EcdsaChannelSigner> Persist<Signer> for TestPersister {
	...
	fn update_persisted_channel(
		&self, monitor_name: MonitorName, update: Option<&ChannelMonitorUpdate>,
		_encoded_channel: Option<&[u8]>, _data: &ChannelMonitor<Signer>,
	) -> chain::ChannelMonitorUpdateStatus {
