...
pub(crate) struct WatchtowerPersister {
	...
	/// Signed justice transactions keyed by channel and revoked commitment txid.
...
impl<Signer: sign::ecdsa::EcdsaChannelSigner> Persist<Signer> for WatchtowerPersister {
	...
	fn update_persisted_channel(
		...
		if let Some(update) = update {
			if update.updates_watchtower_state() {
				let justice_txs = data.get_pending_justice_txs(
