...
use crate::util::persist::{KVStore, KVStoreSync, MonitorName, PaginatedListResponse};
...
impl crate::util::persist::PaginatedKVStoreSync for TestPaginatedStore {
	fn list_paginated(
		...
		let response_last_key = if page_entries.len() == self.page_size {
			page_entries.last().map(|(k, _)| k.clone())
		} else {
			None
		};
