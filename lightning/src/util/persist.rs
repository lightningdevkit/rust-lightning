...
impl<Signer: ChannelSigner, K: KVStore + ?Sized> Persist<Signer> for K {
	...
	fn persist_new_channel(
		&self, funding_txo: OutPoint, monitor: &ChannelMonitor<Signer>,
...
impl<Signer: ChannelSigner, K: Deref, L: Deref, ES: Deref, SP: Deref, BI: Deref, FE: Deref>
	Persist<Signer> for MonitorUpdatingPersister<K, L, ES, SP, BI, FE>
where
	K::Target: KVStore,
	L::Target: Logger,
	ES::Target: EntropySource + Sized,
...
mod tests {
	...
	fn persist_fn<P: Deref, Signer: ChannelSigner>(_persist: P) -> bool
	where
