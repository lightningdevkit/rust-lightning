...
) -> Result<Vec<(BlockHash, ChannelMonitor<<SP as SignerProvider>::EcdsaSigner>)>, io::Error>
where
	K::Target: KVStoreSync,
{
	let mut res = Vec::new();
	...
	for stored_key in kv_store.list(
		...
		match <Option<(BlockHash, ChannelMonitor<<SP as SignerProvider>::EcdsaSigner>)>>::read(
	...
	K::Target: KVStoreSync,
{
	...
	) -> Result<(BlockHash, ChannelMonitor<<SP as SignerProvider>::EcdsaSigner>), io::Error> {
	...
	> MonitorUpdatingPersisterAsync<K, S, L, ES, SP, BI, FE>
{
	...
	where
		...
		<SP as SignerProvider>::EcdsaSigner: MaybeSend,
	{
		let primary = CHANNEL_MONITOR_PERSISTENCE_PRIMARY_NAMESPACE;
		let secondary = CHANNEL_MONITOR_PERSISTENCE_SECONDARY_NAMESPACE;
		let monitor_list = self.0.kv_store.list(primary, secondary).await?;
	...
	<SP as SignerProvider>::EcdsaSigner: MaybeSend + 'static,
{
	pub(crate) fn spawn_async_persist_new_channel(
		...
		monitor: &ChannelMonitor<<SP as SignerProvider>::EcdsaSigner>, notifier: Arc<Notifier>,
	) {
	...
	> MonitorUpdatingPersisterAsyncInner<K, S, L, ES, SP, BI, FE>
{
	...
	async fn maybe_read_channel_monitor_with_updates(
		&self, monitor_key: &str,
	) -> Result<Option<(BlockHash, ChannelMonitor<<SP as SignerProvider>::EcdsaSigner>)>, io::Error>
	{
