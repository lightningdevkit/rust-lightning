...
mod test {
	...
	fn test_sign() {
		...
		let zbase32_sig = sign(message.as_bytes(), &sk);
	...
	fn test_recover_pk() {
		...
		assert_eq!(pk.unwrap(), PublicKey::from_secret_key(&Secp256k1::signing_only(), &sk))
	}
	...
	fn test_verify() {
        ...
        let sk = SecretKey::from_slice(&ONE).unwrap();
		let sig = sign(message.as_bytes(), &sk).unwrap();
