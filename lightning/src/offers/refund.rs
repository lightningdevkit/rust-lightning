...
impl TryFrom<RefundTlvStream> for RefundContents {
	...
	fn try_from(tlv_stream: RefundTlvStream) -> Result<Self, Self::Error> {
		...
		if issuer_id.is_some() {
			return Err(Bolt12SemanticError::UnexpectedIssuerId);
		}
...
mod tests {
	...
	fn fails_parsing_refund_with_unexpected_fields() {
		...
		let node_id = payer_pubkey();
		let mut tlv_stream = refund.as_tlv_stream();
		tlv_stream.1.issuer_id = Some(&node_id);

		match Refund::try_from(tlv_stream.to_bytes()) {
			Ok(_) => panic!("expected error"),
			Err(e) => {
				assert_eq!(e, Bolt12ParseError::InvalidSemantics(Bolt12SemanticError::UnexpectedIssuerId));
			},
		}
	}
