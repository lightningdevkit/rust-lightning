...
macro_rules! offer_explicit_metadata_builder_methods { (
	...
	/// Creates a new builder for an offer using the [`Offer::issuer_id`] for signing invoices. The
	...
	/// [`ChannelManager`]: crate::ln::channelmanager::ChannelManager
	/// [`ChannelManager::create_offer_builder`]: crate::ln::channelmanager::ChannelManager::create_offer_builder
	pub fn new(issuer_id: PublicKey) -> Self {
		Self {
			offer: OfferContents {
				chains: None, metadata: None, amount: None, description: None,
				features: OfferFeatures::empty(), absolute_expiry: None, issuer: None, paths: None,
				supported_quantity: Quantity::One, issuer_id: Some(issuer_id),
			},
...
macro_rules! offer_derived_metadata_builder_methods { ($secp_context: ty) => {
	/// Similar to [`OfferBuilder::new`] except, if [`OfferBuilder::path`] is called, the issuer id
	/// is derived from the given [`ExpandedKey`] and [`Nonce`]. This provides recipient privacy by
	/// using a different issuer id for each offer. Otherwise, the provided `node_id` is used for
	/// the [`Offer::issuer_id`].
	///
	...
	pub fn deriving_issuer_id(
		...
		Self {
			offer: OfferContents {
				...
				features: OfferFeatures::empty(), absolute_expiry: None, issuer: None, paths: None,
				supported_quantity: Quantity::One, issuer_id: Some(node_id),
			},
			metadata_strategy: core::marker::PhantomData,
...
macro_rules! offer_builder_methods { (
	...
	/// private channels or if [`Offer::issuer_id`] is not a public node id.
	///
	...
	fn build_without_checks($($self_mut)* $self: $self_type) -> Offer {
		if let Some(mut metadata) = $self.offer.metadata.take() {
			...
			if metadata.has_derivation_material() {
				...
				// Don't derive keys if no blinded paths were given since this means the issuer id
				// must be the node id of an announced node.
				...
				// Either replace the issuer id with the derived pubkey or include the metadata for
				// verification. In the former case, the blinded paths must include
				// `OffersContext::InvoiceRequest` instead.
				let (derived_metadata, keys) =
					metadata.derive_from(iv_bytes, tlv_stream, $self.secp_ctx);
				match keys {
					Some(keys) => $self.offer.issuer_id = Some(keys.public_key()),
...
macro_rules! offer_builder_test_methods { (
	...
	#[cfg_attr(c_bindings, allow(dead_code))]
	pub(crate) fn clear_issuer_id($($self_mut)* $self: $self_type) -> $return_type {
		$self.offer.issuer_id = None;
		$return_value
	}
...
impl OfferContents {
	...
	pub(super) fn issuer_id(&self) -> Option<PublicKey> {
		self.issuer_id
	}
	...
	fn verify<T: secp256k1::Signing>(
		...
		match metadata {
			Some(metadata) => {
				...
				let signing_pubkey = match self.issuer_id() {
	...
	pub(super) fn as_tlv_stream(&self) -> OfferTlvStreamRef {
		...
		OfferTlvStreamRef {
			...
			issuer_id: self.issuer_id.as_ref(),
		}
	}
}
...
/// TLV record type for [`Offer::issuer_id`].
...
impl TryFrom<OfferTlvStream> for OfferContents {
	...
	fn try_from(tlv_stream: OfferTlvStream) -> Result<Self, Self::Error> {
		...
		let (issuer_id, paths) = match (issuer_id, paths) {
			(None, None) => return Err(Bolt12SemanticError::MissingIssuerId),
...
mod tests {
	...
	fn builds_offer_with_defaults() {
		...
		assert_eq!(offer.issuer_id(), Some(pubkey(42)));
	...
	fn builds_offer_with_metadata_derived() {
		...
		assert_eq!(offer.issuer_id(), Some(node_id));
	...
	#[test]
	fn builds_offer_with_derived_issuer_id() {
		let node_id = recipient_pubkey();
		let expanded_key = ExpandedKey::new(&KeyMaterial([42; 32]));
		let entropy = FixedEntropy {};
		let nonce = Nonce::from_entropy_source(&entropy);
		...
		assert_ne!(offer.issuer_id(), Some(node_id));
		...
		// Fails verification with altered issuer id
		let mut tlv_stream = offer.as_tlv_stream();
	...
	fn parses_offer_with_issuer_id() {
		...
		match Offer::try_from(encoded_offer) {
			Ok(_) => panic!("expected error"),
			Err(e) => {
				assert_eq!(e, Bolt12ParseError::InvalidSemantics(Bolt12SemanticError::MissingIssuerId));
			},
		}
	}
...
mod bolt12_tests {
	...
	fn fails_parsing_bech32_encoded_offers() {
		...
		assert_eq!(
			"lno1".parse::<Offer>(),
			Err(Bolt12ParseError::InvalidSemantics(Bolt12SemanticError::MissingIssuerId)),
		);
