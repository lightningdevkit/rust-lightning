...
macro_rules! invoice_request_respond_with_explicit_signing_pubkey_methods { (
	...
	/// # Note
	///
	/// If the originating [`Offer`] was created using [`OfferBuilder::deriving_issuer_id`],
	...
	/// [`OfferBuilder::deriving_issuer_id`]: crate::offers::offer::OfferBuilder::deriving_issuer_id
	pub fn respond_with_no_std(
		...
		let signing_pubkey = match $contents.contents.inner.offer.issuer_id() {
			Some(signing_pubkey) => signing_pubkey,
			None => return Err(Bolt12SemanticError::MissingIssuerId),
		};
	...
	pub(super) fn respond_with_no_std_using_signing_pubkey(
		...
		debug_assert!($contents.contents.inner.offer.issuer_id().is_none());
...
macro_rules! invoice_request_respond_with_derived_signing_pubkey_methods { (
	...
	pub fn respond_using_derived_keys_no_std(
		...
		match $contents.contents.inner.offer.issuer_id() {
...
mod tests {
	...
	fn builds_invoice_request_with_defaults() {
		...
		assert_eq!(unsigned_invoice_request.issuer_id(), Some(recipient_pubkey()));
		...
		assert_eq!(invoice_request.issuer_id(), Some(recipient_pubkey()));
