...
#[cfg(async_payments)]
use {
	crate::blinded_path::message::{AsyncPaymentsContext, VerifiedAsyncPaymentContext},
	...
	MR::Target: MessageRouter,
{
	...
	#[cfg(async_payments)]
	pub fn verify_async_payments_context(
		&self, context: AsyncPaymentsContext,
	) -> Result<VerifiedAsyncPaymentContext, ()> {
		match context {
			...
			AsyncPaymentsContext::OutboundPayment { payment_id, hmac, nonce } => {
				...
				Ok(VerifiedAsyncPaymentContext::OutboundPayment(payment_id))
			},
		}
	}
	...
	pub fn create_offer_builder<ES: Deref>(
		&self, entropy_source: ES, absolute_expiry: Option<Duration>, nonce: Option<Nonce>,
		peers: Vec<MessageForwardNode>,
	...
	pub fn enqueue_invoice_request<ES: Deref>(
		&self, entropy_source: ES, invoice_request: InvoiceRequest, payment_id: PaymentId,
		nonce: Option<Nonce>, peers: Vec<MessageForwardNode>,
	...
	pub fn enqueue_invoice<ES: Deref>(
		...
		payment_hash: PaymentHash, peers: Vec<MessageForwardNode>,
	) -> Result<(), Bolt12SemanticError>
	where
		ES::Target: EntropySource,
	{
	...
	#[cfg(async_payments)]
	pub fn enqueue_async_payment_messages<ES: Deref>(
