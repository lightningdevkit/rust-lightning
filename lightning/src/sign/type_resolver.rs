...
pub(crate) enum ChannelSignerType<SP: SignerProvider> {
	...
	Ecdsa(<SP as SignerProvider>::EcdsaSigner),
	#[cfg(taproot)]
	#[allow(unused)]
	Taproot(<SP as SignerProvider>::TaprootSigner),
}
...
impl<SP: SignerProvider> ChannelSignerType<SP> {
	...
	#[allow(unused)]
	pub(crate) fn as_ecdsa(&self) -> Option<&<SP as SignerProvider>::EcdsaSigner> {
		match self {
	...
	#[allow(unused)]
	pub(crate) fn as_mut_ecdsa(&mut self) -> Option<&mut <SP as SignerProvider>::EcdsaSigner> {
		match self {
			ChannelSignerType::Ecdsa(ecs) => Some(ecs),
			_ => None,
		}
