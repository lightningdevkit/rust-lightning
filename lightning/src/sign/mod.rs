...
use crate::ln::channel::{ANCHOR_OUTPUT_VALUE_SATOSHI, ChannelType};
...
impl InMemorySigner {
	...
	pub fn channel_type(&self) -> ChannelType {
		self.get_channel_parameters().channel_type.clone()
	}
...
impl EcdsaChannelSigner for InMemorySigner {
	fn sign_counterparty_commitment(&self, commitment_tx: &CommitmentTransaction, _preimages: Vec<PaymentPreimage>, secp_ctx: &Secp256k1<secp256k1::All>) -> Result<(Signature, Vec<Signature>), ()> {
		...
		for htlc in commitment_tx.htlcs() {
			...
			let htlc_tx = chan_utils::build_htlc_transaction(&commitment_txid, commitment_tx.feerate_per_kw(), self.holder_selected_contest_delay(), htlc, &self.channel_type(), channel_parameters.opt_non_zero_fee_anchors.is_some(), &keys.broadcaster_delayed_payment_key, &keys.revocation_key);
			let htlc_redeemscript = chan_utils::get_htlc_redeemscript(&htlc, &self.channel_type(), &keys);
			let htlc_sighashtype = if self.channel_type().supports_anchors() { EcdsaSighashType::SinglePlusAnyoneCanPay } else { EcdsaSighashType::All };
	...
	fn sign_justice_revoked_htlc(&self, justice_tx: &Transaction, input: usize, amount: u64, per_commitment_key: &SecretKey, htlc: &HTLCOutputInCommitment, secp_ctx: &Secp256k1<secp256k1::All>) -> Result<Signature, ()> {
		...
		let witness_script = {
			...
			chan_utils::get_htlc_redeemscript_with_explicit_keys(&htlc, &self.channel_type(), &counterparty_htlcpubkey, &holder_htlcpubkey, &revocation_pubkey)
		};
		let mut sighash_parts = sighash::SighashCache::new(justice_tx);
		let sighash = hash_to_message!(&sighash_parts.segwit_signature_hash(input, &witness_script, amount, EcdsaSighashType::All).unwrap()[..]);
		return Ok(sign_with_aux_rand(secp_ctx, &sighash, &revocation_key, &self))
	...
	fn sign_counterparty_htlc_transaction(&self, htlc_tx: &Transaction, input: usize, amount: u64, per_commitment_point: &PublicKey, htlc: &HTLCOutputInCommitment, secp_ctx: &Secp256k1<secp256k1::All>) -> Result<Signature, ()> {
		...
		let witness_script = chan_utils::get_htlc_redeemscript_with_explicit_keys(&htlc, &self.channel_type(), &counterparty_htlcpubkey, &htlcpubkey, &revocation_pubkey);
