...
pub trait NodeSigner {
	...
	/// this method. This is because LDK avoids storing inbound payment data by encrypting payment
	/// data in the payment hash and/or payment secret, therefore for a payment to be receivable by
	/// multiple nodes, they must share the key that encrypts this payment data.
	///
	/// Consistent return values across nodes are also essential for verifying signatures and decrypting
	/// payment data in both Offers and spontaneous payments.
	///
