...
pub(super) enum Payload<T: OnionMessageContents> {
	...
	Dummy {
		/// The [`DummyControlTlvs`] were authenticated with the additional key which was
...
impl<T: OnionMessageContents> Writeable for (Payload<T>, [u8; 32]) {
	fn write<W: Writer>(&self, w: &mut W) -> Result<(), io::Error> {
		match &self.0 {
			...
			Payload::Dummy {
				control_tlvs: DummyControlTlvs::Unblinded(control_tlvs),
				control_tlvs_authenticated: _,
			} => {
				let write_adapter = ChaChaPolyWriteAdapter::new(self.1, &control_tlvs);
				_encode_varint_length_prefixed_tlv!(w, { (4, write_adapter, required) })
		...
		match read_adapter {
			...
			Some(ChaChaDualPolyReadAdapter { readable: ControlTlvs::Dummy(tlvs), used_aad }) => {
				Ok(Payload::Dummy {
					...
					control_tlvs: DummyControlTlvs::Unblinded(tlvs),
				})
			},
