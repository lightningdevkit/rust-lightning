...
use crate::blinded_path::message::{AsyncPaymentsContext, BlindedMessagePath, DNSResolverContext, MessageContext, MessageForwardNode, OffersContext, ReceiveTlvs};
...
fn one_blinded_hop() {
	...
	let blinded_path = BlindedMessagePath::new(&[], nodes[1].node_id, receive_tlvs, &*nodes[1].entropy_source, &secp_ctx).unwrap();
...
fn two_unblinded_two_blinded() {
	...
	let blinded_path = BlindedMessagePath::new(&intermediate_nodes, nodes[4].node_id, receive_tlvs, &*nodes[4].entropy_source, &secp_ctx).unwrap();
	let path = OnionMessagePath {
...
fn three_blinded_hops() {
	...
	let blinded_path = BlindedMessagePath::new(&intermediate_nodes, nodes[3].node_id, receive_tlvs, &*nodes[3].entropy_source, &secp_ctx).unwrap();
	let destination = Destination::BlindedPath(blinded_path);
	let instructions = MessageSendInstructions::WithoutReplyPath { destination };
...
fn async_response_over_one_blinded_hop() {
	...
	let reply_path = BlindedMessagePath::new(&[], nodes[1].node_id, receive_tlvs, &*nodes[1].entropy_source, &secp_ctx).unwrap();
...
fn async_response_with_reply_path_succeeds() {
	...
	let reply_path = BlindedMessagePath::new(&[], bob.node_id, receive_tlvs, &*bob.entropy_source, &secp_ctx).unwrap();
...
fn we_are_intro_node() {
	...
	let blinded_path = BlindedMessagePath::new(&intermediate_nodes, nodes[2].node_id, receive_tlvs, &*nodes[2].entropy_source, &secp_ctx).unwrap();
	let destination = Destination::BlindedPath(blinded_path);
	let instructions = MessageSendInstructions::WithoutReplyPath { destination };
	...
	let blinded_path = BlindedMessagePath::new(&intermediate_nodes, nodes[1].node_id, receive_tlvs, &*nodes[1].entropy_source, &secp_ctx).unwrap();
	let destination = Destination::BlindedPath(blinded_path);
	let instructions = MessageSendInstructions::WithoutReplyPath { destination };
...
fn invalid_blinded_path_error() {
	...
	let mut blinded_path = BlindedMessagePath::new(&intermediate_nodes, nodes[2].node_id, receive_tlvs, &*nodes[2].entropy_source, &secp_ctx).unwrap();
...
fn reply_path() {
	...
	let reply_path = BlindedMessagePath::new(&intermediate_nodes, nodes[0].node_id, receive_tlvs, &*nodes[0].entropy_source, &secp_ctx).unwrap();
...
fn requests_peer_connection_for_buffered_messages() {
	...
	let blinded_path = BlindedMessagePath::new(
		&intermediate_nodes, nodes[2].node_id, receive_tlvs, &*nodes[0].entropy_source, &secp_ctx
	).unwrap();
	let destination = Destination::BlindedPath(blinded_path);
	let instructions = MessageSendInstructions::WithoutReplyPath { destination };
...
fn intercept_offline_peer_oms() {
	...
	let blinded_path = BlindedMessagePath::new(
		&intermediate_nodes, nodes[2].node_id, receive_tlvs, &*nodes[2].entropy_source, &secp_ctx
	).unwrap();
	let destination = Destination::BlindedPath(blinded_path);
	let instructions = MessageSendInstructions::WithoutReplyPath { destination };
