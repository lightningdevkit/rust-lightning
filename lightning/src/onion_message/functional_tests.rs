...
impl DNSResolverMessageHandler for TestDNSResolverMessageHandler {
	...
	fn handle_dnssec_proof(&self, _message: DNSSECProof, _context: DNSResolverContext, custom_data: Option<Vec<u8>>) {}
}
	...
	fn handle_custom_message(&self, msg: Self::CustomMessage, context: Option<Vec<u8>>, custom_data: Option<Vec<u8>>, responder: Option<Responder>) -> Option<(Self::CustomMessage, ResponseInstruction)> {
		...
		match responder {
			Some(responder) if expectation.include_reply_path => {
				Some((response, responder.respond_with_reply_path(MessageContext::Custom(context.unwrap_or_else(Vec::new)), custom_data)))
			},
...
fn async_response_over_one_blinded_hop() {
	...
	let response_instruction = nodes[0].custom_message_handler.handle_custom_message(message, None, None, responder);
...
fn async_response_with_reply_path_succeeds() {
	...
	let response_instruction = alice.custom_message_handler.handle_custom_message(message, None, None, Some(responder));
