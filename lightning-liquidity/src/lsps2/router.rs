...
/// offer ids while delegating all other routing behavior to an inner [`Router`].
...
impl<R: Router, ES: EntropySource + Send + Sync> LSPS2BOLT12Router<R, ES> {
	...
	pub fn new(inner_router: R, entropy_source: ES) -> Self {
		Self { inner_router, entropy_source, offer_to_invoice_params: Mutex::new(new_hash_map()) }
	}
