...
mod tests {
	...
	fn test_prune_stale_webhooks() {
		...
		let handler = LSPS5ServiceHandler::new_with_custom_time_provider(
			event_queue,
			pending_messages,
			client_has_open_channel,
			config,
...

		{
			...
			for (i, pubkey) in client_keys.iter().enumerate() {
				...
				for j in 0..2 {
					let app_name = LSPS5AppName::from_string(format!("app_{}_{}", i, j)).unwrap();
					let url = LSPS5WebhookUrl::from_string(format!(
						"https://example.com/webhook_{}_{}",
						i, j
					))
					.unwrap();
					client_webhooks.insert(
						...
						StoredWebhook {
							...
							last_used: LSPSDateTime::new_from_duration_since_epoch(
