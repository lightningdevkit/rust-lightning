...
fn channel_open_failed() {
	...
	let (service_node, client_node) = create_service_and_client_nodes(
		"channel_open_failed_happy_path",
		service_config,
		client_config,
	);
	...
	let open_channel_event = match service_node.liquidity_manager.next_event().unwrap() {
	...
	let reopen_channel_event = match service_node.liquidity_manager.next_event().unwrap() {
		LiquidityEvent::LSPS2Service(LSPS2ServiceEvent::OpenChannel {
			user_channel_id: channel_id,
			intercept_scid: scid,
			..
...
fn channel_open_failed_invalid_state() {
	...
	let (service_node, client_node) = create_service_and_client_nodes(
		"channel_open_failed_invalid_state",
		service_config,
		client_config,
	);
...
fn channel_open_failed_nonexistent_channel() {
	...
	let (service_node, client_node) = create_service_and_client_nodes(
		"channel_open_abandoned_happy_path",
		service_config,
		client_config,
	);
