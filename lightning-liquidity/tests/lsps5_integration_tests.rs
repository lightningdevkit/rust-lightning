...
use lightning_liquidity::lsps5::client::{
	LSPS5ClientConfig, LSPS5ClientHandler, SignatureStorageConfig,
};
...
pub(crate) fn lsps5_test_setup(
	time_provider: Arc<dyn TimeProvider>, max_signatures: Option<usize>,
) -> (
	&'static LSPS5ClientHandler<Arc<KeysManager>, Arc<dyn TimeProvider>>,
	&'static LSPS5ServiceHandler<Arc<ChannelManager>, Arc<dyn TimeProvider>>,
...
fn test_notify_without_webhooks_does_nothing() {
	let (_, service_handler, _, client_node_id, service_node, _) =
		lsps5_test_setup(Arc::new(DefaultTimeProvider), None);
