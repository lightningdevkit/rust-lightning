...
use common::{create_service_and_client_nodes, get_lsps_message, ChannelManager, Node};
...
fn webhook_error_handling_test() {
	...
	let error = result.unwrap_err();
	assert_eq!(error, LSPS5ProtocolError::UrlParse("Error parsing URL".to_string()).into());
	...
	match event {
		LiquidityEvent::LSPS5Client(LSPS5ClientEvent::WebhookRegistrationFailed {
			...
			let error_to_check =
				LSPS5ProtocolError::TooManyWebhooks(DEFAULT_MAX_WEBHOOKS_PER_CLIENT.to_string());
...
fn stale_webhooks() {
	...
	let (client_handler, _, service_node_id, client_node_id, service_node, client_node) =
		lsps5_test_setup(Some(mock_time_provider.clone()));
...
fn test_all_notifications() {
	...
	for expected_method in expected_notifications {
		...
		if let LiquidityEvent::LSPS5Client(LSPS5ClientEvent::WebhookNotificationReceived {
			notification,
			..
		}) = event
		{
...
fn test_timestamp_notification_window_validation() {
	...
	}) = event
	{
		...
		let err_past = client_handler
			...
			.unwrap_err();
		assert!(
			matches!(err_past, LSPS5ClientError::InvalidTimestamp(_)),
		...
		let err_future = client_handler
			...
			.unwrap_err();
		assert!(
			matches!(err_future, LSPS5ClientError::InvalidTimestamp(_)),
			"Expected InvalidTimestamp error variant, got {:?}",
			err_future
		);
	} else {
...
fn test_notify_without_webhooks_does_nothing() {
	let (_, service_handler, _, client_node_id, service_node, _) = lsps5_test_setup(None);
