...
fn test_basic_sign_and_verify() {
	...
	let timestamp =
		LSPSDateTime::new_from_duration(time_provider.duration_since_epoch()).to_rfc3339();
...
fn test_timestamp_window_validation() {
	...
	let valid_timestamp = LSPSDateTime::new_from_duration(current_time).to_rfc3339();
	...
	let past_timestamp = LSPSDateTime::new_from_duration(
	...
	let future_timestamp = LSPSDateTime::new_from_duration(
...
fn test_unknown_method_and_malformed_notifications() {
	...
	let edge_past_timestamp = LSPSDateTime::new_from_duration(
		time_provider.duration_since_epoch().checked_sub(Duration::from_secs(9 * 60)).unwrap(),
	)
	.to_rfc3339();
	let edge_future_timestamp = LSPSDateTime::new_from_duration(
