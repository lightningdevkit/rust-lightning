...
impl FilesystemStore {
	...
	fn write_version(
		...
		self.inner
			.execute_locked_write(inner_lock_ref, dest_file_path.clone(), version, || {
				#[cfg(not(target_os = "windows"))]
				{
...
impl KVStoreSync for FilesystemStore {
	fn read(
		...
		let path = self.get_checked_dest_file_path(
			primary_namespace,
			secondary_namespace,
			Some(key),
			"read",
	...
	fn write(
		...
		self.write_version(inner_lock_ref, path, buf, version)
	}
	...
	fn remove(
		...
		let (inner_lock_ref, version) = self.get_new_version_and_lock_ref(path.clone());
		self.remove_version(inner_lock_ref, path, lazy, version)
	}
	...
	fn list(
		...
		let path =
			self.get_checked_dest_file_path(primary_namespace, secondary_namespace, None, "list")?;
		self.list_impl(path)
	}
}
...
impl KVStore for FilesystemStore {
	fn read(
		...
		async move {
			...
			tokio::task::spawn_blocking(move || {
				let mut buf = Vec::new();
				this.execute_locked_read(path.clone(), || {
	...
	fn write(
		...
		let this = Arc::clone(&self.inner);
		let path_result = self.get_checked_dest_file_path(
			primary_namespace,
			secondary_namespace,
			Some(key),
			"write",
		);
		let version_and_lock = path_result
			.as_ref()
			.ok()
			...
			tokio::task::spawn_blocking(move || {
				this.execute_locked_write(inner_lock_ref, dest_file_path.clone(), version, || {
					...
					if lazy {
						...
						#[cfg(target_os = "windows")]
						{
							remove_file_windows(&this, &dest_file_path)?;
						}
					}
	...
	fn list(
		...
		async move {
			...
			tokio::task::spawn_blocking(move || {
				...
				'retry_list: loop {
					keys = Vec::new();
					'skip_entry: for entry in fs::read_dir(&path)? {
						let entry = entry?;
						let p = entry.path();
						...
						match res {
							Ok(true) => {
								let key = get_key_from_dir_entry_path(&p, &path)?;
								keys.push(key);
							},
							Ok(false) => {
								continue 'skip_entry;
