...
if [ -f "$CACHE_PATH/$ELECTRS_DL_FILE_NAME" ]; then
	cp "$CACHE_PATH/$ELECTRS_DL_FILE_NAME" "$DL_TMP_DIR"/
	if ! echo "$ELECTRS_DL_HASH  $DL_TMP_DIR/$ELECTRS_DL_FILE_NAME"|shasum -a 256 -c; then
		rm -f "$DL_TMP_DIR/$ELECTRS_DL_FILE_NAME"
	fi
fi

if [ -f "$CACHE_PATH/$BITCOIND_DL_FILE_NAME" ]; then
	cp "$CACHE_PATH/$BITCOIND_DL_FILE_NAME" "$DL_TMP_DIR"/
	if ! echo "$BITCOIND_DL_HASH  $DL_TMP_DIR/$BITCOIND_DL_FILE_NAME"|shasum -a 256 -c; then
		rm -f "$DL_TMP_DIR/$BITCOIND_DL_FILE_NAME"
	fi
fi

pushd "$DL_TMP_DIR"

if [ ! -f "$ELECTRS_DL_FILE_NAME" ]; then
	...
	[ -d "$CACHE_PATH" ] && cp "$ELECTRS_DL_FILE_NAME" "$CACHE_PATH/"
fi
unzip "$ELECTRS_DL_FILE_NAME"
export ELECTRS_EXE="$DL_TMP_DIR"/electrs
chmod +x "$ELECTRS_EXE"

if [ ! -f "$BITCOIND_DL_FILE_NAME" ]; then
	...
	[ -d "$CACHE_PATH" ] && cp "$BITCOIND_DL_FILE_NAME" "$CACHE_PATH/"
fi
