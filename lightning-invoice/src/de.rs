...
impl FromBase32 for ExpiryTime {
	...
	fn from_base32(field_data: &[u5]) -> Result<ExpiryTime, Bolt11ParseError> {
		match parse_int_be::<u64, u5>(field_data, 32).map(ExpiryTime::from_seconds) {
			Some(t) => Ok(t),
	...
	fn from_base32(field_data: &[u5]) -> Result<PrivateRoute, Bolt11ParseError> {
		...
		while !bytes.is_empty() {
			...
			let hop = RouteHintHop {
				...
				short_channel_id: parse_int_be(&channel_id, 256)
					.expect("short chan ID slice too big?"),
				fees: RoutingFees {
					base_msat: parse_int_be(&hop_bytes[41..45], 256).expect("slice too big?"),
					proportional_millionths: parse_int_be(&hop_bytes[45..49], 256)
						.expect("slice too big?"),
				},
