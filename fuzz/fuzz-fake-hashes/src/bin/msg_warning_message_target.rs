...
use lightning_fuzz::msg_targets::msg_warning_message::*;
...
fn main() {
	fuzz!(|data| {
		msg_warning_message_test(&data, test_logger::DevNull {});
	});
}
...
fn main() {
	...
	if std::env::var_os("LDK_FUZZ_SUPPRESS_LOGS").is_some() {
		...
		msg_warning_message_test(&data, test_logger::Stdout {});
	}
}
...
fn run_test_cases() {
	...
	if let Ok(tests) = fs::read_dir("../test_cases/msg_warning_message") {
		for test in tests {
			...
			threads.push((path.file_name().unwrap().to_str().unwrap().to_string(),
				std::thread::spawn(move || {
					...
					let res = if ::std::panic::catch_unwind(move || {
						msg_warning_message_test(&data, panic_logger);
					}).is_err() {
