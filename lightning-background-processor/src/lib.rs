...
mod tests {
	...
	macro_rules! open_channel {
		($node_a: expr, $node_b: expr, $channel_value: expr) => {{
			...
			$node_a.chain_monitor.flush($node_a.chain_monitor.pending_operation_count(), &$node_a.logger);
			...
			$node_b.chain_monitor.flush($node_b.chain_monitor.pending_operation_count(), &$node_b.logger);
	...
	fn test_background_event_handling() {
		...
		nodes[0].chain_monitor.flush(nodes[0].chain_monitor.pending_operation_count(), &nodes[0].logger);
		...
		nodes[1].chain_monitor.flush(nodes[1].chain_monitor.pending_operation_count(), &nodes[1].logger);
